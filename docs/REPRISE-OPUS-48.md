...
6. **CS/moves OW pas finis** : mugshot du Pokémon MANQUANT à l'usage (field_effect_show_mon — cf. topic next-chantier-field-move-show-mon) ; anim de VOL absente pendant Vol ; passer chaque CS (Coupe/Surf/Force✓/Flash/Vol/Éclate-Roc) au callgraph + test en jeu.
7. **SAC/OBJETS 100%** : cf. chantier-sac-objets-complet — restes connus : mail ÉCRITURE (DoEasyChatScreen), PokéblockCase, Berry plant/Wailmer, ÉTIQUETTES BAIES (descriptions), item-combat joueur (message « UNDI utilise X ! » à re-tester).
8. **Anims pas 1:1** : greper `dette|net-effect|approximation|fallback` dans src/battle_anim_*.ts (les « dettes douces » = candidates divergence visuelle), comparer au film vs le script asm (battle_anim_scripts.s). Candidates connues : SoundTask_PlayDoubleCry timing fixe (vs IsCryFinished), PlayCryWithEcho net-effect, pan stéréo ignoré partout.
