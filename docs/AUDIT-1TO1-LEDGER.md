...
| `menu_helpers.c` (ratio 0.47) | 🔧 | Parties pures 1:1 ✅. **`ResetVramOamAndBgCntRegs` DÉDUP ✅** (`9a17dbaa`+`7f7556de`+`13f9e68d`+`5c19ea0f`) : 1 fn partagée (gba-window-system) ← 6 écrans dédupés (summary/trainer_card/wallclock/bag-screen/bag-menu/party-screen), A/B 4 contextes (résumé/carte/sac/party), bug latent bag-menu (clear PLTT RAM manquant) corrigé. EXCLUS à raison : shop (`BuyMenuInitBgs` préserve VRAM), starter_choose (OAM-save custom), option-menu-return (reset BLD/WIN). **Reste (différé, valeur+++faible)** : `ResetAllBgsCoordinates` (inline = SetGpuReg HOFS/VOFS BG0-2 ≠ décomp ChangeBgX/Y BG0-3 → nuance mécanisme), `SetVBlankHBlankCallbacksToNull` (inline partiels = VBlank only). Swap-line sprites = relocalisés `engine/field/swap-line.ts`. |
...
| 2 implémentations **bag** (`bag-screen.ts` supplanté vs `bag-menu.ts` LIVE) | ⬜ | Découvert pendant la dédup VRAM (2026-06-25). bag-menu = vivant (start_menu→OpenBagScreen) ; bag-screen = seuls `preloadBagAssets`/`initItemIconMap` importés, sa logique d'écran (`_initBagBgs`/`CB2_InitBagMenu`/`OpenBagScreen` legacy) est MORTE. Chantier « retirer/fusionner le module supplanté » (≈ easy-chat-render). |
