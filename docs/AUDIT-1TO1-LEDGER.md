...
- **BRANCHE `finale` (autonome, Authored by Claude)** — **CAMPAGNE STRINGS FR HARDCODÉES : 96 → 11**, fichier par fichier, chaque pas prouvé (sonde `scripts/audit-hardcoded-strings.cjs` + tsc=0 + cold-boot/jeu). Construit l'**oracle déterministe** (lexer fiable ; exclut console/throw/dev/auto-gen ; exception `@strings-ignore`/`-file` tracée pour DATA décomp vérifiée). Vrais fixes : sac (bag-menu-ctx+bag-menu = 0, dont textes FAUX « Combien de X à jeter » etc.), player_pc (7, dépôt/retrait/mail), start_menu (save), field_poison, summary, battle menu, labels POKéDEX. **1 vrai bug de contenu** : `B_BUFF_NEGATIVE_FLAVOR` appendait des degrés au lieu des saveurs POKéBLOCK (`gPokeblockWasTooXStringTable`), fixé 1:1 (2 fichiers, `d4bb707f`). **1 module faussement-mort sauvé** : `bag-screen` cru supplanté = en fait VIVANT (combat+PC, import dyn.) → vérif des importeurs avant de couper a évité la régression (`c879c01b`, ledger corrigé). Reste 11 = **stubs de features non-portées** (pokédex/pokénav/welcome 9) + **2 fallbacks défensifs** → le « 0 » strict demande des **ports d'écran** (visuel, hors-déterministe). 17 commits, jamais push.
