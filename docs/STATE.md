...
- T8 🌊 QUALIFICATION FIDÉLITÉ (2026-06-11/12, en cours) : vérificateur __verifyMoveAnim/__fidelitySweep (trace opcodes : sheets/sprites/tasks résolus, durées, résiduels) ; 4 racines systémiques tuées (appel immédiat des tasks 1:1 = args frais, compteurs visual wrap-u8 [++ avant l'appel] tasks ET createsprite, GET_TRUE_SPRITE_INDEX tags absolus, cleanup harness par diff) ; **26 vagues F1-F26 = 64 AnimTasks 1:1 ≈ 313 usages de scripts** (shakes/sways/blends/rotations/affine-task-data/booléens de branchement/famille PSY complète/ExtremeSpeed 5-tasks/FakeOut WIN0...) ; chantier BG ANIM COMPLET 3 étapes (27 fonds + 77 symboles extraits, LoadMoveBg/fadetobg/restorebg réels, ScaryFace opérationnel) ; sous-système task-affine entier (Prepare/RunAffineAnimFromTaskData + Y-offsets) ; monbg réel (MoveBattlerSpriteToBG + Task_UpdateMonBg + clone-blend). DETTE STRICTE (ordre user « pas de compromis ») : OBJ-window plateforme requis pour MetallicShine & co. Le rapport exhaustif : audit-reports/anims/QUALIFICATION-FIDELITE-2026-06-11.md.
