...
// ─── gTypeEffectiveness 1:1 décomp (battle_main.c:335-449) ──────────────────

/** 1:1 décomp `const u8 gTypeEffectiveness[336]` (battle_main.c:335-449).
 *  Triplets plats (atkType, defType, multiplier) — accès décomp
 *  TYPE_EFFECT_ATK/DEF_TYPE/MULTIPLIER(i) = [i+0]/[i+1]/[i+2] (battle_main.h:26-28).
 *  Le triplet TYPE_FORESIGHT sépare la table principale de la section
 *  « after Foresight » (Normal/Combat touchent Spectre) ; fin = TYPE_ENDTABLE.
 *  Multiplicateurs TYPE_MUL_* : include/battle_main.ts (battle_main.h:31-34).
 *  Oracle de fidélité : `scripts/audit-type-chart.cjs` (token-par-token). */
...
// ─── gTrainerMoneyTable 1:1 décomp (battle_main.c:474-532) ──────────────────

/** 1:1 décomp `struct TrainerMoney` (battle_main.h). */
export interface TrainerMoney { classId: number; value: number; }

/** 1:1 décomp `const struct TrainerMoney gTrainerMoneyTable[]` (battle_main.c:474-532).
 *  « This is a factor in how much money you get for beating a trainer. »
 *  55 entrées + sentinelle 0xFF (default 5). Lookup = boucle inline de
 *  Cmd_getmoneyreward (battle_script_commands.c:5621-5625, cf. bsc.ts).
 *  Oracle de fidélité : `scripts/audit-trainer-money-table.cjs`. */
export const gTrainerMoneyTable: TrainerMoney[] = [
  ...
  { classId: 0xFF, value: 5 },  // 1:1 sentinelle — default pour classes non listées.
];
...
import {
  ...
  ABILITY_NONE,
  // TYPE_FLYING/TYPE_STEEL : fournis par l'import include/constants/pokemon du
  // haut (lot 25, table gTypeEffectiveness) — doublon re-export retiré.
  BIT_FLANK,
} from './engine/battle/constants';
...
// ─── End Link Battle 1:1 (battle_main.c:2177-2383) — ex-battle-link-end.ts ──
// Transcription INERTE rapatriée à l'unification (lot 9) : link battle =
// non-applicable offline, portée pour la complétude 1:1 (cascades gLinkPlayers/
// SetCloseLinkCallback = dette R3 explicite dans les corps).
import {
  gBattleTypeFlags as _gBattleTypeFlags_BLE,
  gBattleCommunication as _gBattleCommunication_BLE,
  setBattleTypeFlags as _setBattleTypeFlags_BLE,
} from './engine/battle/state';
import {
  BATTLE_TYPE_FRONTIER as _BATTLE_TYPE_FRONTIER_BLE,
  BATTLE_TYPE_MULTI as _BATTLE_TYPE_MULTI_BLE,
} from '../include/battle';
import {
  getRuntime as _getRuntime_BLE, FreeMonSpritesGfx as _FreeMonSpritesGfx_BLE,
  PALETTES_ALL as _PALETTES_ALL_BLE,
} from '../harness/runtime/decomp-globals';

// ─── Constants 1:1 décomp ──────────────────────────────────────────────────

/** 1:1 décomp `BATTLE_TYPE_LINK_IN_BATTLE` (battle.h:60) = bit 5. AUDIT FIX : était bit 18. */
const BATTLE_TYPE_LINK_IN_BATTLE_BLE = 1 << 5;

/** 1:1 décomp `MULTIUSE_STATE` = 0 dans _gBattleCommunication_BLE. */
const MULTIUSE_STATE_BLE = 0;

/** 1:1 décomp `RGB_BLACK` = 0. */
const RGB_BLACK = 0;

/** 1:1 décomp `VERSION_EMERALD` = 3 (global.h:10). AUDIT FIX : était 5 (= VERSION_LEAFGREEN). */
const VERSION_EMERALD_BLE = 3;
...
/** 1:1 décomp `SetHBlankCallback(cb)`. */
function _SetHBlankCallback_BLE(_cb: (() => void) | null): void {
  // Dette R3 : HBlank callback (= notre runtime web utilise différentes mécaniques).
}

/** 1:1 décomp `SetVBlankCallback(cb)`. */
function _SetVBlankCallback_BLE(_cb: (() => void) | null): void {
  // Dette R3 : VBlank callback registry.
}

/** 1:1 décomp `SetMainCallback2(cb)`. */
function _SetMainCallback2_BLE(_cb: (() => void) | null): void {
  // Dette R3 : CB2 dispatch (= notre runtime n'a pas CB2 layer).
}

/** 1:1 décomp `FreeBattleResources()` + `FreeBattleSpritesData()`. */
function _FreeBattleResources_BLE(): void {
  // Dette R3 : noop pour notre runtime.
}
function _FreeBattleSpritesData_BLE(): void {
  // Dette R3 : noop.
}
function _FreeAllWindowBuffers_BLE(): void {
  // Dette R3 : window cleanup.
}
function _CpuFill32_BLE(_value: number, _dest: unknown, _size: number): void {
  // Dette R3 : VRAM clear via DMA. Notre runtime gère implicit.
}

/** 1:1 décomp `BeginNormalPaletteFade(palettes, delay, startY, endY, color)`. */
function _BeginNormalPaletteFade_BLE(palettes: number, delay: number, startY: number, endY: number, color: number): void {
  const rt = _getRuntime_BLE();
  rt?.BeginNormalPaletteFade?.(
    palettes as unknown as string, delay, startY, endY, color as unknown as string,
  );
}
...
/** 1:1 décomp `gPaletteFade.active`. */
function _isPaletteFadeActive_BLE(): boolean {
  return _getRuntime_BLE()?.gPaletteFade?.active ?? false;
}
...
function _getLinkPlayerVersion_BLE(_idx: number): number {
  // Dette R3 : link players data. Pour now : assume Emerald.
  return VERSION_EMERALD_BLE;
}

/** 1:1 décomp `gReceivedRemoteLinkPlayers`. */
function _getReceivedRemoteLinkPlayers_BLE(): number {
...
/** 1:1 décomp `gWirelessCommType`. */
function _getWirelessCommType_BLE(): number {
  return 0;  // No wireless.
}

/** 1:1 décomp `CreateTask(taskFn, priority)`. */
function _CreateTask_BLE(_taskFn: () => void, _priority: number): number {
...
/** 1:1 décomp `IsLinkTaskFinished()`. */
function _IsLinkTaskFinished_BLE(): boolean {
...
function _ResetPaletteFade_BLE(): void {
  // Dette R3 : palette fade state reset.
}

/** 1:1 décomp `InitBattleBgsVideo()`. */
function _InitBattleBgsVideo_BLE(): void {
...
function _ResetSpriteData_BLE(): void {
  const r = _getRuntime_BLE();
  r?.gSprites?.fill(undefined);
}
function _ResetTasks_BLE(): void {
  const r = _getRuntime_BLE();
  r?.ResetTasks();
}
...
function _FreeAllSpritePalettes_BLE(): void {
  // Dette R3.
}

/** 1:1 décomp `InitLinkBattleVsScreen` task fn. */
function _InitLinkBattleVsScreen_BLE(): void {
...
/** 1:1 décomp `SetGpuReg(reg, value)`. */
function _SetGpuReg_BLE(reg: number, value: number): void {
  const rt = _getRuntime_BLE();
...
const REG_OFFSET_MOSAIC_BLE = 0x4C;
const REG_OFFSET_WIN0H_BLE = 0x40;
const REG_OFFSET_WIN0V_BLE = 0x44;
const REG_OFFSET_WININ_BLE = 0x48;
const REG_OFFSET_WINOUT_BLE = 0x4A;

// Display constants.
const DISPLAY_WIDTH_BLE = 240;
const DISPLAY_HEIGHT_BLE = 160;
...
export function CB2_InitEndLinkBattle(): void {
  _SetHBlankCallback_BLE(null);
  _SetVBlankCallback_BLE(null);
  _setBattleTypeFlags_BLE(_gBattleTypeFlags_BLE & ~BATTLE_TYPE_LINK_IN_BATTLE_BLE);

  if (_gBattleTypeFlags_BLE & _BATTLE_TYPE_FRONTIER_BLE) {
    // Frontier path : direct exit.
    const m = (globalThis as Record<string, unknown>).__battleMainFunctions as {
      getMainSavedCallback?: () => (() => void) | null;
    } | undefined;
    _SetMainCallback2_BLE(m?.getMainSavedCallback?.() ?? null);
    _FreeBattleResources_BLE();
    _FreeBattleSpritesData_BLE();
    _FreeMonSpritesGfx_BLE();
  } else {
    // Standard link battle end : full setup VS screen.
    _CpuFill32_BLE(0, null /* VRAM */, 0x18000 /* VRAM_SIZE */);

    _SetGpuReg_BLE(REG_OFFSET_MOSAIC_BLE, 0);
    _SetGpuReg_BLE(REG_OFFSET_WIN0H_BLE, DISPLAY_WIDTH_BLE);
    _SetGpuReg_BLE(REG_OFFSET_WIN0V_BLE, _WIN_RANGE(DISPLAY_HEIGHT_BLE / 2, DISPLAY_HEIGHT_BLE / 2 + 1));
    _SetGpuReg_BLE(REG_OFFSET_WININ_BLE, 0);
    _SetGpuReg_BLE(REG_OFFSET_WINOUT_BLE, 0);
    ...
    if (vbm?.battleVBlankState) {
      vbm.battleVBlankState.win0h = DISPLAY_WIDTH_BLE;
      vbm.battleVBlankState.win0v = _WIN_RANGE(DISPLAY_HEIGHT_BLE / 2, DISPLAY_HEIGHT_BLE / 2 + 1);
    ...
    _ResetPaletteFade_BLE();
    _InitBattleBgsVideo_BLE();
    _LoadCompressedPalette(null /* gBattleTextboxPalette */, 0, 64);
    _LoadBattleMenuWindowGfx();
    _ResetSpriteData_BLE();
    _ResetTasks_BLE();
    _DrawBattleEntryBackground();
    _SetGpuReg_BLE(REG_OFFSET_WINOUT_BLE, 0x3F);  // WINOUT_WIN01_BG_ALL + OBJ + CLR
    _FreeAllSpritePalettes_BLE();
    // gReservedSpritePaletteCount = MAX_BATTLERS_COUNT.
    _SetVBlankCallback_BLE(_getVBlankCB_Battle());

    // 1:1 décomp ll. 2242-2247 : create VS screen task + buffer health.
    const taskId = _CreateTask_BLE(_InitLinkBattleVsScreen_BLE, 0);
    // gTasks[taskId].data[1] = 0x10E, data[2] = 0x5A, data[5] = 1.
    _BufferPartyVsScreenHealth_AtEnd(taskId);

    _SetMainCallback2_BLE(CB2_EndLinkBattle);
    _gBattleCommunication_BLE[MULTIUSE_STATE] = 0;
  }
}
...
export function EndLinkBattleInSteps(): void {
  switch (_gBattleCommunication_BLE[MULTIUSE_STATE]) {
    case 0:
      // 1:1 décomp ll. 2270-2276.
      _ShowBg_BLE(0); _ShowBg_BLE(1); _ShowBg_BLE(2);
      _gBattleCommunication_BLE[1] = 0xFF;
      _gBattleCommunication_BLE[MULTIUSE_STATE]++;
      break;
    case 1:
      // 1:1 décomp ll. 2277-2283 : countdown → fade.
      if (--_gBattleCommunication_BLE[1] === 0) {
        _BeginNormalPaletteFade_BLE(_PALETTES_ALL_BLE, 0, 0, 16, RGB_BLACK);
        _gBattleCommunication_BLE[MULTIUSE_STATE]++;
      }
      break;
    case 2:
      // 1:1 décomp ll. 2284-2333 : palette fade end → branch.
      if (!_isPaletteFadeActive_BLE()) {
        let battlerCount: number;
        _gMain_anyLinkBattlerHasFrontierPass = _RecordedBattle_GetFrontierPassFlag();

        if (_gBattleTypeFlags_BLE & _BATTLE_TYPE_MULTI_BLE) battlerCount = 4;
        else battlerCount = 2;

        // 1:1 décomp l. 2296 : check Emerald version all players.
        let i = 0;
        for (; i < battlerCount && (_getLinkPlayerVersion_BLE(i) & 0xFF) === VERSION_EMERALD_BLE; i++);
        ...
        if (!disableRecord && i === battlerCount) {
          if (_FlagGet(FLAG_SYS_FRONTIER_PASS)) {
            ...
            _FreeAllWindowBuffers_BLE();
            _SetMainCallback2_BLE(_CB2_InitAskRecordBattle);
            ...
            _FreeBattleSpritesData_BLE();
            _FreeMonSpritesGfx_BLE();
          } else if (_getReceivedRemoteLinkPlayers_BLE() === 0) {
            // 1:1 décomp ll. 2314-2320 : reconnect with link players.
            _CreateTask_BLE(_Task_ReconnectWithLinkPlayers, 5);
            _gBattleCommunication_BLE[MULTIUSE_STATE]++;
          } else {
            _gBattleCommunication_BLE[MULTIUSE_STATE]++;
          }
    ...
    case 3:
      ...
      _CpuFill32_BLE(0, null /* VRAM */, 0x18000);
      for (let i = 0; i < 2; i++) _LoadChosenBattleElement(i);
      _BeginNormalPaletteFade_BLE(_PALETTES_ALL_BLE, 0, 16, 0, RGB_BLACK);
      _gBattleCommunication_BLE[MULTIUSE_STATE]++;
      break;
    case 4:
      if (!_isPaletteFadeActive_BLE()) _gBattleCommunication_BLE[MULTIUSE_STATE]++;
      break;
    case 5:
    ...
    case 7:
      if (!_IsTextPrinterActive(0)) {
        if (_IsLinkTaskFinished_BLE()) _gBattleCommunication_BLE[MULTIUSE_STATE]++;
      }
      break;
    case 8:
      if (!_getWirelessCommType_BLE()) _SetCloseLinkCallback();
      _gBattleCommunication_BLE[MULTIUSE_STATE]++;
      break;
    case 9:
      if (!_gMain_anyLinkBattlerHasFrontierPass
          || _getWirelessCommType_BLE()
          || _getReceivedRemoteLinkPlayers_BLE() !== 1) {
...
function _ShowBg_BLE(_bgId: number): void {
