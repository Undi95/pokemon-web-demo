...
export function DetermineCyclingRoadResults(...args: any[]): any {
  let result: any = null;
...
export function FinishCyclingRoadChallenge(): any {
  let numFrames = gMain.vblankCounter1 - sBikeCyclingTimer;
...
export function RecordCyclingRoadResults(...args: any[]): any {
...
export function CountSSTidalStep(...args: any[]): any {
...
export function GetSSTidalLocation(...args: any[]): any {
  let varCruiseStepCount = GetVarPointer(VAR_CRUISE_STEP_COUNT);
...
export function LoadLinkPartnerObjectEventSpritePalette(...args: any[]): any {
  let adjustedPaletteNum: any = null;
      ...
      {
          let obj = GetObjectEventIdByLocalIdAndMap(localEventId, gSaveBlock1Ptr.location.mapNum, gSaveBlock1Ptr.location.mapGroup);
          if (obj != OBJECT_EVENTS_COUNT)
          {
              let spriteId = gObjectEvents[obj].spriteId;
...
export function Task_PetalburgGymSlideOpenRoomDoors(...args: any[]): any {
...
export function PetalburgGymSetDoorMetatiles(...args: any[]): any {
...
export function DoPCTurnOnEffect(): any {
  if (FuncIsActiveTask(Task_PCTurnOnEffect) != TRUE)
      {
          let taskId = CreateTask(Task_PCTurnOnEffect, 8);
...
export function Task_PCTurnOnEffect(...args: any[]): any {
...
export function PCTurnOnEffect(...args: any[]): any {
...
export function PCTurnOnEffect_SetMetatile(...args: any[]): any {
  let metatileId = 0;
      if (isScreenOn)
      {
...
export function PCTurnOffEffect(): any {
  let dx = 0;
      ...
      let playerDirection = GetPlayerFacingDirection();
      switch (playerDirection)
      {
      case DIR_NORTH:
          dx = 0;
...
export function DoLotteryCornerComputerEffect(): any {
  if (FuncIsActiveTask(Task_LotteryCornerComputerEffect) != TRUE)
      {
          let taskId = CreateTask(Task_LotteryCornerComputerEffect, 8);
          gTasks[taskId].tPaused = FALSE;
          gTasks[taskId].tTaskId = taskId;
          gTasks[taskId].tFlickerCount = 0;
          gTasks[taskId].tTimer = 0;
...
export function Task_LotteryCornerComputerEffect(...args: any[]): any {
  let task: any = gTasks[taskId];
      if (!task.tPaused)
...
export function LotteryCornerComputerEffect(...args: any[]): any {
  if (task.tTimer == 6)
      {
          task.tTimer = 0;
          if (task.tIsScreenOn)
...
export function SetShoalItemFlag(...args: any[]): any {
...
export function ShakeCamera(): any {
  let taskId = CreateTask(Task_ShakeCamera, 9);
...
export function Task_ShakeCamera(...args: any[]): any {
...
export function StopCameraShake(...args: any[]): any {
  DestroyTask(taskId);
      ScriptContext_Enable();
}
...
export function MoveElevator(): any {
  const sElevatorTripLength: any = [ 8, 16, 24, 32, 38, 46, 52, 56, 57 ];
      ...
      let data = gTasks[CreateTask(Task_MoveElevator, 9)].data;
...
export function Task_MoveElevator(...args: any[]): any {
  let data = gTasks[taskId].data;
      tTimer++;
      if (tTimer % 3 == 0)
      {
...
export function Task_MoveElevatorWindowLights(...args: any[]): any {
  let x, y;
      let data = gTasks[taskId].data;
...
export function UsedPokemonCenterWarp(): any {
  const sPokemonCenters: any = [
      ...
      let i: any = null;
      let map = (gLastUsedWarp.mapGroup << 8) + gLastUsedWarp.mapNum;
...
export function UpdateFrontierManiac(...args: any[]): any {
  let var = GetVarPointer(VAR_FRONTIER_MANIAC_FACILITY);
      *var += daysSince;
...
export function ShowScrollableMultichoice(): any {
  let taskId = CreateTask(Task_ShowScrollableMultichoice, 8);
      let task: any = gTasks[taskId];
...
export function Task_ShowScrollableMultichoice(...args: any[]): any {
  let width: any = null;
      let i, windowId;
      ...
      {
          let text = sScrollableMultichoiceOptions[gSpecialVar_0x8004][i];
      ...
      {
          let adjustedLeft = MAX_MULTICHOICE_WIDTH + 1 - task.tWidth;
          if (adjustedLeft < 0)
              task.tLeft = 0;
          else
...
export function ScrollableMultichoice_MoveCursor(...args: any[]): any {
  let taskId: any = null;
      PlaySE(SE_SELECT);
...
export function ScrollableMultichoice_ProcessInput(...args: any[]): any {
  let task: any = gTasks[taskId];
      let input = ListMenu_ProcessInput(task.tListTaskId);
...
export function CloseScrollableMultichoice(...args: any[]): any {
  let selection: any = null;
      let task: any = gTasks[taskId];
      ListMenuGetCurrentItemArrayId(task.tListTaskId, selection);
...
export function Task_ScrollableMultichoice_WaitReturnToList(...args: any[]): any {
...
export function Task_ScrollableMultichoice_ReturnToList(...args: any[]): any {
  LockPlayerFieldControls();
      ScrollableMultichoice_UpdateScrollArrows(taskId);
      gTasks[taskId].func = ScrollableMultichoice_ProcessInput;
}
...
export function ScrollableMultichoice_UpdateScrollArrows(...args: any[]): any {
...
export function ScrollableMultichoice_RemoveScrollArrows(...args: any[]): any {
  let task: any = gTasks[taskId];
      if (task.tMaxItemsOnScreen != task.tNumItems)
...
export function UpdateFrontierGambler(...args: any[]): any {
  let var = GetVarPointer(VAR_FRONTIER_GAMBLER_CHALLENGE);
      *var += daysSince;
...
export function FrontierGamblerSetWonOrLost(...args: any[]): any {
  const sFrontierChallenges: any = [
      ...
      let battleMode = VarGet(VAR_FRONTIER_BATTLE_MODE);
      let challenge = VarGet(VAR_FRONTIER_GAMBLER_SET_CHALLENGE);
      let frontierFacilityId = VarGet(VAR_FRONTIER_FACILITY);
...
export function FillFrontierExchangeCornerWindowAndItemIcon(...args: any[]): any {
...
export function ShowFrontierExchangeCornerItemIcon(...args: any[]): any {
  FreeSpriteTilesByTag(TAG_ITEM_ICON);
      FreeSpritePaletteByTag(TAG_ITEM_ICON);
...
export function HideFrontierExchangeCornerItemIcon(...args: any[]): any {
  if (sScrollableMultichoice_ItemSpriteId != MAX_SPRITES)
      {
          switch (menu)
          {
...
export function ShowBattleFrontierTutorWindow(...args: any[]): any {
...
export function ShowBattleFrontierTutorMoveDescription(...args: any[]): any {
...
export function Task_DeoxysRockInteraction(...args: any[]): any {
...
export function ChangeDeoxysRockLevel(...args: any[]): any {
...
export function WaitForDeoxysRockMovement(...args: any[]): any {
...
export function SetPCBoxToSendMon(...args: any[]): any {
...
export function GetAbnormalWeatherMapNameAndType(): any {
  const sAbnormalWeatherMapNumbers: any = [
      ...
      let abnormalWeather = VarGet(VAR_ABNORMAL_WEATHER_LOCATION);
...
export function AbnormalWeatherHasExpired(): any {
  const sAbnormalWeatherMapNumbers: any = [
      ...
      let steps = VarGet(VAR_ABNORMAL_WEATHER_STEP_COUNTER);
      let abnormalWeather = VarGet(VAR_ABNORMAL_WEATHER_LOCATION);
...
export function Task_LinkRetireStatusWithBattleTowerPartner(...args: any[]): any {
...
export function Task_LoopWingFlapSE(...args: any[]): any {
  let data = gTasks[taskId].data;
...
export function CloseBattlePikeCurtain(): any {
  let taskId = CreateTask(Task_CloseBattlePikeCurtain, 8);
...
export function Task_CloseBattlePikeCurtain(...args: any[]): any {
  let x, y;
      let data = gTasks[taskId].data;
...
export function TryGainNewFanFromCounter(...args: any[]): any {
...
export function BufferFanClubTrainerName_(...args: any[]): any {
