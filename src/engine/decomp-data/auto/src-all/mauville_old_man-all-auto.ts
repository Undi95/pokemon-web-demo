...
export function SetMauvilleOldMan(): any {
  let trainerId = (gSaveBlock2Ptr.playerTrainerId[1] << 8) | gSaveBlock2Ptr.playerTrainerId[0];
...
export function PrepareSongText(): any {
  let bard: any = gSaveBlock1Ptr.oldMan.bard;
      let lyrics = !gSpecialVar_0x8004 ? bard.songLyrics : bard.newSongLyrics;
      let wordEnd = gStringVar4;
      let str = wordEnd;
  ...
  let giddy: any = gSaveBlock1Ptr.oldMan.giddy;
      ...
      for (i = 0; i < GIDDY_MAX_TALES; i++)
      {
          ...
          else
          {
              ...
              let randWord = Random() % totalWords;
...
export function StartBardSong(...args: any[]): any {
  let taskId = CreateTask(Task_BardSong, 80);
...
export function DisableTextPrinters(...args: any[]): any {
  gDisableTextPrinters = TRUE;
}

/** static void DrawSongTextWindow(const u8 *str) */
export function DrawSongTextWindow(...args: any[]): any {
...
export function BardSing(...args: any[]): any {
  switch (task.tState)
      {
      ...
      {
          let easyChatWord = song.lyrics[song.lyricsIndex];
      ...
      case BARD_STATE_WAIT_WORD:
      {
          ...
          case SOUND_STATE_START:
              ...
              if (template.songId < NUM_PHONEME_SONGS)
              {
                  ...
                  let phonemeTripletId = template.songId / 3;
...
export function Task_BardSong(...args: any[]): any {
  let task: any = gTasks[taskId];
      ...
      {
          let bard: any = gSaveBlock1Ptr.oldMan.bard;
          let str = gStringVar4[task.tCharIndex];
          let wordLen = 0;
...
export function SanitizeMauvilleOldManForRuby(...args: any[]): any {
  let i: any = null;
...
export function SanitizeReceivedEmeraldOldMan(...args: any[]): any {
  const playerName: any[] = [];
      let i: any = null;
...
export function SanitizeReceivedRubyOldMan(...args: any[]): any {
  let isRuby = (version == VERSION_SAPPHIRE || version == VERSION_RUBY);
...
export function StorytellerGetGameStat(...args: any[]): any {
  if (stat == 50)
...
export function StorytellerGetRecordedTrainerStat(...args: any[]): any {
  let ptr = sStorytellerPtr.statValues[trainer];

      return ptr[0] | (ptr[1] << 8) | (ptr[2] << 16) | (ptr[3] << 24);
}

/** static void StorytellerSetRecordedTrainerStat(u32 trainer, u32 val) */
export function StorytellerSetRecordedTrainerStat(...args: any[]): any {
  let ptr = sStorytellerPtr.statValues[trainer];
...
export function HasTrainerStatIncreased(...args: any[]): any {
...
export function GetStoryByStattellerPlayerName(...args: any[]): any {
  let name = sStorytellerPtr.trainerNames[player];
...
export function StorytellerSetPlayerName(...args: any[]): any {
  let name = sStorytellerPtr.trainerNames[player];
...
export function StorytellerRecordNewStat(...args: any[]): any {
...
export function ScrambleStatList(...args: any[]): any {
  let i: any = null;
...
export function StorytellerInitializeRandomStat(): any {
  const storyIds: any[] = [];
      ...
      {
          let stat = sStorytellerStories[storyIds[i]].stat;
          let minVal = sStorytellerStories[storyIds[i]].minVal;
...
export function StorytellerDisplayStory(...args: any[]): any {
  let stat = sStorytellerPtr.gameStatIDs[player];
...
export function PrintStoryList(): any {
  let i: any = null;
      let width = GetStringWidth(FONT_NORMAL, gText_Exit, 0);
      for (i = 0; i < NUM_STORYTELLER_TALES; i++)
      {
          let curWidth: any = null;
          let gameStatID = sStorytellerPtr.gameStatIDs[i];
...
export function Task_StoryListMenu(...args: any[]): any {
  let task: any = gTasks[taskId];
