...
export function BattleArena_ShowJudgmentWindow(...args: any[]): any {
  let i: any = null;
      let result = ARENA_RESULT_RUNNING;
      switch (*state)
      {
      case 0:
      ...
      case 3:
          if (!IsDma3ManagerBusyWithBgCopy())
          {
              ...
              for (i = 0; i < 8; i++)
              {
                  let spriteId = CreateSprite(sSpriteTemplate_JudgmentIcon, 64 + i * 16, 84, 0);
...
export function ShowJudgmentSprite(...args: any[]): any {
  let animNum = 0;
      let pointsPlayer = 0;
      let pointsOpponent = 0;
      let mindPoints = gBattleStruct.arenaMindPoints;
      let skillPoints = gBattleStruct.arenaSkillPoints;
      let hpAtStart = gBattleStruct.arenaStartHp;
...
export function SpriteCB_JudgmentIcon(...args: any[]): any {
...
export function BattleArena_AddMindPoints(...args: any[]): any {
  let mindPoints = gBattleStruct.arenaMindPoints;
      mindPoints[battler] += sMindRatings[gCurrentMove];
}

/** void BattleArena_AddSkillPoints(u8 battler) */
export function BattleArena_AddSkillPoints(...args: any[]): any {
  let skillPoints = gBattleStruct.arenaSkillPoints;

      if (gHitMarker & HITMARKER_OBEYS)
      {
          let failedMoveBits = gBattleStruct.alreadyStatusedMoveAttempt;
...
export function BattleArena_DeductSkillPoints(...args: any[]): any {
  let skillPoints = gBattleStruct.arenaSkillPoints;
