// AUTO-GENERATED by scripts/transpile-decomp-all.mjs
// Source : src/battle_arena.c
// Generated : 2026-05-09
// DO NOT EDIT — re-run `node scripts/transpile-decomp-all.mjs` to refresh.
//
// Total : 17 functions extracted from src/battle_arena.c.
//
// IMPORTANT :
//   - Tous les imports manquants : marqués @ts-nocheck (= pas de typecheck).
//   - Les bodies référencent des helpers (= FaceDirection, InitMovementNormal, etc.)
//     qui doivent être bridged vers nos TS impls. Cf. memory/audit pour la liste.
//   - Pour activer une fonction, la importer depuis runtime engine + bridge
//     ses callsTo manquants.

/* eslint-disable */
// @ts-nocheck


// ─── AUTO-INJECTED file-scope vars (= EWRAM/IWRAM static C decls) ──
let gBattle_WIN0H: any = null;
let gBattle_WIN0V: any = null;
let gTrainerBattleOpponent_A: any = null;
/** void CallBattleArenaFunction(void) */
export function CallBattleArenaFunction(): any {
  sArenaFunctions[gSpecialVar_0x8004]();
}

/** u8 BattleArena_ShowJudgmentWindow(u8 *state) */
export function BattleArena_ShowJudgmentWindow(state: any): any {
  let i: any = null;
      let result: any = (0);
      switch (state)
      {
      case 0:
          BeginNormalPaletteFade(0x7FFFFF1C, 4, 0, 8, (RGB(0, 0, 0)));
          SetGpuReg(REG_OFFSET_WININ, (WININ_WIN0_ALL & ~WININ_WIN0_BG0) | WININ_WIN1_ALL);
          LoadCompressedSpriteSheet(sBattleArenaJudgmentSymbolsSpriteSheet);
          LoadCompressedPalette(gBattleArenaJudgmentSymbolsPalette, OBJ_PLTT_ID(15), PLTT_SIZE_4BPP);
          gBattle_WIN0H = 0xFF;
          gBattle_WIN0V = 0x70;
          state++;
          break;
      case 1:
          if (!gPaletteFade.active)
          {
              HandleBattleWindow(5, 0, 24, 13, 0);
              state++;
          }
          break;
      case 2:
          if (!IsDma3ManagerBusyWithBgCopy())
          {
              gBattleTextBuff1[0] = (0xA1);
              gBattleTextBuff1[1] = (0xFF);
              gBattleTextBuff2[0] = (0xA1);
              gBattleTextBuff2[1] = (0xFF);
              BattleStringExpandPlaceholdersToDisplayedString(gText_PlayerMon1Name);
              BattlePutTextOnWindow(gDisplayedStringBattle, (15));
              BattlePutTextOnWindow(gText_Vs, (16));
              BattleStringExpandPlaceholdersToDisplayedString(gText_OpponentMon1Name);
              BattlePutTextOnWindow(gDisplayedStringBattle, (17));
              BattlePutTextOnWindow(gText_Mind, (18));
              BattlePutTextOnWindow(gText_Skill, (19));
              BattlePutTextOnWindow(gText_Body, (20));
              BattleStringExpandPlaceholdersToDisplayedString(gText_Judgment);
              BattlePutTextOnWindow(gDisplayedStringBattle, (21));
              state++;
          }
          break;
      case 3:
          if (!IsDma3ManagerBusyWithBgCopy())
          {
              SetGpuReg(REG_OFFSET_WININ, WININ_WIN0_ALL | WININ_WIN1_ALL);

               
              for (i = 0; i < 8; i++)
              {
                  let spriteId: any = CreateSprite(sSpriteTemplate_JudgmentIcon, 64 + i * 16, 84, 0);
                  StartSpriteAnim(gSprites[spriteId], ANIM_ICON_LINE);
              }
              result = (1);
              state++;
          }
          break;
      case 4:
          PlaySE((265));
          ShowJudgmentSprite(80, 40, (0), B_POSITION_PLAYER_LEFT);
          ShowJudgmentSprite(160, 40, (0), B_POSITION_OPPONENT_LEFT);
          BattleStringExpandPlaceholdersToDisplayedString(gText_Judgment);
          BattlePutTextOnWindow(gDisplayedStringBattle, (21));
          state++;
          result = (1);
          break;
      case 5:
          PlaySE((265));
          ShowJudgmentSprite(80, 56, (1), B_POSITION_PLAYER_LEFT);
          ShowJudgmentSprite(160, 56, (1), B_POSITION_OPPONENT_LEFT);
          BattleStringExpandPlaceholdersToDisplayedString(gText_Judgment);
          BattlePutTextOnWindow(gDisplayedStringBattle, (21));
          state++;
          result = (1);
          break;
      case 6:
          PlaySE((265));
          ShowJudgmentSprite(80, 72, (2), B_POSITION_PLAYER_LEFT);
          ShowJudgmentSprite(160, 72, (2), B_POSITION_OPPONENT_LEFT);
          BattleStringExpandPlaceholdersToDisplayedString(gText_Judgment);
          BattlePutTextOnWindow(gDisplayedStringBattle, (21));
          state++;
          result = (1);
          break;
      case 7:
          PlaySE((266));
          if (gBattleTextBuff1[0] > gBattleTextBuff2[0])
          {
              result = (2);
              gBattleScripting.battler = 0;
          }
          else if (gBattleTextBuff1[0] < gBattleTextBuff2[0])
          {
              result = (3);
              gBattleScripting.battler = 1;
          }
          else
          {
              result = (4);
          }
          state++;
          break;
      case (8):
           
          state++;
          break;
      case (8) + 1:
          SetGpuReg(REG_OFFSET_WININ, (WININ_WIN0_ALL & ~WININ_WIN0_BG0) | WININ_WIN1_ALL);
          HandleBattleWindow(5, 0, 24, 13, ((1 << 0)));
          CopyBgTilemapBufferToVram(0);
          m4aMPlayVolumeControl(gMPlayInfo_BGM, TRACKS_ALL, 256);
          BeginNormalPaletteFade(0x7FFFFF1C, 4, 8, 0, (RGB(0, 0, 0)));
          state++;
          break;
      case (8) + 2:
          if (!gPaletteFade.active)
          {
              SetGpuReg(REG_OFFSET_WININ, WININ_WIN0_ALL | WININ_WIN1_ALL);
              FreeSpriteTilesByTag((1000));
              result = (1);
              state++;
          }
          break;
      }

      return result;
}

/** static void ShowJudgmentSprite(u8 x, u8 y, u8 category, u8 battler) */
export function ShowJudgmentSprite(x: any, y: any, category: any, battler: any): any {
  let animNum: any = 0;
      let pointsPlayer: any = 0;
      let pointsOpponent: any = 0;
      let mindPoints: any = gBattleStruct.arenaMindPoints;
      let skillPoints: any = gBattleStruct.arenaSkillPoints;
      let hpAtStart: any = gBattleStruct.arenaStartHp;

      switch (category)
      {
      case (0):
          pointsPlayer = mindPoints[battler];
          pointsOpponent = mindPoints[BATTLE_OPPOSITE(battler)];
          break;
      case (1):
          pointsPlayer = skillPoints[battler];
          pointsOpponent = skillPoints[BATTLE_OPPOSITE(battler)];
          break;
      case (2):
          pointsPlayer = (gBattleMons[battler].hp * 100) / hpAtStart[battler];
          pointsOpponent = (gBattleMons[BATTLE_OPPOSITE(battler)].hp * 100) / hpAtStart[BATTLE_OPPOSITE(battler)];
          break;
      }

      if (pointsPlayer > pointsOpponent)
      {
          animNum = ANIM_ICON_CIRCLE;
           
          if (battler != 0)
              gBattleTextBuff2[0] += 2;
          else
              gBattleTextBuff1[0] += 2;
      }
      else if (pointsPlayer == pointsOpponent)
      {
          animNum = ANIM_ICON_TRIANGLE;
           
          if (battler != 0)
              gBattleTextBuff2[0] += 1;
          else
              gBattleTextBuff1[0] += 1;
      }
      else
      {
          animNum = ANIM_ICON_X;
      }

      pointsPlayer = CreateSprite(sSpriteTemplate_JudgmentIcon, x, y, 0);
      StartSpriteAnim(gSprites[pointsPlayer], animNum);
}

/** static void SpriteCB_JudgmentIcon(struct Sprite *sprite) */
export function SpriteCB_JudgmentIcon(sprite: any): any {
  if (gBattleCommunication[0] > (8))
          DestroySprite(sprite);
}

/** void BattleArena_InitPoints(void) */
export function BattleArena_InitPoints(): any {
  let mindPoints: any = gBattleStruct.arenaMindPoints;
      let skillPoints: any = gBattleStruct.arenaSkillPoints;
      let hpAtStart: any = gBattleStruct.arenaStartHp;

      mindPoints[0] = 0;
      mindPoints[1] = 0;
      skillPoints[0] = 0;
      skillPoints[1] = 0;
      hpAtStart[0] = gBattleMons[0].hp;
      hpAtStart[1] = gBattleMons[1].hp;
}

/** void BattleArena_AddMindPoints(u8 battler) */
export function BattleArena_AddMindPoints(battler: any): any {
  let mindPoints: any = gBattleStruct.arenaMindPoints;
      mindPoints[battler] += sMindRatings[gCurrentMove];
}

/** void BattleArena_AddSkillPoints(u8 battler) */
export function BattleArena_AddSkillPoints(battler: any): any {
  let skillPoints: any = gBattleStruct.arenaSkillPoints;

      if (gHitMarker & ((1 << 25)))
      {
          let failedMoveBits: any =gBattleStruct.alreadyStatusedMoveAttempt;
          if (failedMoveBits & gBitTable[battler])
          {
              failedMoveBits &= ~(gBitTable[battler]);
              skillPoints[battler] -= 2;
          }
          else if (gMoveResultFlags & ((((1 << 0)) | ((1 << 3)) | ((1 << 5)))))
          {
              if (!(gMoveResultFlags & ((1 << 0))) || gBattleCommunication[(6)] != (1))
                  skillPoints[battler] -= 2;
          }
          else if ((gMoveResultFlags & ((1 << 1))) && (gMoveResultFlags & ((1 << 2))))
          {
              skillPoints[battler] += 1;
          }
          else if (gMoveResultFlags & ((1 << 1)))
          {
              skillPoints[battler] += 2;
          }
          else if (gMoveResultFlags & ((1 << 2)))
          {
              skillPoints[battler] -= 1;
          }
          else if (!gProtectStructs[battler].protected)
          {
              skillPoints[battler] += 1;
          }
      }
}

/** void BattleArena_DeductSkillPoints(u8 battler, u16 stringId) */
export function BattleArena_DeductSkillPoints(battler: any, stringId: any): any {
  let skillPoints: any = gBattleStruct.arenaSkillPoints;

      switch (stringId)
      {
      case (327):
      case (346):
      case (347):
      case (350):
      case (309):
      case (311):
      case (305):
      case (306):
      case (195):
      case (196):
      case (197):
      case (199):
      case (200):
      case (201):
      case (202):
      case (203):
      case (204):
      case (206):
      case (119):
          skillPoints[battler] -= 3;
          break;
      }
}

/** static void InitArenaChallenge(void) */
export function InitArenaChallenge(): any {
  let isCurrent: any = null;
      let lvlMode: any = gSaveBlock2Ptr.frontier.lvlMode;

      gSaveBlock2Ptr.frontier.challengeStatus = 0;
      gSaveBlock2Ptr.frontier.curChallengeBattleNum = 0;
      gSaveBlock2Ptr.frontier.challengePaused = FALSE;
      gSaveBlock2Ptr.frontier.disableRecordBattle = FALSE;
      if (lvlMode != (0))
          isCurrent = gSaveBlock2Ptr.frontier.winStreakActiveFlags & ((1 << 7));
      else
          isCurrent = gSaveBlock2Ptr.frontier.winStreakActiveFlags & ((1 << 6));

      if (!isCurrent)
          gSaveBlock2Ptr.frontier.arenaWinStreaks[lvlMode] = 0;

      SetDynamicWarp(0, gSaveBlock1Ptr.location.mapGroup, gSaveBlock1Ptr.location.mapNum, ((-1)));
      gTrainerBattleOpponent_A = 0;
}

/** static void GetArenaData(void) */
export function GetArenaData(): any {
  let lvlMode: any = gSaveBlock2Ptr.frontier.lvlMode;

      switch (gSpecialVar_0x8005)
      {
      case (0):
          gSpecialVar_Result = gSaveBlock2Ptr.frontier.arenaPrize;
          break;
      case (1):
          gSpecialVar_Result = gSaveBlock2Ptr.frontier.arenaWinStreaks[lvlMode];
          break;
      case (2):
          if (lvlMode != (0))
              gSpecialVar_Result = gSaveBlock2Ptr.frontier.winStreakActiveFlags & ((1 << 7));
          else
              gSpecialVar_Result = gSaveBlock2Ptr.frontier.winStreakActiveFlags & ((1 << 6));
          break;
      }
}

/** static void SetArenaData(void) */
export function SetArenaData(): any {
  let lvlMode: any = gSaveBlock2Ptr.frontier.lvlMode;

      switch (gSpecialVar_0x8005)
      {
      case (0):
          gSaveBlock2Ptr.frontier.arenaPrize = gSpecialVar_0x8006;
          break;
      case (1):
          gSaveBlock2Ptr.frontier.arenaWinStreaks[lvlMode] = gSpecialVar_0x8006;
          break;
      case (2):
          if (lvlMode != (0))
          {
              if (gSpecialVar_0x8006)
                  gSaveBlock2Ptr.frontier.winStreakActiveFlags |= ((1 << 7));
              else
                  gSaveBlock2Ptr.frontier.winStreakActiveFlags &= ~(((1 << 7)));
          }
          else
          {
              if (gSpecialVar_0x8006)
                  gSaveBlock2Ptr.frontier.winStreakActiveFlags |= ((1 << 6));
              else
                  gSaveBlock2Ptr.frontier.winStreakActiveFlags &= ~(((1 << 6)));
          }
          break;
      }
}

/** static void SaveArenaChallenge(void) */
export function SaveArenaChallenge(): any {
  gSaveBlock2Ptr.frontier.challengeStatus = gSpecialVar_0x8005;
      VarSet(((((0x4000) + 0x0))), 0);
      gSaveBlock2Ptr.frontier.challengePaused = TRUE;
      SaveGameFrontier();
}

/** static void SetArenaPrize(void) */
export function SetArenaPrize(): any {
  let lvlMode: any = gSaveBlock2Ptr.frontier.lvlMode;

      if (gSaveBlock2Ptr.frontier.arenaWinStreaks[lvlMode] > 41)
          gSaveBlock2Ptr.frontier.arenaPrize = sLongStreakPrizeItems[Random() % ARRAY_COUNT(sLongStreakPrizeItems)];
      else
          gSaveBlock2Ptr.frontier.arenaPrize = sShortStreakPrizeItems[Random() % ARRAY_COUNT(sShortStreakPrizeItems)];
}

/** static void GiveArenaPrize(void) */
export function GiveArenaPrize(): any {
  if (AddBagItem(gSaveBlock2Ptr.frontier.arenaPrize, 1) == TRUE)
      {
          CopyItemName(gSaveBlock2Ptr.frontier.arenaPrize, gStringVar1);
          gSaveBlock2Ptr.frontier.arenaPrize = (0);
          gSpecialVar_Result = TRUE;
      }
      else
      {
          gSpecialVar_Result = FALSE;
      }
}

/** static void BufferArenaOpponentName(void) */
export function BufferArenaOpponentName(): any {
  GetFrontierTrainerName(gStringVar1, gTrainerBattleOpponent_A);
}

/** void DrawArenaRefereeTextBox(void) */
export function DrawArenaRefereeTextBox(): any {
  let width: any = 27;
      let palNum: any = 7;

      FillBgTilemapBufferRect(0, 0,    254, 14, 1,  6, palNum);
      FillBgTilemapBufferRect(0, 0,    32,  14, 1,  6, palNum);
      FillBgTilemapBufferRect(0, 0x31, 0,   14, 1,  1, palNum);
      FillBgTilemapBufferRect(0, 0x33, 1,   14, 1,  1, palNum);
      FillBgTilemapBufferRect(0, 0x34, 2,   14, width, 1, palNum);
      width++;
      FillBgTilemapBufferRect(0, 0x35, 28,  14, 1,  1, palNum);
      FillBgTilemapBufferRect(0, 0x36, 29,  14, 1,  1, palNum);
      FillBgTilemapBufferRect(0, 0x37, 0,   15, 1,  5, palNum);
      FillBgTilemapBufferRect(0, 0x39, 1,   15, width, 5, palNum);
      FillBgTilemapBufferRect(0, 0x3A, 29,  15, 1,  5, palNum);
      FillBgTilemapBufferRect(0, 0x831, 0,  19, 1,  1, palNum);
      FillBgTilemapBufferRect(0, 0x833, 1,  19, 1,  1, palNum);
      FillBgTilemapBufferRect(0, 0x834, 2,  19, width - 2, 1, palNum);
      FillBgTilemapBufferRect(0, 0x835, 28, 19, 1,  1, palNum);
      FillBgTilemapBufferRect(0, 0x836, 29, 19, 1,  1, palNum);
}

/** void EraseArenaRefereeTextBox(void) */
export function EraseArenaRefereeTextBox(): any {
  let width: any = null;
      let height: any = null;
      let palNum: any = 0;

      FillBgTilemapBufferRect(0, 3, 0, 14, 1, 1, palNum);
      height = 4;
      FillBgTilemapBufferRect(0, 4, 1, 14, 1, 1, palNum);
      width = 27;
      FillBgTilemapBufferRect(0, 5, 2, 14, width, 1, palNum);
      FillBgTilemapBufferRect(0, 6, 28, 14, 1, 1, palNum);
      FillBgTilemapBufferRect(0, 7, 29, 14, 1, 1, palNum);
      FillBgTilemapBufferRect(0, 8, 0, 15, 1, height, palNum);
      FillBgTilemapBufferRect(0, 9, 1, 15, 1, height, palNum);
      FillBgTilemapBufferRect(0, 0xA, 2, 15, width, height, palNum);
      FillBgTilemapBufferRect(0, 0xB, 28, 15, 1, height, palNum);
      FillBgTilemapBufferRect(0, 0xC, 29, 15, 1, height, palNum);
      FillBgTilemapBufferRect(0, 0xD, 0, 19, 1, 1, palNum);
      FillBgTilemapBufferRect(0, 0xE, 1, 19, 1, 1, palNum);
      FillBgTilemapBufferRect(0, 0xF, 2, 19, width, 1, palNum);
      FillBgTilemapBufferRect(0, 0x10, 28, 19, 1, 1, palNum);
      FillBgTilemapBufferRect(0, 0x11, 29, 19, 1, 1, palNum);
}

// ─── callsTo manifest (= 26 unique callees) ───────────────────────
// (Sorted alphabetically. Used for helper bridge detection.)
export const __callsTo__: ReadonlyArray<string> = ["ARRAY_COUNT","AddBagItem","BATTLE_OPPOSITE","BattlePutTextOnWindow","BattleStringExpandPlaceholdersToDisplayedString","BeginNormalPaletteFade","CopyBgTilemapBufferToVram","CopyItemName","CreateSprite","DestroySprite","FillBgTilemapBufferRect","FreeSpriteTilesByTag","GetFrontierTrainerName","HandleBattleWindow","IsDma3ManagerBusyWithBgCopy","LoadCompressedPalette","LoadCompressedSpriteSheet","OBJ_PLTT_ID","PlaySE","Random","SaveGameFrontier","SetDynamicWarp","SetGpuReg","ShowJudgmentSprite","StartSpriteAnim","VarSet"];
