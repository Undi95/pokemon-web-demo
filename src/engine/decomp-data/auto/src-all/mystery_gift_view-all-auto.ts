...
export function WonderCard_Init(...args: any[]): any {
...
export function WonderCard_Exit(...args: any[]): any {
  if (sWonderCardData == NULL)
          return -1;
      switch (sWonderCardData.enterExitState)
      {
      ...
      case CARD_TYPE_LINK_STAT:
          ...
          for (i = 0, charsUntilStat = 0; i < WONDER_CARD_TEXT_LENGTH; i++)
          {
              ...
              else
              {
                  ...
                  let id = sWonderCardData.card.footerLine2Text[i + 1];
...
export function DrawCardWindow(...args: any[]): any {
  let i = 0;
      let windowId = sWonderCardData.windowIds[whichWindow];
      ...
      case CARD_WIN_FOOTER:
          ...
          {
              let x = 0;
              let y = sCard_FooterTextOffsets[sWonderCardData.card.type] + 16;
              let spacing = GetFontAttribute(FONT_SHORT_COPY_1, FONTATTR_LETTER_SPACING);
...
export function WonderNews_Init(...args: any[]): any {
  if (news == NULL)
          return FALSE;
...
export function WonderNews_Exit(...args: any[]): any {
  if (sWonderNewsData == NULL)
          return -1;
      switch (sWonderNewsData.enterExitState)
      {
...
export function WonderNews_GetInput(...args: any[]): any {
...
export function UpdateNewsScroll(): any {
  let bgMove = sWonderNewsData.scrollIncrement;
      bgMove *= 256;
      if (sWonderNewsData.scrollingDown)
      {
