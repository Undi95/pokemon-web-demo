...
export function BufferApprenticeChallengeText(...args: any[]): any {
  let i, num;
...
export function ResetApprenticeStruct(...args: any[]): any {
  let i: any = null;
...
export function SetPlayersApprenticeLvlMode(...args: any[]): any {
...
export function ShuffleApprenticeSpecies(): any {
  const species: any[] = [];
      ...
      {
          let temp: any = null;
          let rand1 = Random() % ARRAY_COUNT(species);
          let rand2 = Random() % ARRAY_COUNT(species);
...
export function GetMonIdForQuestion(...args: any[]): any {
  let i, count;
      let monId = 0;
...
export function GetRandomAlternateMove(...args: any[]): any {
  let i, j;
      let id: any = null;
          ...
          else
          {
              ...
              else
              {
                  ...
                  do
                  {
                      ...
                      let learnsetId = Random() % (numLearnsetMoves - MAX_MON_MOVES);
...
export function TrySetMove(...args: any[]): any {
  let i: any = null;
...
export function GetLatestLearnedMoves(...args: any[]): any {
  let i, j;
...
export function GetDefaultMove(...args: any[]): any {
  const moves: any[] = [];
      let i, numQuestions;
...
export function SaveApprenticeParty(...args: any[]): any {
  struct ApprenticeMon *apprenticeMons[MULTI_PARTY_SIZE];
      ...
      {
          let questionId = PLAYER_APPRENTICE.questions[i].questionId;
          let monId = PLAYER_APPRENTICE.questions[i].monId;
          ...
          else if (questionId == QUESTION_ID_WHICH_MOVE)
          {
              if (PLAYER_APPRENTICE.questions[i].suggestedChange)
              {
                  let moveSlot = PLAYER_APPRENTICE.questions[i].moveSlot;
...
export function CreateApprenticeMenu(...args: any[]): any {
  let i: any = null;
      ...
      for (i = 0; i < count; i++)
      {
          let width = GetStringWidth(FONT_NORMAL, strings[i], 0);
          if (width > pixelWidth)
              pixelWidth = width;
      }
...
export function Task_ChooseAnswer(...args: any[]): any {
  let input: any = null;
...
export function CreateAndShowWindow(...args: any[]): any {
  let windowId: any = null;
...
export function RemoveAndHideWindow(...args: any[]): any {
  ClearStdWindowAndFrameToTransparent(windowId, TRUE);
      RemoveWindow(windowId);
}

/** static void CreateChooseAnswerTask(bool8 noBButton, u8 answers, u8 windowId) */
export function CreateChooseAnswerTask(...args: any[]): any {
  let taskId = CreateTask(Task_ChooseAnswer, 80);
...
export function IsFinalQuestion(): any {
  let questionNum = CURRENT_QUESTION_NUM;
...
export function Task_WaitForPrintingMessage(...args: any[]): any {
...
export function SetApprenticePartyMon(): any {
  if (gSpecialVar_0x8005)
      {
          let partySlot = gSpecialVar_0x8006;
...
export function SetSavedApprenticeTrainerGfxId(): any {
  let i: any = null;
      ...
      let class = gApprentices[gSaveBlock2Ptr.apprentices[0].id].facilityClass;
...
export function SetPlayerApprenticeTrainerGfxId(): any {
  let i: any = null;
      let objectEventGfxId: any = null;
      let class = gApprentices[PLAYER_APPRENTICE.id].facilityClass;
...
export function Task_ExecuteFuncAfterButtonPress(...args: any[]): any {
  if (JOY_NEW(A_BUTTON) || JOY_NEW(B_BUTTON))
      {
          gApprenticeFunc = (void *)(u32)((gTasks[taskId].data[0] | (gTasks[taskId].data[1] << 16)));
          gApprenticeFunc();
          DestroyTask(taskId);
      }
}
...
export function ExecuteFuncAfterButtonPress(...args: any[]): any {
  let taskId = CreateTask(Task_ExecuteFuncAfterButtonPress, 1);
