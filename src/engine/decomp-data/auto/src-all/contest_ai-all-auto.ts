...
export function ContestAI_ResetAI(...args: any[]): any {
  let i: any = null;
...
export function ContestAI_GetActionToUse(): any {
  while (eContestAI.aiFlags != 0)
      ...
      while (1)
      {
          ...
          let moveIndex = MOD(Random(), MAX_MON_MOVES);
          let score = eContestAI.moveScores[moveIndex];
          let i: any = null;
          for (i = 0; i < MAX_MON_MOVES; i++)
          {
...
export function GetContestantIdByTurn(...args: any[]): any {
  let i: any = null;
...
export function ContestAICmd_score(): any {
  let score = eContestAI.moveScores[eContestAI.nextMoveIndex] + gAIScriptPtr[1];
...
export function ContestAICmd_get_move_effect(): any {
  let move = gContestMons[eContestAI.contestantId].moves[eContestAI.nextMoveIndex];
...
export function ContestAICmd_check_most_appealing_move(): any {
  let i: any = null;
      let move = gContestMons[eContestAI.contestantId].moves[eContestAI.nextMoveIndex];
      let appeal = gContestEffects[gContestMoves[move].effect].appeal;

      for (i = 0; i < MAX_MON_MOVES; i++)
      {
          let newMove = gContestMons[eContestAI.contestantId].moves[i];
...
export function ContestAICmd_check_most_jamming_move(): any {
  let i: any = null;
      let move = gContestMons[eContestAI.contestantId].moves[eContestAI.nextMoveIndex];
      let jam = gContestEffects[gContestMoves[move].effect].jam;
...
export function ContestAICmd_get_condition(): any {
  let contestant = GetContestantIdByTurn(gAIScriptPtr[1]);
...
export function ContestAICmd_get_used_moves_effect(): any {
  let contestant = GetContestantIdByTurn(gAIScriptPtr[1]);
      let round = gAIScriptPtr[2];
      let move = eContest.moveHistory[round][contestant];
...
export function ContestAICmd_get_used_moves_excitement(): any {
  let contestant = GetContestantIdByTurn(gAIScriptPtr[1]);
      let round = gAIScriptPtr[2];
      let result = eContest.excitementHistory[round][contestant];
...
export function ContestAICmd_check_user_has_move(): any {
  let hasMove = FALSE;
      let i: any = null;
      let targetMove = T1_READ_16(gAIScriptPtr + 1);

      for (i = 0; i < MAX_MON_MOVES; i++)
      {
          let move = gContestMoves[gContestMons[eContestAI.contestantId].moves[i]].effect;
          let move = gContestMons[eContestAI.contestantId].moves[i];
