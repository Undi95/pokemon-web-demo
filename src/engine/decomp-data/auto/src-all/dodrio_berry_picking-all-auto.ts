...
export function StartDodrioBerryPicking(...args: any[]): any {
  sExitingGame = FALSE;
...
export function InitDodrioGame(...args: any[]): any {
  let i: any = null;
...
export function Task_StartDodrioGame(...args: any[]): any {
  let i, numPlayers;
...
export function Task_DodrioGame_Leader(...args: any[]): any {
  RecvLinkData_Leader();
...
export function Task_DodrioGame_Member(...args: any[]): any {
  RecvLinkData_Member();
...
export function AllLinkBlocksReceived(): any {
  let recvStatus = GetBlockReceivedStatus();
      let playerFlags = GetLinkPlayerCountAsBitFlags();
...
export function DoResults(): any {
  let playAgainState = PLAY_AGAIN_YES;
      let i: any = null;
      ...
      case 4:
          if (AllLinkBlocksReceived())
          {
              for (i = 0; i < sGame.numPlayers; i++)
              {
                  *(sGame.playAgainStates[i]) = *gBlockRecvBuffer[i];
                  sGame.playersReceived = sGame.numPlayers;
              }
          }
          if (sGame.playersReceived >= sGame.numPlayers)
...
export function Task_NewGameIntro(...args: any[]): any {
  switch (sGame.state)
      {
      case 0:
  ...
  let data = gTasks[taskId].data;
      ...
      case 2:
          if (AllLinkBlocksReceived())
          {
              for (i = 0; i < sGame.numPlayers; i++)
              {
                  *(u8 *)&sGame.monInfo[i] = *gBlockRecvBuffer[i];
                  sGame.playersReceived = sGame.numPlayers;
              }
          }
          if (sGame.playersReceived >= sGame.numPlayers)
...
export function RecvLinkData_Gameplay(): any {
  let i: any = null;
      let numPlayers = sGame.numPlayers;
...
export function HandleSound_Member(): any {
  let berryStart = sGame.berryColStart;
      let berryEnd = sGame.berryColEnd;
      let i: any = null;
      if (sGame.players[sGame.multiplayerId].comm.pickState == PICK_NONE)
      {
...
export function SetGameFunc(...args: any[]): any {
...
export function HandlePickBerries(): any {
  let berryStart = sGame.berryColStart;
      ...
      for (i = 0; i < numPlayers; i++)
      {
          let pickState = sGame.players[i].comm.pickState;
      ...
      for (j = berryStart; j < berryEnd; j++)
      {
          let playerIdMissed = PLAYER_NONE;
          column = sActiveColumnMap[0][0][j];
          if (sGame.berryState[column] == BERRYSTATE_PICKED)
          {
              ...
              if (++sGame.eatTimer[column] >= 6)
              {
                  ...
                  else
                  {
                      ...
                      let playerId1 = sGame.playersAttemptingPick[column][0];
...
export function TryPickBerry(...args: any[]): any {
  let pick = 0;
      let numPlayersIdx = sGame.numPlayers - 1;
      let berries: any = sGame.player.berries;
...
export function UpdateBerrySprites(): any {
  let i: any = null;
      ...
      for (i = berryStart; i < berryEnd; i++)
      {
          ...
          let column = sActiveColumnMap[sGame.numPlayers - 1][sGame.multiplayerId][i];
...
export function GetActiveBerryColumns(...args: any[]): any {
  switch (numPlayers)
      {
      case 1:
          *start = 4, *end = 7;
...
export function TryIncrementDifficulty(...args: any[]): any {
  let threshold = sDifficultyThresholds[sGame.difficulty[playerId] % NUM_DIFFICULTIES] + (sGame.difficulty[playerId] / NUM_DIFFICULTIES) * 100;
      if (sGame.berriesEaten[playerId] >= threshold)
          sGame.difficulty[playerId]++;
}

/** static u8 GetPlayerIdAtColumn(u8 column) */
export function GetPlayerIdAtColumn(...args: any[]): any {
  return sPlayerIdAtColumn[sGame.numPlayers - 1][column];
}

/** static u8 GetNewBerryId(u8 playerId, u8 column) */
export function GetNewBerryId(...args: any[]): any {
  let i, highestDifficulty;
      let numPlayersIdx = sGame.numPlayers - 1;
      let leftPlayer = sDodrioNeighborMap[numPlayersIdx][playerId][0];
      let middlePlayer = sDodrioNeighborMap[numPlayersIdx][playerId][1];
      let rightPlayer = sDodrioNeighborMap[numPlayersIdx][playerId][2];
...
export function GetNewBerryIdByDifficulty(...args: any[]): any {
  let prevBerryId = sGame.prevBerryIds[column];
...
export function IsTotalBerriesMissedOver10(...args: any[]): any {
  let missed = 0, i = 0;
...
export function IncrementBerryResult(...args: any[]): any {
...
export function UpdateBerriesPickedInRow(...args: any[]): any {
...
export function GetBerriesPicked(...args: any[]): any {
  let sum = sGame.berryResults[playerId][BERRY_BLUE]
...
export function TryUpdateRecords(): any {
  let berriesPicked = Min(GetBerriesPicked(sGame.multiplayerId), MAX_BERRIES);  
      let score = Min(GetScore(sGame.multiplayerId), MAX_SCORE);
...
export function UpdatePickStateQueue(...args: any[]): any {
  let i, nextState;
...
export function GetBerryResult(...args: any[]): any {
  return sGame.berryResults[playerId][berryId];
}

/** static u32 GetScore(u8 playerId) */
export function GetScore(...args: any[]): any {
  let i: any = null;
...
export function GetHighestBerryResult(...args: any[]): any {
  u8 i, numPlayers = sGame.numPlayers;
      let maxScore = sGame.berryResults[0][berryId];

      for (i = 0; i < numPlayers; i++)
      {
          let score = sGame.berryResults[i][berryId];
          if (score > maxScore)
              maxScore = score;
      }
      return maxScore;
...
export function GetScoreByRanking(...args: any[]): any {
...
export function SetScoreResults(): any {
  u8 i, ranking = 0, nextRanking = 0, playersRanked = 0;
      ...
      do
      {
          let score = GetScoreByRanking(ranking);
          let curRanking = nextRanking;
...
export function IncrementWithLimit(...args: any[]): any {
  if (num < max)
          return num + 1;
      else
          return max;
...
export function GetPlayerIdByPos(...args: any[]): any {
...
export function ShowDodrioBerryPickingRecords(): any {
  let taskId = CreateTask(Task_ShowDodrioBerryPickingRecords, 0);
      Task_ShowDodrioBerryPickingRecords(taskId);
}

/** static void Task_ShowDodrioBerryPickingRecords(u8 taskId) */
export function Task_ShowDodrioBerryPickingRecords(...args: any[]): any {
  let window: any = null;
...
export function SendPacket_ReadyToStart(...args: any[]): any {
  let packet: any = null;
...
export function RecvPacket_ReadyToStart(...args: any[]): any {
  let packet: any = null;
...
export function SendPacket_GameState(...args: any[]): any {
  let packet: any = null;
...
/** static bool32 RecvPacket_GameState(u32 playerId,
                                   ...
                                   bool32 *berriesFalling,
                                   bool32 *allReadyToEnd) */
export function RecvPacket_GameState(...args: any[]): any {
  let packet: any = null;
      let berries: any = player.berries;
...
export function SendPacket_PickState(...args: any[]): any {
  let packet: any = null;
...
export function RecvPacket_PickState(...args: any[]): any {
  let packet: any = null;
...
export function SendPacket_ReadyToEnd(...args: any[]): any {
  let packet: any = null;
...
export function RecvPacket_ReadyToEnd(...args: any[]): any {
  let packet: any = null;
...
export function CreateDodrioSprite(...args: any[]): any {
  let template: any = [
...
export function SpriteCB_Dodrio(...args: any[]): any {
...
export function StartDodrioMissedAnim(...args: any[]): any {
...
export function StartDodrioIntroAnim(...args: any[]): any {
  let sprite: any = gSprites[*sDodrioSpriteIds[GetMultiplayerId()]];
      sprite.sState = 2;
      sprite.sTimer = 0;
      sprite.sUnused1 = 0;
...
export function DoDodrioMissedAnim(...args: any[]): any {
  let x: any = null;
      let state = (++sprite.sTimer / 2) % 4;
...
export function DoDodrioIntroAnim(...args: any[]): any {
  let pickState = (++sprite.sTimer / FRAMES_PER_STATE) % NUM_INTRO_PICK_STATES;
...
export function FreeDodrioSprites(...args: any[]): any {
  let i: any = null;
      for (i = 0; i < numPlayers; i++)
      {
...
export function SetDodrioAnim(...args: any[]): any {
...
export function UpdateStatusBarAnim(...args: any[]): any {
  let i: any = null;
...
export function SetStatusBarInvisibility(...args: any[]): any {
  let i: any = null;
      for (i = 0; i < NUM_STATUS_SQUARES; i++)
...
export function SetBerryInvisibility(...args: any[]): any {
  gSprites[*sBerrySpriteIds[id]].invisible = invisible;
}

/** static void SetBerryIconsInvisibility(bool8 invisible) */
export function SetBerryIconsInvisibility(...args: any[]): any {
  let i: any = null;
      for (i = 0; i < NUM_BERRY_TYPES; i++)
...
export function SetCloudInvisibility(...args: any[]): any {
  let i: any = null;
      for (i = 0; i < NUM_CLOUDS; i++)
...
export function GetDodrioXPos(...args: any[]): any {
  let x = 0;
      switch (numPlayers)
      {
      case 1:
...
export function LoadWindowFrameGfx(...args: any[]): any {
...
export function DrawYesNoMessageWindow(...args: any[]): any {
  let pal = 10;
...
export function DrawMessageWindow(...args: any[]): any {
  let pal = 11;
...
export function InitGameGfx(...args: any[]): any {
  sGfx = ptr;
      sGfx.finished = FALSE;
      sGfx.state = 0;
      sGfx.loadState = 0;
...
export function SetGfxFuncById(...args: any[]): any {
  let i: any = null;
...
export function Task_TryRunGfxFunc(...args: any[]): any {
  if (!sGfx.finished)
          GetGfxFunc()();
}
...
export function PrintRankedScores(...args: any[]): any {
  u8 i, ranking = 0, rankedPlayers = 0;
      let numPlayers = numPlayers_;  
      let name: any = null;
      let x, numWidth;
      const numString: any[] = [];
      ...
      for (i = 0; i < numPlayers; i++)
      {
          let colorsId = COLORID_GRAY;
          let playerId = playersByRanking[i];
          let points = scoreResults[playerId].score;
  ...
  u8 i, j, prizeState, numPlayers = GetNumPlayers();
      ...
      case 2:
          ...
          for (i = 0; i < numPlayers; i++)
          {
              ...
              {
                  let width: any = null;
                  let berriesPicked = Min(GetBerryResult(i, j), MAX_BERRIES);
                  let maxBerriesPicked = Min(GetHighestBerryResult(j), MAX_BERRIES);
...
export function SetGfxFunc(...args: any[]): any {
  sGfx.state = 0;
      sGfx.finished = FALSE;
      sGfx.func = func;
}
