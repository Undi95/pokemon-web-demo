...
export function RotatingGate_ResetAllGateOrientations(): any {
  let i: any = null;
      let ptr = GetVarPointer(VAR_TEMP_0);
...
export function RotatingGate_LoadPuzzleConfig(): any {
  let puzzleType = GetCurrentMapRotatingGatePuzzleType();
      let i: any = null;
...
export function RotatingGate_CreateGatesWithinViewport(...args: any[]): any {
  let i: any = null;

       
       
      let x = gSaveBlock1Ptr.pos.x - 2;
      let x2 = gSaveBlock1Ptr.pos.x + MAP_OFFSET_W + 2;
      let y = gSaveBlock1Ptr.pos.y - 2;
      let y2 = gSaveBlock1Ptr.pos.y + MAP_OFFSET_H;

      for (i = 0; i < sRotatingGate_PuzzleCount; i++)
      {
          let x3 = sRotatingGate_PuzzleConfig[i].x + MAP_OFFSET;
          let y3 = sRotatingGate_PuzzleConfig[i].y + MAP_OFFSET;
...
export function RotatingGate_CreateGate(...args: any[]): any {
  let sprite: any = null;
...
export function SpriteCallback_RotatingGate(...args: any[]): any {
  let affineAnimation: any = null;
      let rotationDirection = sprite.data[1];
      let orientation = sprite.data[2];
...
export function RotatingGate_HideGatesOutsideViewport(...args: any[]): any {
  let x, y;
      let x2, y2;
...
export function RotatingGate_DestroyGatesOutsideViewport(): any {
  let i: any = null;
      ...
      for (i = 0; i < sRotatingGate_PuzzleCount; i++)
      {
          let xGate = sRotatingGate_PuzzleConfig[i].x + MAP_OFFSET;
          let yGate = sRotatingGate_PuzzleConfig[i].y + MAP_OFFSET;
...
export function RotatingGate_CanRotate(...args: any[]): any {
  let armPos: any = null;
      ...
      for (i = GATE_ARM_NORTH; i <= GATE_ARM_WEST; i++)
      {
          ...
          {
              let armIndex = 2 * ((orientation + i) % 4) + j;
...
export function RotatingGate_HasArm(...args: any[]): any {
  let arm = armInfo / 2;
      let isLongArm = armInfo % 2;

      let armOrientation = (arm - RotatingGate_GetGateOrientation(gateId) + 4) % 4;
      let shape = sRotatingGate_PuzzleConfig[gateId].shape;
...
export function RotatingGate_TriggerRotationAnimation(...args: any[]): any {
...
export function RotatingGate_GetRotationInfo(...args: any[]): any {
  let ptr: any = null;
...
export function RotatingGatePuzzleCameraUpdate(...args: any[]): any {
  if (GetCurrentMapRotatingGatePuzzleType())
      {
...
export function CheckForRotatingGatePuzzleCollision(...args: any[]): any {
  let i: any = null;
      ...
      for (i = 0; i < sRotatingGate_PuzzleCount; i++)
      {
          let gateX = sRotatingGate_PuzzleConfig[i].x + MAP_OFFSET;
          let gateY = sRotatingGate_PuzzleConfig[i].y + MAP_OFFSET;

          if (gateX - 2 <= x && x <= gateX + 1 && gateY - 2 <= y && y <= gateY + 1)
          {
              let centerX = x - gateX + 2;
              let centerY = y - gateY + 2;
              let rotationInfo = RotatingGate_GetRotationInfo(direction, centerX, centerY);

              if (rotationInfo != GATE_ROT_NONE)
              {
                  let rotationDirection = ((rotationInfo & 0xF0) >> 4);
                  let armInfo = rotationInfo & 0xF;
...
export function CheckForRotatingGatePuzzleCollisionWithoutAnimation(...args: any[]): any {
  let i: any = null;
