          ...
          for (j = 0; j < FRONTIER_PARTY_SIZE; j++)
          {
              do
              {
                  ...
                  for (k = 0; k < j; k++)
                  {
                      ...
                      let alreadySelectedMonId = DOME_MONS[i][k];
...
export function CalcDomeMonStats(...args: any[]): any {
  let i, count;
      let bits: any = null;
      ...
      {
          let n = 2 * gSpeciesInfo[species].baseHP;
...
export function SwapDomeTrainers(...args: any[]): any {
  let i: any = null;
      let temp: any = null;
...
export function CreateDomeOpponentMon(...args: any[]): any {
  let i: any = null;
      let friendship = MAX_FRIENDSHIP;
      let fixedIv = GetDomeTrainerMonIvs(DOME_TRAINERS[tournamentTrainerId].trainerId);
      let fixedIv = GetDomeTrainerMonIvs(tournamentTrainerId);  
...
export function CreateDomeOpponentMons(...args: any[]): any {
  let monsCount = 0;
      let otId = 0;
...
export function GetDomeTrainerSelectedMons(...args: any[]): any {
...
export function SelectOpponentMons_Good(...args: any[]): any {
...
export function SelectOpponentMons_Bad(...args: any[]): any {
  let i, moveIndex, playerMonId;
      const partyMovePoints: any[] = [];
...
export function SelectOpponentMonsFromParty(...args: any[]): any {
  let i, j;
      let selectedMonBits = 0;
       ...
       && partyMovePoints[0] == partyMovePoints[2])
      {
          if (allowRandom)
          {
              ...
              {
                  let rand = Random() & FRONTIER_PARTY_SIZE;
...
export function GetTypeEffectivenessPoints(...args: any[]): any {
  let defType1, defType2, defAbility, moveType;
      let i = 0;
      let typePower = TYPE_x1;
...
export function GetDomeTrainerMonIvs(...args: any[]): any {
...
export function TournamentIdOfOpponent(...args: any[]): any {
  let i, j, opponentMax;
...
export function ShowDomeOpponentInfo(): any {
  let taskId = CreateTask(Task_ShowTourneyInfoCard, 0);
...
export function Task_ShowTourneyInfoCard(...args: any[]): any {
  let i: any = null;
      let tournamentId = gTasks[taskId].tTournamentId;
      let mode = gTasks[taskId].tMode;
      let id = gTasks[taskId].tPrevTaskId;
...
export function SpriteCB_TrainerIconCardScrollUp(...args: any[]): any {
  sprite.y += 4;
      if (sprite.data[0] != 0)
      {
          if (sprite.y >= -32)
...
export function SpriteCB_TrainerIconCardScrollDown(...args: any[]): any {
  sprite.y -= 4;
      if (sprite.data[0] != 0)
      {
          if (sprite.y <= 192)
...
export function SpriteCB_TrainerIconCardScrollLeft(...args: any[]): any {
  sprite.x += 4;
      if (sprite.data[0] != 0)
      {
          if (sprite.x >= -32)
...
export function SpriteCB_TrainerIconCardScrollRight(...args: any[]): any {
  sprite.x -= 4;
      if (sprite.data[0] != 0)
      {
          if (sprite.x <= DISPLAY_WIDTH + 32)
...
export function SpriteCB_MonIconDomeInfo(...args: any[]): any {
  if (!sprite.sMonIconStill)
          UpdateMonIconFrame(sprite);
}

/** static void SpriteCB_MonIconCardScrollUp(struct Sprite *sprite) */
export function SpriteCB_MonIconCardScrollUp(...args: any[]): any {
  if (!sprite.sMonIconStill)
          UpdateMonIconFrame(sprite);
      sprite.y += 4;
      if (sprite.data[0] != 0)
...
export function SpriteCB_MonIconCardScrollDown(...args: any[]): any {
  if (!sprite.sMonIconStill)
          UpdateMonIconFrame(sprite);
      sprite.y -= 4;
      if (sprite.data[0] != 0)
...
export function SpriteCB_MonIconCardScrollLeft(...args: any[]): any {
  if (!sprite.sMonIconStill)
          UpdateMonIconFrame(sprite);
      sprite.x += 4;
      if (sprite.data[0] != 0)
...
export function SpriteCB_MonIconCardScrollRight(...args: any[]): any {
  if (!sprite.sMonIconStill)
          UpdateMonIconFrame(sprite);
      sprite.x -= 4;
      if (sprite.data[0] != 0)
...
export function SpriteCB_HorizontalScrollArrow(...args: any[]): any {
  let taskId1 = sprite.data[0];
      let arrId = gTasks[gTasks[taskId1].data[4]].data[1];
...
export function SpriteCB_VerticalScrollArrow(...args: any[]): any {
  let taskId1 = sprite.data[0];
...
export function Task_HandleInfoCardInput(...args: any[]): any {
  let i: any = null;
      let windowId = 0;
      let mode = gTasks[taskId].data[3];
      let taskId2 = gTasks[taskId].data[4];
...
export function Task_GetInfoCardInput(...args: any[]): any {
  let input = INFOCARD_INPUT_NONE;
      let taskId2 = gTasks[taskId].data[4];
      let position = gTasks[taskId2].data[1];
      let tourneyId = sTourneyTreeTrainerIds[position];
      let roundId = gSaveBlock2Ptr.frontier.curChallengeBattleNum;
...
export function DisplayTrainerInfoOnCard(...args: any[]): any {
  let trClassName: any = null;
      ...
      let nature = 0;
      let arrId = 0;
      let windowId = WIN_TRAINER_NAME;
      let x = 0, y = 0;
      let palSlot = 0;
      let allocatedArray = AllocZeroed(sizeof(s16) * ALLOC_ARRAY_SIZE);
      ...
      else
      {
          for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
          {
              let evBits = gFacilityTrainerMons[DOME_MONS[trainerTourneyId][i]].evSpread;
...
export function BufferDomeWinString(...args: any[]): any {
  let i: any = null;
...
export function DisplayMatchInfoOnCard(...args: any[]): any {
  let textPrinter: any = null;
...
export function ShowDomeTourneyTree(): any {
  let taskId = CreateTask(Task_ShowTourneyTree, 0);
      gTasks[taskId].tState = 0;
...
export function Task_HandleTourneyTreeInput(...args: any[]): any {
  let newTaskId = 0;
      let spriteId = gTasks[taskId].data[1];
...
export function UpdateTourneyTreeCursor(...args: any[]): any {
  let selection = TOURNEY_TREE_NO_SELECTION;
      let direction = MOVE_DIR_NONE;
      let tourneyTreeCursorSpriteId = gTasks[taskId].data[1];
      let roundId = gSaveBlock2Ptr.frontier.curChallengeBattleNum;
...
export function GetWinningMove(...args: any[]): any {
  let i, j, k;
      ...
      for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
      {
          for (j = 0; j < MAX_MON_MOVES; j++)
          {
              ...
              {
                  let var = 0;
                  let targetSpecies = SPECIES_NONE;
                  let targetAbility = ABILITY_NONE;
                  do
                  {
                      var = Random32();
...
export function Task_ShowTourneyTree(...args: any[]): any {
  let i: any = null;
      let textPrinter: any = null;
      let notInteractive = gTasks[taskId].tNotInteractive;
      let r4 = gTasks[taskId].data[2];
...
export function DrawTourneyAdvancementLine(...args: any[]): any {
  let i: any = null;
...
export function Task_HandleStaticTourneyTreeInput(...args: any[]): any {
  let i: any = null;
      let textPrinter: any = null;
...
export function ResetSketchedMoves(): any {
  let i, moveSlot;
      ...
      for (i = 0; i < DOME_BATTLE_PARTY_SIZE; i++)
      {
          let playerMonId = gSaveBlock2Ptr.frontier.selectedPartyMons[gSelectedOrderFromParty[i] - 1] - 1;
          let count: any = null;
...
export function RestoreDomePlayerPartyHeldItems(): any {
  let i: any = null;
      ...
      {
          let playerMonId = gSaveBlock2Ptr.frontier.selectedPartyMons[gSelectedOrderFromParty[i] - 1] - 1;
          let item = GetMonData(gSaveBlock1Ptr.playerParty[playerMonId], MON_DATA_HELD_ITEM, NULL);
...
export function TrainerIdToTournamentId(...args: any[]): any {
  let i: any = null;
...
export function TrainerIdToDomeTournamentId(...args: any[]): any {
  let i: any = null;
...
export function GetOpposingNPCTournamentIdByRound(...args: any[]): any {
  const tournamentIds: any[] = [];
...
export function DecideRoundWinners(...args: any[]): any {
  let i: any = null;
...
export function CopyDomeTrainerName(...args: any[]): any {
  let i = 0;
      SetFacilityPtrsGetLevel();
...
export function CopyDomeBrainTrainerName(...args: any[]): any {
  let i: any = null;
