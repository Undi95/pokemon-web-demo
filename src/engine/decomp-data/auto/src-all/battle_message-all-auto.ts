...
export function BufferStringBattle(...args: any[]): any {
  let i: any = null;
      let stringPtr = NULL;
      let size: any = null;
...
export function BattleStringExpandPlaceholdersToDisplayedString(...args: any[]): any {
  return BattleStringExpandPlaceholders(src, gDisplayedStringBattle);
}

/** u32 BattleStringExpandPlaceholders(const u8 *src, u8 *dst) */
export function BattleStringExpandPlaceholders(...args: any[]): any {
  let dstID = 0;  
      let toCpy = NULL;
      ...
      while (*src != EOS)
      {
          if (*src == PLACEHOLDER_BEGIN)
          {
              ...
              {
                  let trClass = GetFrontierOpponentClass(gPartnerTrainerId);
                  let gender = IsFrontierTrainerFemale(gPartnerTrainerId);
...
export function ExpandBattleTextBuffPlaceholders(...args: any[]): any {
  let srcID = 1;
      let value = 0;
      const text: any[] = [];
      let hword: any = null;
...
export function ChooseMoveUsedParticle(...args: any[]): any {
  let counter = 0;
      let i = 0;
...
export function ChooseTypeOfMoveUsedString(...args: any[]): any {
  let counter = 0;
      let i = 0;
...
export function BattlePutTextOnWindow(...args: any[]): any {
  let textInfo: any = sBattleTextOnWindowsInfo[gBattleScripting.windowsType];
      ...
      {
          let width = GetBattleWindowTemplatePixelWidth(gBattleScripting.windowsType, windowId);
          let alignX = GetStringCenterAlignXOffsetWithLetterSpacing(printerTemplate.fontId, printerTemplate.currentChar, width, printerTemplate.letterSpacing);
...
export function SetPpNumbersPaletteInMoveSelection(): any {
  let chooseMoveStruct: any = (struct ChooseMoveStruct *)(gBattleBufferA[gActiveBattler][4]);
      let palPtr = gPPTextPalette;
      let var = GetCurrentPpToMaxPpState(chooseMoveStruct.currentPp[gMoveSelectionCursor[gActiveBattler]],
...
export function GetCurrentPpToMaxPpState(...args: any[]): any {
  if (maxPp == currentPp)
      {
          return 3;
      }
