...
// Source : src/list_menu.c (via SPRITE_CALLBACKS / TASKS / SPRITE_CBS / CB2S)
...
/** Source: list_menu.c → Task_ScrollIndicatorArrowPair */
export const Task_ScrollIndicatorArrowPair: TaskCallback = (task, rt) => {
  const taskId = task.taskId;
  let data: any = task.data;
      let currItem = (data.scrollOffset);

      if (currItem == data.fullyUpThreshold && currItem != 0xFFFF)
          _gs(rt, data.topSpriteId).invisible = true;
      else
          _gs(rt, data.topSpriteId).invisible = false;

      if (currItem == data.fullyDownThreshold)
          _gs(rt, data.bottomSpriteId).invisible = true;
      else
          _gs(rt, data.bottomSpriteId).invisible = false;
};

/** Source: list_menu.c → Task_ScrollIndicatorArrowPairOnMainMenu */
export const Task_ScrollIndicatorArrowPairOnMainMenu: TaskCallback = (task, rt) => {
  const taskId = task.taskId;
  const data = task.data;
      ...
      if (data[15])
      {
          _gs(rt, scrollData.topSpriteId).invisible = false;
          _gs(rt, scrollData.bottomSpriteId).invisible = true;
      }
      else
      {
          _gs(rt, scrollData.topSpriteId).invisible = true;
          _gs(rt, scrollData.bottomSpriteId).invisible = false;
      }
};

/** Source: list_menu.c (failed: empty bodyC) */
export const Task_RedOutlineCursor: TaskCallback = (_task, _rt) => { /* TODO empty bodyC */ };

/** Source: list_menu.c (failed: empty bodyC) */
export const Task_RedArrowCursor: TaskCallback = (_task, _rt) => { /* TODO empty bodyC */ };
