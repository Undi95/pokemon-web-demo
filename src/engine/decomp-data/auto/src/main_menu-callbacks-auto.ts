      ...
      if (!rt.gPaletteFade.active)
      {
          ...
          switch (action)
          {
              ...
              case ACTION_CONTINUE:
                  rt.gPlttBufferUnfaded.set(0, RGB_BLACK);
                  rt.gPlttBufferFaded.set(0, RGB_BLACK);
                  /* TODO scene transition: SetMainCallback2(CB2_ContinueSavedGame) */;
                  rt.DestroyTask(taskId);
                  break;
              case ACTION_OPTION:
                  gMain.savedCallback = CB2_ReinitMainMenu;
                  /* TODO scene transition: SetMainCallback2(CB2_InitOptionMenu) */;
                  rt.DestroyTask(taskId);
                  break;
  ...
  if (!rt.gPaletteFade.active)
      {
          ...
          sCurrItemAndOptionMenuCheck = 0;
          FreeAllWindowBuffers();
          /* TODO scene transition: SetMainCallback2(CB2_InitTitleScreen) */;
          rt.DestroyTask(taskId);
      }
};
...
export const Task_NewGameBirchSpeechSub_InitPokeBall: TaskCallback = (task, rt) => {
  const taskId = task.taskId;
  let spriteId = _gt(rt, sBirchSpeechMainTaskId).data[9];
...
export const Task_NewGameBirchSpeechSub_WaitForLotad: TaskCallback = (task, rt) => {
  const taskId = task.taskId;
  const data = task.data;
      let sprite: any = _gs(rt, _gt(rt, sBirchSpeechMainTaskId).data[9]);
      ...
      switch (data[0])
      {
          case 0:
              ...
              break;
          case 1:
              if (_gt(rt, sBirchSpeechMainTaskId).data[7] >= 96)
              {
                  rt.DestroyTask(taskId);
                  if (_gt(rt, sBirchSpeechMainTaskId).data[7] < 0x4000)
                      _gt(rt, sBirchSpeechMainTaskId).data[7]++;
              }
              return;
      }
      data[0]++;
  ...
  if (!rt.gPaletteFade.active)
      {
          ...
          FreeAndDestroyMonPicSprite(task.data[9]);
          ResetAllPicSprites();
          /* TODO scene transition: SetMainCallback2(CB2_NewGame) */;
          rt.DestroyTask(taskId);
      }
};
...
export const CB2_NewGameBirchSpeech_ReturnFromNamingScreen: CB2Callback = (rt) => {
  let taskId = 0;
      ...
      REG_IME = savedIme;
      /* noop SetVBlankCallback */;
      /* TODO scene transition: SetMainCallback2(CB2_MainMenu) */;
      InitWindows(sNewGameBirchSpeechTextWindows);
      LoadMainMenuWindowFrameTiles(0, 0xF3);
      LoadMessageBoxGfx(0, BIRCH_DLG_BASE_TILE_NUM, BG_PLTT_ID(15));
      PutWindowTilemap(0);
