...
// Source : src/event_object_movement.c (via SPRITE_CALLBACKS / TASKS / SPRITE_CBS / CB2S)
...
/** Source: event_object_movement.c → SpriteCB_CameraObject */
export const SpriteCB_CameraObject: SpriteCallback = (sprite, rt) => {
  void (callbacks[((sCameraObjectFuncs)?.length ?? 0)]);

      memcpy(callbacks, sCameraObjectFuncs, ((sCameraObjectFuncs)?.length ?? 32));
      callbacks[sprite.sCamera_State](sprite);
};

/** Source: event_object_movement.c → SpriteCB_VirtualObject */
export const SpriteCB_VirtualObject: SpriteCallback = (sprite, rt) => {
