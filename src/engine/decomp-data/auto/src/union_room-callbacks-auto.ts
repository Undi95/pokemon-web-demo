...
export const Task_TryBecomeLinkLeader: TaskCallback = (task, rt) => {
  ...
  let id, val;
      ...
      case LL_STATE_INIT:
          ...
          break;
      case LL_STATE_INIT2:
          data.incomingPlayerList = AllocZeroed(RFU_CHILD_MAX * /* ((struct)?.length ?? 32) */ 0);
          data.playerList = AllocZeroed(MAX_RFU_PLAYERS * /* ((struct)?.length ?? 32) */ 0);
          data.playerListBackup = AllocZeroed(MAX_RFU_PLAYERS * /* ((struct)?.length ?? 32) */ 0);
...
export const Task_TryJoinLinkGroup: TaskCallback = (task, rt) => {
  ...
  let id = 0;
      ...
      case LG_STATE_INIT:
          ...
          data.incomingPlayerList = AllocZeroed(RFU_CHILD_MAX * /* ((struct)?.length ?? 32) */ 0);
          data.playerList = AllocZeroed(MAX_RFU_PLAYER_LIST_SIZE * /* ((struct)?.length ?? 32) */ 0);
...
export const Task_StartUnionRoomTrade: TaskCallback = (task, rt) => {
  ...
  let monId = GetPartyPositionOfRegisteredMon(sUnionRoomTrade, GetMultiplayerId());
      ...
      case 0:
          task.data[0]++;
          SendBlock(0, gPlayerParty[monId], /* ((struct)?.length ?? 32) */ 0);
          break;
      case 1:
          ...
          break;
      case 2:
          memcpy(gBlockSendBuffer, gSaveBlock1Ptr.mail, /* ((struct)?.length ?? 32) */ 0 * PARTY_SIZE + 4);
          if (SendBlock(0, gBlockSendBuffer, /* ((struct)?.length ?? 32) */ 0 * PARTY_SIZE + 4))
              task.data[0]++;
          break;
      case 3:
          if (GetBlockReceivedStatus() == 3)
          {
              memcpy(gTradeMail, gBlockRecvBuffer[GetMultiplayerId() ^ 1], /* ((struct)?.length ?? 32) */ 0 * PARTY_SIZE);
              ResetBlockReceivedFlags();
...
export const Task_RunUnionRoom: TaskCallback = (task, rt) => {
  ...
  let id = 0;
      ...
      {
      case UR_STATE_INIT:
          uroom.incomingChildList = AllocZeroed(RFU_CHILD_MAX * /* ((struct)?.length ?? 32) */ 0);
          uroom.incomingParentList = AllocZeroed(RFU_CHILD_MAX * /* ((struct)?.length ?? 32) */ 0);
          uroom.playerList = AllocZeroed(MAX_UNION_ROOM_LEADERS * /* ((struct)?.length ?? 32) */ 0);
          uroom.spawnPlayer = AllocZeroed(/* ((struct)?.length ?? 32) */ 0);
      ...
      case UR_STATE_CANCEL_REQUEST_PRINT_MSG:
          if (!gReceivedRemoteLinkPlayers)
          {
              ...
              memset(uroom.playerSendBuffer, 0, /* sizeof(uroom.playerSendBuffer) */ 0);
              uroom.recvActivityRequest[0] = 0;
              uroom.partnerYesNoResponse = 0;
          }
          break;
...
export const Task_InitUnionRoom: TaskCallback = (task, rt) => {
  ...
  let i = 0;
      ...
      case 1:
          ...
          break;
      case 2:
          data.incomingChildList = AllocZeroed(RFU_CHILD_MAX * /* ((struct)?.length ?? 32) */ 0);
          ClearIncomingPlayerList(data.incomingChildList, RFU_CHILD_MAX);
          data.incomingParentList = AllocZeroed(RFU_CHILD_MAX * /* ((struct)?.length ?? 32) */ 0);
          ClearIncomingPlayerList(data.incomingParentList, RFU_CHILD_MAX);
          data.playerList = AllocZeroed(MAX_UNION_ROOM_LEADERS * /* ((struct)?.length ?? 32) */ 0);
          ClearRfuPlayerList(data.playerList.players, MAX_UNION_ROOM_LEADERS);
          data.spawnPlayer = AllocZeroed(/* ((struct)?.length ?? 32) */ 0);
