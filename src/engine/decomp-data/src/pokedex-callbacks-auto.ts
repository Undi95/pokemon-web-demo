...
import { CopyBgTilemapBufferToVram, CopyWindowToVram, DeactivateAllTextPrinters, EnableInterrupts, FillWindowPixelBuffer, FreeAllSpritePalettes, FreeAllWindowBuffers, FreeAndDestroyMonPicSprite, HideBg, InitBgsFromTemplates, InitWindows, JOY_NEW, LoadCompressedSpriteSheet, LoadPalette, LoadSpritePalettes, OAM, OAM_SIZE, PutWindowTilemap, ResetAllPicSprites, ResetBgsAndClearDma3BusyFlags, ResetPaletteFade, ResetTasks, ScanlineEffect_Stop, ShowBg, UpdatePaletteFade, VRAM, VRAM_SIZE, gMPlayInfo_BGM, gMain, gPlttBufferUnfaded, gSaveBlock2Ptr, m4aSongNumStart } from '../../system/decomp-globals';
...
const R_BUTTON = 256;
const SE_BALL = 23;
const SE_DEX_PAGE = 109;
const SE_DEX_SCROLL = 108;
const SE_DEX_SEARCH = 112;
const SE_FAILURE = 32;
const SE_PC_LOGIN = 2;
const SE_PC_OFF = 3;
const SE_PIN = 21;
...
export const SpriteCB_PokedexListMonSprite: SpriteCallback = (sprite, rt) => {
  let monId = sprite.data[1];
      ...
      {
          let _var = 0;sprite.y2 = Math.trunc(gSineTable(sprite.data[5]) * 76 / 256);
...
export const SpriteCB_RotatingPokeBall: SpriteCallback = (sprite, rt) => {
  if (sPokedexView.currentPage != PAGE_MAIN && sPokedexView.currentPage != PAGE_SEARCH_RESULTS)
      ...
      else
      {
          ...
          r3 = gSineTable(val);
          r0 = gSineTable(val + 64);sprite.x2 = Math.trunc(r0 * 40 / 256);
...
export const SpriteCB_DexListStartMenuCursor: SpriteCallback = (sprite, rt) => {
  if (sPokedexView.currentPage != PAGE_MAIN && sPokedexView.currentPage != PAGE_SEARCH_RESULTS)
      ...
      else
      {
          ...
          {
              sprite.invisible = false;
              sprite.y2 = sPokedexView.menuCursorPos * 16;sprite.x2 = Math.trunc(gSineTable(sprite.data[2]) / 64);
              sprite.data[2] += 8;
          }
          else
          {
  ...
  if (_gt(rt, sprite.data[0]).func == Task_HandleSearchParameterInput)
      {
          ...
          val = sprite.data[2] + sprite.data[1] * 128;sprite.y2 = Math.trunc(gSineTable(val) / 128);
          sprite.data[2] += 8;
      }
      else
      {
  ...
  if (!rt.gPaletteFade.active)
      {
          ...
          paletteNum = rt.gba.oam[_gs(rt, task.data[3]).oamIndex].paletteBank;
