...
export const Task_ShowWinnerMonBanner: TaskCallback = (task, rt) => {
  ...
  let i = 0;
      ...
      case 1:
          if (++task.data[3] == 1)
          {
              ...
              counter = task.data[2];gBattle_WIN0V = Math.trunc(WIN_RANGE(DISPLAY_HEIGHT / 2) - counter, DISPLAY_HEIGHT / 2 + counter);
              if (counter == 32)
                  task.data[10]++;
          }
          break;
  ...
  top = Math.trunc(DISPLAY_HEIGHT / 2);
...
export const CB2_StartShowContestResults: CB2Callback = (rt) => {
  rt.gPaletteFade.bufferTransferDisabled = true;
      ...
      memset(sContestResults.data, 0, 0);
      memset(sContestResults.monResults, 0, 0);
