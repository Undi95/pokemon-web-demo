...
function _destroyTypeSprites(): void {
  const rt = getRuntime();
  for (const sid of _typeSpriteIds) { try { DestroySprite(sid); } catch { /* déjà */ } }
  _typeSpriteIds = [];
}
...
function _createSetStatusSprite(): void {
  ...
  if (ailment === 0) {                       // SetSpriteInvisibility(TRUE)
    if (_statusSpriteId >= 0) { try { DestroySprite(_statusSpriteId); } catch { /* */ } _statusSpriteId = -1; }
    return;
  }
  const spr = rt.CreateSpriteAtOam({
...
function _changeSummaryPokemon(delta: number): void {
  ...
  if (_monPicSpriteId >= 0) {
    ...
    try { DestroySprite(_monPicSpriteId); } catch { /* */ } _monPicSpriteId = -1;
  }
  if (_statusSpriteId >= 0) { try { DestroySprite(_statusSpriteId); } catch { /* */ } _statusSpriteId = -1; }
  if (_markingsSpriteId >= 0) { try { DestroySprite(_markingsSpriteId); } catch { /* */ } _markingsSpriteId = -1; }
  if (_ballSpriteId >= 0) { try { DestroySprite(_ballSpriteId); } catch { /* */ } _ballSpriteId = -1; }
...
function _destroyMoveSelectorSprites(firstArrayId: number): void {
  ...
  for (const id of ids) { try { DestroySprite(id); } catch { /* déjà */ } }
...
function _freeSummary(): void {
  ...
  const rt = getRuntime();
  _destroyTypeSprites();
  if (_monPicSpriteId >= 0) { try { DestroySprite(_monPicSpriteId); } catch { /* */ } _monPicSpriteId = -1; }
  if (_statusSpriteId >= 0) { try { DestroySprite(_statusSpriteId); } catch { /* */ } _statusSpriteId = -1; }
  if (_markingsSpriteId >= 0) { try { DestroySprite(_markingsSpriteId); } catch { /* */ } _markingsSpriteId = -1; }
  if (_ballSpriteId >= 0) { try { DestroySprite(_ballSpriteId); } catch { /* */ } _ballSpriteId = -1; }
