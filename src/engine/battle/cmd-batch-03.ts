...
function uproarWakeUpCheck(battler: number): boolean {
  ...
  for (let i = 0; i < battlersCount; i++) {
    if ((battleMons[i].status2 & STATUS2_UPROAR)
        && battleMons[battler].ability !== ABILITY_SOUNDPROOF) {
      return true;
    }
  }
  return false;
...
export function installBatch03Handlers(commandsTable: BattleOpcodeHandler[]): void {
  ...
  console.log('[battle/cmd-batch-03] installed 8/8 Batch 03 handlers (branching)');
}
