...
// import { runNewGameInit } from '../engine/new-game-init'; // TODO: restore when available
...
export class NamingScene extends Phaser.Scene {
  ...
  private confirm() {
    ...
    // Spawn par défaut : InsideOfTruck (comme décomp)
    gameState.setDynamicWarp('MAP_INSIDE_OF_TRUCK', 4, 1);
    const dw = gameState.dynamicWarp!;
    gameState.map = { name: 'InsideOfTruck', x: dw.x, y: dw.y };
    gameState.save();
    this.scene.start('OverworldScene', { mapName: 'InsideOfTruck', spawnX: dw.x, spawnY: dw.y });
  }
}
