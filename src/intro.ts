// src/intro.ts — miroir 1:1 de `src/intro.c` (pokeemerald) : intro Game Freak +
// anim légendaires (Groudon/Kyogre/Rayquaza) + scène joueur à vélo (Flygon/Manectric/
// Torchic) + bubbles/sparkles/water-drops + CB2_InitCopyrightScreen* / scene tasks.
//
// Dissous depuis l'ex-`decomp-data/src/intro-callbacks-auto.ts` (@ts-nocheck,
// auto-transpilé) → relocalisé à son foyer 1:1 + TYPÉ (relocate+retype, self-contained).
// Constantes GBA bare (ex-globalThis) importées (bloc io_reg/decomp-runtime/palette).
/* eslint-disable */
...
import { gSpriteSheet_IntroBrendan, gSpriteSheet_IntroMay, gSpriteSheet_IntroBicycle, gSpriteSheet_IntroFlygon, gSpritePalettes_IntroPlayerFlygon } from './intro_credits_graphics';
...
// Constantes GBA (ex-bare globalThis sous @ts-nocheck) — foyers io_reg / decomp-runtime / palette / defines.
import {
  BGCNT_16COLOR, BGCNT_256COLOR, BGCNT_AFF256x256, BGCNT_AFF512x512,
  BGCNT_TXT256x256, BGCNT_TXT256x512, BGCNT_TXT512x256,
  DISPCNT_BG0_ON, DISPCNT_BG1_ON, DISPCNT_BG2_ON, DISPCNT_BG_ALL_ON,
  DISPCNT_MODE_0, DISPCNT_MODE_1, DISPCNT_OBJ_1D_MAP, DISPCNT_OBJ_ON, DISPCNT_WIN0_ON,
  REG_OFFSET_BG0CNT, REG_OFFSET_BG0HOFS, REG_OFFSET_BG0VOFS,
  REG_OFFSET_BG1CNT, REG_OFFSET_BG1HOFS, REG_OFFSET_BG1VOFS,
  REG_OFFSET_BG2CNT, REG_OFFSET_BG2HOFS, REG_OFFSET_BG2VOFS,
  REG_OFFSET_BG3CNT, REG_OFFSET_BG3VOFS,
...
import {
  BGCNT_CHARBASE, BGCNT_PRIORITY, BGCNT_SCREENBASE, BG_CHAR_ADDR, BG_SCREEN_ADDR,
  BLDCNT_EFFECT_BLEND, BLDCNT_TGT1_BG0, BLDCNT_TGT1_BG1, BLDCNT_TGT1_BG2,
  BLDCNT_TGT2_BD, BLDCNT_TGT2_BG0, BLDCNT_TGT2_BG1, BLDCNT_TGT2_BG2, BLDCNT_TGT2_BG3,
  BLDCNT_TGT2_OBJ, DISPLAY_HEIGHT, DISPLAY_WIDTH,
...
// (heap / save-blocks) ne vivent que dans ces CB2 morts → stubs ambient (corps gardé
// 1:1 intro.c mais jamais exécuté ; évite aussi le cycle intro↔copyright-boot).
declare function InitHeap(heap: unknown, size: number): void;
declare function SetSaveBlocksPointers(offset: number): void;
declare function GetSaveBlocksPointersBaseOffset(): number;
declare function Sav2_ClearSetDefault(): void;
declare function ResetMenuAndMonGlobals(): void;
declare function SetUpCopyrightScreen(): number;
declare const gHeap: unknown;
...
// MANUAL FIX session 71 : remove local stub — real PlayCryInternal already
// imported from decomp-globals (line ~58). The transpileur emits a stub for
// unresolved identifiers; here it shadowed the real function.
// (PlayCryInternal is in the imports block above.)
...
export const SpriteCB_WaterDrop_Ripple: SpriteCallback = (sprite, rt) => {
  let palNum = 0;
      ...
      if (sprite.data[2] >= 192)
      {
          ...
          {
              sprite.invisible = false;
              SetOamMatrix(rt.gba, sprite.data[1], sprite.data[2], 0, 0, sprite.data[2]);sprite.data[2] = Math.trunc((sprite.data[2] * 95) / 100);palNum = Math.trunc((sprite.data[2] - 192) / 128) + 9;
              if (palNum > 15)
                  palNum = 15;
...
export const SpriteCB_WaterDrop_Slide: SpriteCallback = (sprite, rt) => {
  if (sprite.x <= 116)
      ...
      else
      {
          ...
          sprite.data[4] += 2;sprite.y2 = Math.trunc(sin1 / 32);
          sprite.x--;
          if (sprite.x & 1)
              sprite.y++;temp = Math.trunc(-sin2 / 16);
          data2 = sprite.data[2];
          data3 = sprite.data[3];
          sin3 = gSineTable((temp - 16));
          sin4 = gSineTable((temp + 48));var1 = Math.trunc(sin4 * data2 / 256);
  var2 = Math.trunc(-sin3 * data3 / 256);
  var3 = Math.trunc(sin3 * data2 / 256);
  var4 = Math.trunc(sin4 * data3 / 256);
...
export const SpriteCB_WaterDrop_ReachLeafEnd: SpriteCallback = (sprite, rt) => {
  SetOamMatrix(rt.gba, sprite.data[1], sprite.data[6] + 64, 0, 0, sprite.data[6] + 64);
      ...
      if (sprite.data[4] != MAX_SPRITES)
      {
          let sinIdx = 0;
          sprite.data[4] -= 8;
          sinIdx = sprite.data[4];sprite.x2 = Math.trunc(gSineTable((sinIdx + 64)) / 64);
  sprite.y2 = Math.trunc(gSineTable(sinIdx) / 64);
      }
      else
      {
          sprite.data[4] = 0;
  ...
  if (sprite.data[0] != 2)
      {
          ...
          sprite.data[4] += 8;r2 = Math.trunc(gSineTable(sprite.data[4]) / 16) + 64;sprite.x2 = Math.trunc(gSineTable((r2 + 64)) / 64);
  sprite.y2 = Math.trunc(gSineTable(r2) / 64);
      }
      else
      {
...
export const SpriteCB_Flygon: SpriteCallback = (sprite, rt) => {
  switch (sprite.data[0])
      ...
      sprite.y2 = Sin(sprite.data[1], 8) - (globalThis as any).sFlygonYOffset;
      sprite.data[1] += 4;
};
  ...
  switch (sprite.data[0])
      ...
      case 2:
          ...
          if (sprite.data[3] == 0)
          {
              ...
              if (sprite.data[7] != 0)
              {
                  rt.CpuCopy16((rt.getExtraPalette("gIntroGameFreakTextFade_Pal") ?? new Uint16Array(0)), sprite.data[7], (256 + (1) * 16) + 15, Math.floor((PLTT_SIZEOF(1) / 2)));
                  rt.CpuCopy16((rt.getExtraPalette("gIntroGameFreakTextFade_Pal") ?? new Uint16Array(0)), sprite.data[7] + 16, (256 + (1) * 16) + 4, Math.floor((PLTT_SIZEOF(1) / 2)));
                  rt.CpuCopy16((rt.getExtraPalette("gIntroGameFreakTextFade_Pal") ?? new Uint16Array(0)), sprite.data[7] + 32, (256 + (1) * 16) + 10, Math.floor((PLTT_SIZEOF(1) / 2)));
                  sprite.data[7]--;
              }
              else
              {
  ...
  let foo = 0;
      ...
      case 1:
          ...
          else
          {
              ...
              if (sprite.data[1] < 64)
                  sprite.data[1]++;
          }foo = Math.trunc(256 - gSineTable(sprite.data[1]) / 2);
...
export const Task_Scene1_Load: TaskCallback = (task, rt) => {
  ...
  rt.SetVBlankCallback(VBlankCB);
      ...
      LoadSpritePalettes(sSpritePalettes_Intro1 as unknown as { data: string; tag: string | number }[]);
      LoadCompressedSpriteSheet(sSpriteSheet_Sparkle);
      LoadSpritePalettes(sSpritePalette_Sparkle as unknown as { data: string; tag: string | number }[]);
      ...
      task.data[0] = CreateWaterDrop(236, -14, 0x200, 1, 0x78, 0);
...
export const Task_CreateSparkles: TaskCallback = (task, rt) => {
  ...
  const data = task.data;
      ...
      case 0:
          ...
          _CreateSpriteAtTemplate(rt, sSpriteTemplate_Sparkle as unknown as SpriteTemplate,  sSparkleCoords[_sparkleIdx][0], sSparkleCoords[_sparkleIdx][1] + data[3], 0);
          data[0]++;
          data[6] = 12;
          data[4]++;
          break;
  ...
  if (rt.gIntroFrameCounter < TIMER_END_PAN_UP)
      {
          ...
          offset = (task.data[1] << 16) + (task.data[2] & 0xFFFF);
          offset -= 0x6000;
          ...
          if (rt.gIntroFrameCounter == TIMER_FLYGON_SILHOUETTE_APPEAR)
          {
              ...
              let spriteId = _CreateSpriteAtTemplate(rt, sSpriteTemplate_FlygonSilhouette as unknown as SpriteTemplate,  120, DISPLAY_HEIGHT, 10);
...
export const Task_Scene2_Load: TaskCallback = (task, rt) => {
  ...
  rt.IntroResetGpuRegs();
      ...
      (globalThis as any).sFlygonYOffset = 0;
...
export const Task_Scene2_CreateSprites: TaskCallback = (task, rt) => {
  ...
  let spriteId = 0;
      ...
      LoadSpritePalettes(gSpritePalettes_IntroPlayerFlygon as unknown as { data: string; tag: string | number }[]);
      LoadSpritePalettes(sSpritePalettes_RunningPokemon as unknown as { data: string; tag: string | number }[]);

       
      _CreateSpriteAtTemplate(rt, sSpriteTemplate_Manectric as unknown as SpriteTemplate,  DISPLAY_WIDTH + 32, 128, 0);
      _CreateSpriteAtTemplate(rt, sSpriteTemplate_Torchic as unknown as SpriteTemplate,  DISPLAY_WIDTH + 48, 110, 1);
      ...
      _CreateSpriteAtTemplate(rt, sSpriteTemplate_Volbeat as unknown as SpriteTemplate,  DISPLAY_WIDTH + 32, 80, 4);
...
export const Task_Scene2_BikeRide: TaskCallback = (task, rt) => {
  ...
  let offset = 0;
      ...
      if (rt.gIntroFrameCounter == TIMER_TORCHIC_EXIT)
      {
          ...
          (globalThis as any).gIntroCredits_MovingSceneryState = INTROCRED_SCENERY_FROZEN;
...
export const Task_Scene3_Groudon: TaskCallback = (task, rt) => {
  ...
  const data = task.data;
      ...
      data[5]++;
      if ((data[0] >= 1 && data[0] <= 7) && data[5] % 2 == 0)
          data[4] ^= 3;
...
export const Task_Scene3_Clouds: TaskCallback = (task, rt) => {
  ...
  const data = task.data;
      ...
      case 1:
          ...
          if (data[6] == 40 << 8)
              rt.BeginNormalPaletteFade(0xFFFE, 3, 0, 16, 10569);
...
export const Task_Scene3_LoadLightning: TaskCallback = (task, rt) => {
  ...
  LZDecompressVram(gIntroRayquaza_Tilemap, (BG_SCREEN_ADDR(28)));
      ...
      LoadSpritePalettes(sSpritePalette_Lightning as unknown as { data: string; tag: string | number }[]);
};
  ...
  const data = task.data;
      ...
      case 0:
          if (--data[6] == 0)
          {
              ...
              _CreateSpriteAtTemplate(rt, sSpriteTemplate_Lightning as unknown as SpriteTemplate,  200, 48, 0);
              spriteId = _CreateSpriteAtTemplate(rt, sSpriteTemplate_Lightning as unknown as SpriteTemplate,  200, 80, 1);
              rt.StartSpriteAnim(spriteId, 1);
              spriteId = _CreateSpriteAtTemplate(rt, sSpriteTemplate_Lightning as unknown as SpriteTemplate,  200, 112, 2);
      ...
      case 1:
          if (--data[6] == 0)
          {
              ...
              _CreateSpriteAtTemplate(rt, sSpriteTemplate_Lightning as unknown as SpriteTemplate,  40, 48, 0);
              spriteId = _CreateSpriteAtTemplate(rt, sSpriteTemplate_Lightning as unknown as SpriteTemplate,  40, 80, 1);
              rt.StartSpriteAnim(spriteId, 1);
              spriteId = _CreateSpriteAtTemplate(rt, sSpriteTemplate_Lightning as unknown as SpriteTemplate,  40, 112, 2);
              rt.StartSpriteAnim(spriteId, 2);
              data[0]++;
              data[6] = 48;
          }
...
export const Task_Scene3_LoadRayquazaAttack: TaskCallback = (task, rt) => {
  ...
  let attackTaskId = 0;
      ...
      LoadSpritePalettes(sSpritePalette_RayquazaOrb as unknown as { data: string; tag: string | number }[]);
      rt.SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_MODE_0
                                  | DISPCNT_OBJ_1D_MAP
                                  | DISPCNT_BG0_ON
                                  | DISPCNT_BG2_ON
      ...
      rt.BeginNormalPaletteFade(0xFFDE, 0, 16, 0, 10569);
      task.data[0] = 0;
      task.data[1] = 0xA8;
      task.data[2] = -0x10;
      task.data[3] = -0x88;
...
export const Task_EndIntroMovie: TaskCallback = (task, rt) => {
  const taskId = task.taskId;
  rt.DestroyTask(taskId);
  rt.SetMainCallback2(MainCB2_EndIntro);
};
  ...
  let spriteId = 0;
      ...
      case 2:
          if (data[3] == 0)
          {
              ...
              if (data[1] == 6)
              {
                  spriteId = _CreateSpriteAtTemplate(rt, sSpriteTemplate_RayquazaOrb as unknown as SpriteTemplate,  120, 88, 15);
  ...
  switch (task.data[0])
      ...
      case 1:
          if (task.data[1] != 0)
          {
              ...
              task.data[1]--;tmp = Math.trunc(task.data[1] / 2);
              rt.SetGpuReg(REG_OFFSET_BLDALPHA, gTitleScreenAlphaBlend[tmp]);
          }
          else
          {
              rt.SetGpuReg(REG_OFFSET_BLDALPHA, gTitleScreenAlphaBlend[0]);task.data[1] = Math.trunc(((gTitleScreenAlphaBlend)?.length ?? 0) / 4);
              task.data[0]++;
          }
          break;
      case 2:
...
export const Task_BlendLogoOut: TaskCallback = (task, rt) => {
  ...
  switch (task.data[0])
      ...
      case 1:
          ...
          {
              rt.SetGpuReg(REG_OFFSET_BLDALPHA, gTitleScreenAlphaBlend[31]);task.data[1] = Math.trunc(((gTitleScreenAlphaBlend)?.length ?? 0) / 4);
              task.data[0]++;
          }
          break;
      case 2:
  ...
  if (!SetUpCopyrightScreen())
      {
          ...
          (LoadGameSave as unknown as (m: number) => void)(SAVE_NORMAL);
          if ((globalThis as any).gSaveFileStatus == SAVE_STATUS_EMPTY || (globalThis as any).gSaveFileStatus == SAVE_STATUS_CORRUPT)
...
export function CreateKyogreBubbleSprites_Body(taskId: number): number {
  ...
  let i = 0;
      ...
      {
          spriteId = _CreateSpriteAtTemplate(rt, sSpriteTemplate_Bubbles as unknown as SpriteTemplate,
...
export function CreateGameFreakLogoSprites(x: number, y: number, unused: number): number {
  ...
  let i = 0;
      ...
      {
          spriteId = _CreateSpriteAtTemplate(rt, sSpriteTemplate_GameFreakLetter as unknown as SpriteTemplate,  sGameFreakLetterData[i][1] + x, y - 4, 0);
          _gs(rt, spriteId).data[0] = 0;
      ...
      spriteId = _CreateSpriteAtTemplate(rt, sSpriteTemplate_GameFreakLogo as unknown as SpriteTemplate,  120, y - 6, 0);
      _gs(rt, spriteId).data[0] = 0;
      _gs(rt, spriteId).invisible = true;
      _gs(rt, spriteId).matrixNum = i + 12;
