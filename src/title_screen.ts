// src/title_screen.ts — miroir 1:1 de `src/title_screen.c` (pokeemerald).
//
// Dissous depuis l'ex-`decomp-data/src/title_screen-callbacks-auto.ts` (@ts-nocheck,
// auto-transpilé) → relocalisé à son foyer 1:1 + TYPÉ (relocate + retype ; fichier
// self-contained : CB2_InitTitleScreen + Task_TitleScreenPhase* + SpriteCB_* +
// helpers CreatePressStartBanner/CreateCopyrightBanner). Les constantes GBA bare
// (résolues via globalThis sous @ts-nocheck) sont maintenant importées (bloc
// io_reg/decomp-runtime/palette ci-dessous).
/* eslint-disable */

import type { DecompRuntime, DecompSprite, DecompTask } from '../harness/runtime/decomp-runtime';
import { ResetSpriteData, DestroySprite, _CreateSpriteAtTemplate, ANIMCMD_FRAME, ANIMCMD_END, type SpriteTemplate } from './sprite';
import {
...
// Constantes GBA (ex-bare globalThis sous @ts-nocheck) — foyers io_reg.h / decomp-runtime / palette.
import {
  BGCNT_16COLOR, BGCNT_256COLOR, BGCNT_AFF256x256, BGCNT_TXT256x256,
  DISPCNT_BG0_ON, DISPCNT_BG1_ON, DISPCNT_BG2_ON, DISPCNT_MODE_1,
...
import {
  ...
  BLDCNT_EFFECT_BLEND, BLDCNT_TGT1_BG1, BLDCNT_TGT1_BG2, BLDCNT_TGT1_OBJ,
...
export const Task_TitleScreenPhase1: TaskCallback = (task, rt) => {
  ...
  if (JOY_NEW(A_B_START_SELECT) || task.data[1])
      ...
      else
      {
          ...
          spriteId = _CreateSpriteAtTemplate(rt, sVersionBannerLeftSpriteTemplate as unknown as SpriteTemplate, VERSION_BANNER_LEFT_X, VERSION_BANNER_Y, 0);
          _gs(rt, spriteId).data[0] = ((gTitleScreenAlphaBlend)?.length ?? 0);
          _gs(rt, spriteId).data[1] = taskId;

           
          spriteId = _CreateSpriteAtTemplate(rt, sVersionBannerRightSpriteTemplate as unknown as SpriteTemplate, VERSION_BANNER_RIGHT_X, VERSION_BANNER_Y, 0);
          _gs(rt, spriteId).data[1] = taskId;
...
export const CB2_GoToClearSaveDataScreen: CB2Callback = (rt) => {
  if (!UpdatePaletteFade()) {
          // 1:1 décomp : SetMainCallback2(CB2_InitClearSaveDataScreen). Écran debug
          // (combo bouton clear-save rare) NON porté → no-op stopgap.
  }
};
...
export const CB2_GoToResetRtcScreen: CB2Callback = (rt) => {
  if (!UpdatePaletteFade()) {
          // 1:1 décomp : SetMainCallback2(CB2_InitResetRtcScreen). Écran debug
          // (combo bouton reset-RTC rare) NON porté → no-op stopgap.
  }
};
...
export function CreatePressStartBanner(rt: DecompRuntime, x: number, y: number): number {
  ...
  let i = 0;
      ...
      {
          spriteId = _CreateSpriteAtTemplate(rt, sStartCopyrightBannerSpriteTemplate as unknown as SpriteTemplate, x, y, 0);
          rt.StartSpriteAnim(spriteId, i);
          _gs(rt, spriteId).data[0] = 1;
      }
  return -1;
}
