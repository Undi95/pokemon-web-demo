...
export function MetatileBehavior_IsReflective(mb: number): boolean {
  return mb === MB_POND_WATER
      || mb === MB_PUDDLE
      || mb === MBC.MB_UNUSED_SOOTOPOLIS_DEEP_WATER_2  // FIX 1:1 décomp : était manquant
      || mb === MB_ICE
...
export function MetatileBehavior_IsShallowFlowingWater(mb: number): boolean {
  return mb === MB_SHALLOW_WATER
      ...
      || mb === MBC.MB_SHOAL_CAVE_ENTRANCE;  // FIX 1:1 décomp : MB_NO_SURFACING était FAUX, SHOAL_CAVE manquait
}
...
export function MetatileBehavior_IsUnableToEmerge(mb: number): boolean {
  // 1:1 VANILLA : le décomp ajoute `|| mb == MB_WATER_DOOR` UNIQUEMENT sous #ifdef
  // BUGFIX (glitch émeraude documenté : on peut émerger sur les portes d'eau). On
  // reproduit la ROM d'origine (BUGFIX OFF) → on n'inclut PAS MB_WATER_DOOR.
...
export function MetatileBehavior_IsEastBlocked(mb: number): boolean {
  return mb === MB_IMPASSABLE_EAST
      ...
      || mb === MBC.MB_SECRET_BASE_BREAKABLE_DOOR;  // FIX 1:1 décomp : était manquant
}
...
export function MetatileBehavior_IsBridgeOverWater(mb: number): boolean {
  // FIX 1:1 décomp : miroir avait MED_EDGE_1/2 (PAS dans le décomp) + manquait
  // MB_UNUSED_BRIDGE + MB_BIKE_BRIDGE_OVER_BARRIER.
  return mb === MB_BRIDGE_OVER_OCEAN
      || mb === MB_BRIDGE_OVER_POND_LOW
      ...
      || mb === MBC.MB_UNUSED_BRIDGE
      || mb === MBC.MB_BIKE_BRIDGE_OVER_BARRIER;
}
export function MetatileBehavior_IsBridgeOverWaterNoEdge(mb: number): boolean {
  // FIX 1:1 décomp : RANGE `>= MB_BRIDGE_OVER_OCEAN && <= MB_BRIDGE_OVER_POND_HIGH`
  // (miroir faisait 3 égalités → manquait MB_BRIDGE_OVER_OCEAN).
  return mb >= MB_BRIDGE_OVER_OCEAN && mb <= MB_BRIDGE_OVER_POND_HIGH;
}
