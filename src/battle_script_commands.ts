// ─── Imports des sections helper fusionnées (ex type-calc/stat-stages/set-move-effect/ai-damage) ───
import {
  gEffectBattler,
  setPaydayMoney,
...
import { PressurePPLose as PressurePPLoseAtkCanceler } from './battle_util';
...
// ═══════════════════════════════════════════════════════════════════════
// SECTIONS HELPER (type-calc/stat-stages/set-move-effect/ai-damage) — fusionnées
// depuis l'ex-engine type-calc/etc. (battle_script_commands.c). Cmd_typecalc réel
// renommé TypecalcImpl (appelé par le wrapper Cmd_typecalc de l'interpréteur).
// ═══════════════════════════════════════════════════════════════════════
...
// isBattlerOfType : défini plus haut (section interpréteur, ex-engine 2811) —
// le doublon de la section type-calc fusionnée a été retiré (fusion 2026-06-13).
...
/** 1:1 décomp `TypecalcImpl` (battle_script_commands.c:1355-1424).
 *
 *  Logic :
...
export function TypecalcImpl(): boolean {
  ...
  // 1:1 décomp pokemon.c:TypecalcImpl :
  // `if (gMoveResultFlags & MOVE_RESULT_DOESNT_AFFECT_FOE)
  //    gProtectStructs[gBattlerAttacker].targetNotAffected = 1;`
  if (gMoveResultFlags & MOVE_RESULT_DOESNT_AFFECT_FOE) {
    gProtectStructs[gBattlerAttacker].targetNotAffected = 1;
