// src/intro_credits_graphics.ts — miroir 1:1 de `src/intro_credits_graphics.c`
// (pokeemerald) : sprites/scenery partagés intro + credits de fin (Brendan/May/
// Flygon/bicycle, moving scenery clouds/trees/houses, BG bicycle anim).
//
// Dissous depuis l'ex-`decomp-data/src/intro_credits_graphics-callbacks-auto.ts`
// (@ts-nocheck) → relocalisé à son foyer 1:1 + TYPÉ (relocate+retype).
// NB : les Create* (CreateIntroBrendanSprite/CreateBicycleBgAnimationTask/...) ont
// aussi une version hand-written dans decomp-globals.ts que l'intro live utilise
// (= ces Create* ici 1:1 mais doublon à réconcilier plus tard) ; les SpriteCB_*
// credits sont consommés par intro-host.
/* eslint-disable */
...
// Data tables scenery CREDITS non portées (clouds/trees/houses) — utilisées seulement
// par les Create*Sprites credits (scène credits de fin non atteignable en dev) -> stubs
// ambient. À porter dans `intro_credits_graphics-data` quand la scène credits sera faite.
declare const sAnims_Clouds: any;
declare const sAnims_Trees: any;
declare const sAnims_HouseSilhouette: any;
declare const sSpriteMetadata_Clouds: any;
declare const sSpriteMetadata_Trees: any;
declare const sSpriteMetadata_HouseSilhouette: any;
...
export function CreateBicycleBgAnimationTask(rt: DecompRuntime, mode: number, bg1Speed: number, bg2Speed: number, bg3Speed: number): number {
  ...
  let taskId = rt.CreateTask((t) => Task_BicycleBgAnimation(t, rt), 0);
      ...
      task.data[8] = 8;
      task.data[9] = 0;
      Task_BicycleBgAnimation(_gt(rt, taskId), rt);
      return taskId;
  return -1;
}
...
export function CreateMovingScenerySprites(rt: DecompRuntime, hasVerticalMove: number, metadata: any, anims: any, numSprites: number): number {
  ...
  let i = 0;
      ...
      for(i = 0; i < numSprites; i++)
      {
          ...
          rt.StartSpriteAnim(sprite, metadata[i].animNum);
...
export function CreateCloudSprites(rt: DecompRuntime): number {
  let sprite: any = _emptySprite;
  let task: any = _emptyTask;
  CreateMovingScenerySprites(rt, 0, sSpriteMetadata_Clouds, sAnims_Clouds, 9);
  return -1;
}
...
export function CreateTreeSprites(rt: DecompRuntime): number {
  let sprite: any = _emptySprite;
  let task: any = _emptyTask;
  CreateMovingScenerySprites(rt, 1, sSpriteMetadata_Trees, sAnims_Trees, 12);
  return -1;
}
...
export function CreateHouseSprites(rt: DecompRuntime): number {
  let sprite: any = _emptySprite;
  let task: any = _emptyTask;
  CreateMovingScenerySprites(rt, 1, sSpriteMetadata_HouseSilhouette, sAnims_HouseSilhouette, 6);
  return -1;
}
