...
import { GetSpriteTileStartByTag, FreeSpritePaletteByTag, DestroySprite, FreeOamMatrix, IndexOfSpritePaletteTag } from './sprite';

import { BALL_POKE, BALL_DIVE, BALL_LUXURY, BALL_PREMIER, POKEBALL_PLAYER_SENDOUT, POKEBALL_OPPONENT_SENDOUT } from '../include/pokeball';
import { ItemIdToBallId, AnimateBallOpenParticles, AnimateBallOpenParticlesForPokeball, LaunchBallFadeMonTask, SetUpForReleaseAffineAnim } from './battle_anim_throw';
...
// ════════════════════════════════════════════════════════════════════════════
// RELEASE BALL HORS-COMBAT (Birch/OW) — relocalisé depuis decomp-globals (lot1e-2, fix cycle).
// 1:1 décomp pokeball.c:1031 CreatePokeballSpriteToReleaseMon + :1088 SpriteCB_ReleasedMonFlyOut.
// Était dans le god-module → cycle decomp-globals↔battle_anim_throw ; remis à sa maison pokeball.c.
// ════════════════════════════════════════════════════════════════════════════
...
export function CreatePokeballSpriteToReleaseMon(
  ...
  const tileBaseRaw = GetSpriteTileStartByTag(POKEBALL_TAG);
  const palSlotRaw = IndexOfSpritePaletteTag(POKEBALL_PAL_TAG);
  ...
  ballSprite.callback = (sprite, runtime) => {
    ...
    AnimateBallOpenParticlesForPokeball(runtime, ball.x, ball.y - 5, oamPriority, 0, BALL_POKE);
    if (m) {
