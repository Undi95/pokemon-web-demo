/**
 * pokemon_animation.ts — miroir 1:1 de `src/pokemon_animation.c` (décomp, 5544 l)
 * + `PokemonSummaryDoMonAnimation` (pokemon.c:6826) + delay task (pokemon.c:6761)
 * + entrées consommateur front-sprite (pokemon.c:6779 DoMonFrontSpriteAnimation).
 *
 * Port FIDÈLE de TOUTES les fonctions d'animation (≈240 : 151 Anim_* + helpers
 * partagés). Arithmétique ENTIÈRE C respectée (Math.trunc pour `/`, `%` JS = `%`
 * C tronqué, `>>` s32). `sprite->data[]` = DecompSprite.data (Int16Array s16).
 *
 * CONSOLIDATION (2026-06-29) : ce fichier ABSORBE l'ancien src/pokemon_animation.ts
 * (version combat partielle, registre string) + l'ancien src/engine/ui/
 * mon-summary-anim.ts (copie fidèle résumé). Les DEUX substrats coexistent 1:1
 * comme le décomp : `sIsSummaryAnim` (module-level) branche les helpers affine —
 *   • TRUE  (résumé, StartMonSummaryAnimation) → substrat OAM réel (AllocOamMatrix
 *     + SetOamMatrix, pattern pokéball SAC) ;
 *   • FALSE (combat/Birch, LaunchAnimationTaskForFrontSprite) → substrat
 *     battle_anim_mons (Prepare/Set/Reset SpriteRotScale, matrice OAM combat).
 * Chaque branche = comportement VÉRIFIÉ avant fusion (zéro changement de rendu).
 * Le dispatch combat passe par `sMonAnimFunctions` (array, 151 anims — l'ancienne
 * « dette » du registre string est résorbée) ; `globalThis.__pokemonAnimation`
 * exposé en bas (anti-cycle battle_main).
 *
 * Limite HONNÊTE documentée : `HasTwoFramesAnimation`/StartSpriteAnim(.,1)
 * (2e frame du front-pic) — front.png 1-frame → no-op. L'anim AFFINE joue 1:1.
 */
...
function SetAffineData(s: S, xScale: number, yScale: number, rotation: number): void {
  if (!sIsSummaryAnim) { SetSpriteRotScale(s.spriteId, xScale, yScale, rotation); return; } // substrat combat
...
function HandleStartAffineAnim(s: S): void {
  if (!sIsSummaryAnim) { PrepareBattlerSpriteForRotScale(s.spriteId, 0); return; } // substrat combat
  const rt = getRuntime(); if (!rt) return;
...
function HandleSetAffineData(s: S, xScale: number, yScale: number, rotation: number): void {
  if (!sIsSummaryAnim) { SetSpriteRotScale(s.spriteId, xScale, yScale, rotation); return; } // substrat combat (sDontFlip=TRUE → pas de négation)
...
function ResetSpriteAfterAnim(s: S): void {
  ...
  if (sIsSummaryAnim) {
    ...
    // === COMBAT/Birch (sIsSummaryAnim=FALSE) : substrat battle_anim_mons — reset
    // de la matrice OAM combat (= comportement VÉRIFIÉ de l'ancien
    // src/pokemon_animation.ts : ResetSpriteAfterAnim → ResetSpriteRotScale). ===
    ResetSpriteRotScale(s.spriteId);
  }
}
...
/** 1:1 décomp `HasTwoFramesAnimation` (pokemon.c:6959) : TRUE sauf Castform/
 *  Deoxys/Spinda/Unown. Accepte l'enum string (résumé) OU l'id numérique
 *  (combat/Birch via decomp-globals) — mêmes 4 exclusions. */
export function HasTwoFramesAnimation(species: number | string): boolean {
  if (typeof species === 'number') {
    return species !== 385 /* SPECIES_CASTFORM */ && species !== 410 /* SPECIES_DEOXYS */
        && species !== 308 /* SPECIES_SPINDA */ && species !== 201 /* SPECIES_UNOWN */;
  }
 ...
 *
 * ACTIFS (consolidation 2026-06-29) : le combat (battle_main via
 * `globalThis.__pokemonAnimation`, posé en bas de CE fichier) et Birch/évo/
 * Pokédex (decomp-globals → CE fichier) passent désormais ICI. Ces wrappers
 * posent `sIsSummaryAnim = FALSE` → les helpers affine prennent la branche
 * substrat COMBAT (battle_anim_mons) et WaitAnimEnd/ResetSpriteAfterAnim la
 * branche combat. StartMonSummaryAnimation le remet TRUE (substrat OAM résumé).
 * ════════════════════════════════════════════════════════════════════════════ */
...
export function LaunchAnimationTaskForFrontSprite(spriteId: number, frontAnim: number | string): void {
  const rt = getRuntime();
  const sprite = rt?.gSprites?.[spriteId] as S | undefined;
  if (!sprite) return;
  // battle_main (_BattleAnimateFrontSprite) passe le NOM ANIM_* (string) ; le
  // décomp/DoMonFront passe l'id numérique. Résolution unifiée.
  const frontAnimId = typeof frontAnim === 'number' ? frontAnim : (ANIM[frontAnim] ?? ANIM.ANIM_V_SQUISH_AND_BOUNCE);
  const fn = sMonAnimFunctions[frontAnimId];
  if (!fn) return;
...
// ─── Surface lazy combat (battle_main _BattleAnimateFrontSprite, anti-cycle) ──
// battle_main lit globalThis.__pokemonAnimation.LaunchAnimationTaskForFrontSprite
// (id, NOM ANIM_*) sans importer ce module directement.
