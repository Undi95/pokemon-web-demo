...
// Espace d'ids DEDIE aux symboles anim (ptr C des battle_anim_scripts :
// AnimTask_*, templates, SpriteCB_*) : 0x1000+ -> zero collision avec les
// ids 0-81 de la table commune (les bytecodes battle_scripts_1/2 RESTAURES
// les referencent ; l'ordre de compilation n'est PAS stable). Goal T4.
const ANIM_SYMBOL_BASE = 0x1000;
const animSymbolIds = new Map();
function getAnimSymbolId(name) {
...
function resolveValue(v, labelOffsets, warnings) {
  ...
  // battle_anim_script (goal T4 2026-06-10) : les ptr C (AnimTask_*, sprite
  // templates gXxx, SpriteCB_*) sont INCONNUS du bytecode — emettre un
  // MARQUEUR NOMINAL (SYMBOL_MARKER | id, SYMBOLS_TABLE) au lieu de 0 (nom
  // perdu) -> le runtime route createsprite/createvisualtask par REGISTRY.
...
writeFileSync(join(outRoot, '_anim-symbols-table.ts'), [
  ...
  ...[...animSymbolIds.entries()].map(([name, id]) => `  { id: ${id}, name: ${JSON.stringify(name)} },`),
  '];',
  '',
].join('\n'));
