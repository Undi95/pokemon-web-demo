...
## 🟢 CANDIDATS migration → game/ (5) — engine/, purs, .c existe

| fichier engine | → game/ | nos fn | match | pureté | complét. | divergence |
|---|---|---|---|---|---|---|
| `engine/ui/mon-summary-anim.ts` | `game/pokemon_animation.ts` | 250 | 227/241 | 91% | 94% | 17 |
| `engine/field/direction-coords.ts` | `game/event_object_movement.ts` | 10 | 8/726 | 80% | 1% | 2 |
| `engine/field/swap-line.ts` | `game/menu_helpers.ts` | 7 | 5/25 | 71% | 20% | 2 |
| `engine/field/field-effect-active-list.ts` | `game/field_effect.ts` | 4 | 4/227 | 100% | 2% | 0 |
| `engine/battle/battle-window-frame.ts` | `game/battle_script_commands.ts` | 4 | 3/287 | 75% | 1% | 1 |

## 🟡 Partiels (17) — décomp-nommés mais dispersés/hybrides (évaluer au cas par cas)

| fichier engine | best .c | nos fn | match partout | pureté(p) | divergence |
|---|---|---|---|---|---|
| `engine/battle/battle-anim-interpreter.ts` | `battle_anim.c` | 131 | 85 | 65% | 46 |
| `engine/bag/bag-menu.ts` | `item_menu.c` | 82 | 57 | 70% | 25 |
| `engine/battle/battle-controllers.ts` | `battle_controllers.c` | 50 | 41 | 82% | 9 |
| `engine/battle/battle-link-end.ts` | `battle_main.c` | 42 | 37 | 88% | 5 |
| `engine/ui/gba-window-system.ts` | `window.c` | 43 | 34 | 79% | 9 |
| `engine/battle/battle-setup-helpers.ts` | `battle_setup.c` | 28 | 22 | 79% | 6 |
| `engine/battle/constants.ts` | `—` | 22 | 22 | 100% | 0 |
| `engine/bag/bag-menu-ctx.ts` | `item_menu.c` | 28 | 17 | 61% | 11 |
| `engine/battle/text-buffers.ts` | `—` | 14 | 13 | 93% | 1 |
| `engine/ui/pokedex-flags.ts` | `pokemon.c` | 11 | 11 | 100% | 0 |
| `engine/battle/battle-controllers-ipc.ts` | `battle_controllers.c` | 13 | 10 | 77% | 3 |
| `engine/battle/data/experience-tables.ts` | `—` | 11 | 8 | 73% | 3 |
| `engine/bag/bag-pockets.ts` | `item.c` | 10 | 6 | 60% | 4 |
| `engine/battle/battle-controllers-init.ts` | `battle_controllers.c` | 6 | 6 | 100% | 0 |
| `engine/script/script-vars.ts` | `event_data.c` | 9 | 6 | 67% | 3 |
| `engine/ui/money-box-ui.ts` | `coins.c` | 9 | 6 | 67% | 3 |
| `engine/field/region-map-data.ts` | `region_map.c` | 8 | 5 | 63% | 3 |

## 🔴 Harness/maison (81) — reste en engine/ (peu/pas de noms décomp)
...
- `engine/battle/battle-anim-generated-bridge.ts` (6 fn, 0 décomp-nommées)
- `engine/battle/battle-anim-registry.ts` (6 fn, 0 décomp-nommées)
- `engine/battle/battle-controller-tick.ts` (2 fn, 0 décomp-nommées)
...
- `engine/battle/battle-string-decoder.ts` (31 fn, 0 décomp-nommées)
...
- `engine/battle/battle-yesno-box.ts` (6 fn, 1 décomp-nommées)
...
- `engine/battle/party-storage.ts` (35 fn, 16 décomp-nommées)
- `engine/battle/script-interpreter.ts` (30 fn, 11 décomp-nommées)
- `engine/battle/state.ts` (61 fn, 0 décomp-nommées)
- `engine/battle/wire-bytecode-bridge.ts` (20 fn, 0 décomp-nommées)
- `engine/data/game-data.ts` (34 fn, 2 décomp-nommées)
- `engine/field/character-anims.ts` (3 fn, 0 décomp-nommées)
- `engine/field/field-globals.ts` (10 fn, 0 décomp-nommées)
...
- `engine/field/movement.ts` (4 fn, 0 décomp-nommées)
- `engine/field/npc-loader.ts` (2 fn, 0 décomp-nommées)
- `engine/field/object-event-base-oam.ts` (1 fn, 0 décomp-nommées)
- `engine/field/object-event-graphics-info-data.ts` (495 fn, 1 décomp-nommées)
...
- `engine/field/region-map.ts` (20 fn, 1 décomp-nommées)
...
- `engine/pokemon/pokemon.ts` (28 fn, 2 décomp-nommées)
...
- `engine/save/reset-all-berries.ts` (1 fn, 0 décomp-nommées)
...
- `engine/script/script-opcodes.ts` (5 fn, 0 décomp-nommées)
- `engine/script/specials-registry.ts` (9 fn, 1 décomp-nommées)
- `engine/system/fade-screen.ts` (1 fn, 1 décomp-nommées)
- `engine/system/flash-mask.ts` (2 fn, 0 décomp-nommées)
- `engine/system/multichoice-data.ts` (3 fn, 0 décomp-nommées)
- `engine/system/music.ts` (13 fn, 0 décomp-nommées)
- `engine/system/pokeball-effects.ts` (11 fn, 3 décomp-nommées)
- `engine/system/string-buffers.ts` (7 fn, 1 décomp-nommées)
- `engine/ui/bitmap-font.ts` (7 fn, 0 décomp-nommées)
- `engine/ui/easy-chat-render.ts` (206 fn, 116 décomp-nommées)
- `engine/ui/gba-menu-system.ts` (13 fn, 7 décomp-nommées)
- `engine/ui/gba-strings.ts` (3 fn, 0 décomp-nommées)
- `engine/ui/gba-text-printer.ts` (14 fn, 0 décomp-nommées)
- `engine/ui/gba-text-system.ts` (12 fn, 1 décomp-nommées)
- `engine/ui/menu.ts` (4 fn, 0 décomp-nommées)
- `engine/ui/option-menu-impl.ts` (31 fn, 17 décomp-nommées)
- `engine/ui/option-menu-return.ts` (10 fn, 3 décomp-nommées)
- `engine/ui/overworld-welcome-impl.ts` (1 fn, 0 décomp-nommées)
- `engine/ui/party-screen.ts` (113 fn, 28 décomp-nommées)
- `engine/ui/pokedex-screen.ts` (5 fn, 0 décomp-nommées)
- `engine/ui/summary-screen.ts` (160 fn, 9 décomp-nommées)
