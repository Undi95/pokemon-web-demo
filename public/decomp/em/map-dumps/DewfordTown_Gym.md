...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_BRAWLY` | 4,3 | `MOVEMENT_TYPE_FACE_DOWN` | `DewfordTown_Gym_EventScript_Brawly` | `0` |
| `` | `OBJ_EVENT_GFX_BLACK_BELT` | 2,18 | `MOVEMENT_TYPE_FACE_UP` | `DewfordTown_Gym_EventScript_Takao` | `0` |
| `` | `OBJ_EVENT_GFX_GIRL_3` | 15,3 | `MOVEMENT_TYPE_FACE_DOWN` | `DewfordTown_Gym_EventScript_Jocelyn` | `0` |
| `` | `OBJ_EVENT_GFX_GIRL_3` | 12,25 | `MOVEMENT_TYPE_FACE_UP` | `DewfordTown_Gym_EventScript_Laura` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_2` | 5,24 | `MOVEMENT_TYPE_FACE_DOWN` | `DewfordTown_Gym_EventScript_GymGuide` | `0` |
| `` | `OBJ_EVENT_GFX_BLACK_BELT` | 7,8 | `MOVEMENT_TYPE_FACE_RIGHT` | `DewfordTown_Gym_EventScript_Cristian` | `0` |
| `` | `OBJ_EVENT_GFX_GIRL_3` | 16,9 | `MOVEMENT_TYPE_FACE_DOWN` | `DewfordTown_Gym_EventScript_Lilith` | `0` |
| `` | `OBJ_EVENT_GFX_SAILOR` | 14,12 | `MOVEMENT_TYPE_FACE_RIGHT` | `DewfordTown_Gym_EventScript_Brenden` | `0` |

## Warps (2)
- #0 (5,27) → `MAP_DEWFORD_TOWN` warp #2
- #1 (6,27) → `MAP_DEWFORD_TOWN` warp #2

## BG events / signs (4)
- (4,23) [sign] → `DewfordTown_Gym_EventScript_LeftGymStatue`
- (7,23) [sign] → `DewfordTown_Gym_EventScript_RightGymStatue`
- (4,24) [sign] → `DewfordTown_Gym_EventScript_LeftGymStatue`
- (7,24) [sign] → `DewfordTown_Gym_EventScript_RightGymStatue`
...
```
Hé, comment ça va, futur MAITRE\n{PLAYER}?\pBASTIEN, le CHAMPION D'ARENE\nde MYOKARA a des POKéMON de\ltype COMBAT.\pSi tu comptes te battre contre lui avec\ndes POKéMON de type NORMAL, tu\lrisques la déception.\pLes POKéMON COMBAT infligent de\nsérieux dégâts aux POKéMON de type\lNORMAL. Sois prudent!\pEn plus, l'ARENE est plongée dans\nl'obscurité la plus totale.\pMais elle s'illuminera quand tu auras\nvaincu les DRESSEURS sur ton chemin.\pBon, allez, c'est parti!$
```
### DewfordTown_Gym_Text_GymGuidePostVictory
```
Ouah! Il y a trop de lumière, ici!\pToi aussi, tu as l'air d'être une lumière!$
```
### DewfordTown_Gym_Text_TakaoIntro
```
La puissance est ma raison d'être!\nAllez, bats-toi!$
```
...
```
Ta compétence a surpassé ma force!\nTa technique est impressionnante!$
```
### DewfordTown_Gym_Text_JocelynIntro
```
Inutile de déranger BASTIEN!\nJe vais t'écraser moi-même!$
```
### DewfordTown_Gym_Text_JocelynDefeat
```
Mais… Comment?\nComment as-tu pu gagner si facilement?$
```
### DewfordTown_Gym_Text_JocelynPostBattle
```
Je ne vais pas rester sur une défaite.\nJe vais me motiver pour l'entraînement!\pUn jour, je deviendrai CHAMPION\nD'ARENE…\pNon, je deviendrai MAITRE de la\nLIGUE POKéMON!$
```
### DewfordTown_Gym_Text_LauraIntro
```
Comment oses-tu me provoquer?\nBon, tu ne viendras pas pleurer!$
```
...
```
J'ai l'habitude de me battre dans le noir\npour endurcir mon cœur.\pMais il fait toujours si noir que je n'ai\njamais vu le visage de BASTIEN…$
```
...
```
Je te souhaite de te perdre et de\nte retrouver à l'entrée!$
```
### DewfordTown_Gym_Text_BrendenIntro
```
Tu vas voir! On ne la fait pas à un vieux\nloup de mer comme moi!$
```
### DewfordTown_Gym_Text_BrendenDefeat
```
Je t'assure, ce n'est pas mon genre\nde perdre comme ça!$
```
### DewfordTown_Gym_Text_BrendenPostBattle
```
Je crois que tu as bien mérité de\nvoir notre CHAMPION D'ARENE.$
```
### DewfordTown_Gym_Text_CristianIntro
```
Si tu veux aller plus loin, tu devras\nd'abord me passer sur le corps!$
```
...
```
Tu as gagné!\nVa chercher ce BADGE!$
```
### DewfordTown_Gym_Text_BrawlyIntro
```
Je suis BASTIEN!\nLe CHAMPION D'ARENE de MYOKARA!\pJ'ai résisté aux vagues déchaînées\nde la région et je me suis endurci dans\lles cavernes obscures!\pAlors, comme ça, tu viens me défier?\nVoyons donc de quoi tu es capable!$
```
### DewfordTown_Gym_Text_BrawlyDefeat
```
Pfffou, bon sang! Tu as fait plus de\nvagues que je ne l'aurais imaginé!\pTu m'as submergé!\pJe l'admets, tu m'as vaincu.\nPrends ce BADGE!$
```
### DewfordTown_Gym_Text_ReceivedKnuckleBadge
```
{PLAYER} reçoit le BADGE POING\nde la part de BASTIEN.$
```
### DewfordTown_Gym_Text_KnuckleBadgeInfoTakeThis
```
Le BADGE POING oblige tous les POKéMON\njusqu'au niveau 30, même ceux échangés,\là obéir sans poser de question.\pEt tu pourras aussi utiliser la CS FLASH\nen dehors des combats.\pSi tu veux, tu peux aussi utiliser cette\nCAPSULE TECHNIQUE.$
```
### DewfordTown_Gym_Text_ExplainBulkUp
```
Cette CT08 contient GONFLETTE.\pC'est une technique qui permet de\nbooster l'ATTAQUE et la DEFENSE.\p… … … … … …$
```
### DewfordTown_Gym_Text_RegisteredBrawly
```
Vous avez enregistré le CHAMPION\nD'ARENE BASTIEN dans le POKéNAV.$
```
### DewfordTown_Gym_Text_BrawlyPostBattle
```
Je vois que tes talents ont causé un\nraz-de-marée d'admiration parmi les\lDRESSEURS, l'autre jour!$
```
...
```
ARENE POKéMON du VILLAGE MYOKARA\pNOUVEAU DRESSEUR RECONNU PAR BASTIEN:\n{PLAYER}$
```
### DewfordTown_Gym_Text_BrawlyPreRematch
```
BASTIEN: La vague se retire, mais elle\nrevient toujours sur le rivage.\pToi… Une telle vague de talent…\nJe savais que tu reviendrais!\pMontre-moi tes progrès!$
```
### DewfordTown_Gym_Text_BrawlyRematchDefeat
```
Waouh!\nJe me suis de nouveau fait submerger!$
```
### DewfordTown_Gym_Text_BrawlyPostRematch
```
BASTIEN: Les combats sont vastes et\nprofonds. On n'en voit pas le fond!\lExactement comme la mer de HOENN!$
```
### DewfordTown_Gym_Text_BrawlyRematchNeedTwoMons
```
BASTIEN: La vague se replie, mais elle\nrevient toujours sur le rivage.\pUne telle vague de talent comme toi…\nJe savais que tu reviendrais!\pOh? Tu n'as qu'un POKéMON apte\nau combat?\pJe suis désolé, mais je préfère que tu\nreviennes avec au moins deux POKéMON.$
```
