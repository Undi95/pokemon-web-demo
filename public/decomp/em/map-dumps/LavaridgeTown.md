...
## Connexions
- right (offset -40) → `MAP_ROUTE112`

## Object events (9 NPCs)
| local_id | gfx | x,y | mvmt | script | flag |
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_EXPERT_F` | 8,7 | `MOVEMENT_TYPE_FACE_DOWN` | `LavaridgeTown_EventScript_ExpertF` | `0` |
| `` | `OBJ_EVENT_GFX_EXPERT_M` | 5,1 | `MOVEMENT_TYPE_FACE_DOWN` | `LavaridgeTown_EventScript_ExpertM` | `0` |
| `` | `OBJ_EVENT_GFX_OLD_MAN` | 5,8 | `MOVEMENT_TYPE_FACE_DOWN` | `LavaridgeTown_EventScript_OldMan` | `0` |
| `` | `OBJ_EVENT_GFX_TWIN` | 10,13 | `MOVEMENT_TYPE_WANDER_UP_AND_DOWN` | `LavaridgeTown_EventScript_Twin` | `0` |
| `` | `OBJ_EVENT_GFX_HOT_SPRINGS_OLD_WOMAN` | 4,4 | `MOVEMENT_TYPE_FACE_DOWN_AND_RIGHT` | `LavaridgeTown_EventScript_HotSpringsOldWoman1` | `0` |
| `` | `OBJ_EVENT_GFX_HOT_SPRINGS_OLD_WOMAN` | 5,4 | `MOVEMENT_TYPE_FACE_DOWN_AND_LEFT` | `LavaridgeTown_EventScript_HotSpringsOldWoman2` | `0` |
| `LOCALID_LAVARIDGE_RIVAL_ON_BIKE` | `OBJ_EVENT_GFX_VAR_3` | 6,16 | `MOVEMENT_TYPE_FACE_LEFT` | `0x0` | `FLAG_HIDE_LAVARIDGE_TOWN_RIVAL_ON_BIKE` |
| `LOCALID_LAVARIDGE_RIVAL` | `OBJ_EVENT_GFX_VAR_0` | 12,15 | `MOVEMENT_TYPE_FACE_DOWN` | `0x0` | `FLAG_HIDE_LAVARIDGE_TOWN_RIVAL` |
| `` | `OBJ_EVENT_GFX_EXPERT_F` | 4,7 | `MOVEMENT_TYPE_FACE_DOWN` | `LavaridgeTown_EventScript_EggWoman` | `0` |

## Warps (6)
- #0 (12,15) → `MAP_LAVARIDGE_TOWN_HERB_SHOP` warp #0
- #1 (5,15) → `MAP_LAVARIDGE_TOWN_GYM_1F` warp #0
- #2 (15,5) → `MAP_LAVARIDGE_TOWN_MART` warp #0
- #3 (9,6) → `MAP_LAVARIDGE_TOWN_POKEMON_CENTER_1F` warp #0
- #4 (16,15) → `MAP_LAVARIDGE_TOWN_HOUSE` warp #0
- #5 (9,2) → `MAP_LAVARIDGE_TOWN_POKEMON_CENTER_1F` warp #3

## Coord events / triggers (1)
- (6,3) → `LavaridgeTown_EventScript_HotSpringsTrigger` (si `TRIGGER_RUN_IMMEDIATELY` == `0`)

## BG events / signs (8)
- (14,16) [sign] → `LavaridgeTown_EventScript_HerbShopSign`
- (7,15) [sign] → `LavaridgeTown_EventScript_GymSign`
- (17,5) [sign] → `Common_EventScript_ShowPokemartSign`
- (13,8) [sign] → `LavaridgeTown_EventScript_TownSign`
- (10,6) [sign] → `Common_EventScript_ShowPokemonCenterSign`
- (16,5) [sign] → `Common_EventScript_ShowPokemartSign`
- (11,6) [sign] → `Common_EventScript_ShowPokemonCenterSign`
- (4,5) [hidden_item] → ``
...
```
FLORA: {PLAYER}{KUN}! Ça faisait un bail!\pOh? Tu as eu le BADGE de VERMILAVA\npendant que j'étais aux sources.\pTu as dû attraper pas mal de POKéMON…\nEnfin, je devrais plutôt dire “dresser”\lpas mal de POKéMON.\pJe pense que ce serait bien pour toi\nd'avoir ça.$
```
### LavaridgeTown_Text_MayExplainGoGogglesChallengeDad
```
FLORA: Avec ces LUNET. SABLE,\ntu pourras sans problème traverser le\ldésert près de la ROUTE 111.\pMais, waouh, {PLAYER}{KUN}…\nTe battre dans les ARENES…\pTu t'entraînes dur car je parie que tu\nne veux pas perdre contre moi!\p{PLAYER}{KUN}, je crois que je devrais défier\nton père à l'ARENE de CLEMENTI-VILLE.\pA une prochaine fois, {PLAYER}{KUN}!$
```
### LavaridgeTown_Text_BrendanNiceBadgesTakeThis
```
BRICE: Hé, {PLAYER}, ça fait un bail.\nComment ça va?\pHum…\nVoilà une collection de BADGES\lplutôt honnête.\pBon, très bien.\nTu peux aussi prendre ça.$
```
### LavaridgeTown_Text_BrendanExplainGoGogglesChallengeDad
```
BRICE: Prends-les avec toi si tu\ncomptes aller dans ce désert, près de\lla ROUTE 111.\pIl devrait y avoir des POKéMON que tu\nne peux affronter que dans le désert.\lRegarde autour de toi pour les trouver.\pEn ce qui me concerne…\nJ'envisage de défier NORMAN, le\lCHAMPION D'ARENE de CLEMENTI-VILLE.\pContrairement à toi, {PLAYER}, ton père\nsemble être très fort.\pA plus tard!$
```
...
```
On prend toute notre eau chaude aux\nsources, mais elles ne s'assèchent pas.\pN'est-ce pas magique?\pLes sources chaudes se forment près\ndes volcans actifs. Des filets d'eau,\lchauffés par le magma, jaillissent de\lla terre sous forme de sources chaudes.$
```
### LavaridgeTown_Text_PokemonNippedBackside
```
Etre enseveli dans ce sable chaud…\nAhhhh…\pSi chaud et divin…\pEh? Aaaaah! Ouille!\pUn POKéMON m'a pincé le dos!$
```
### LavaridgeTown_Text_OhYouLikeHotSprings
```
Oh, tu aimes les sources chaudes,\nn'est-ce pas?\pC'est surprenant pour quelqu'un d'aussi\njeune que toi.$
```
### LavaridgeTown_Text_BatheInHotSpringsEveryDay
```
Je me baigne chaque jour dans les\nsources chaudes.\pJe veux être DRESSEUR comme\nADRIANE, et aussi belle et forte qu'elle.$
```
### LavaridgeTown_Text_IfPokemonInHotSprings
```
Si on mettait des POKéMON dans les\nsources, ça pourrait être très étrange.\pÇa pourrait être un bain électrique, un\nbain de bulles ou même un bain de lave…$
```
### LavaridgeTown_Text_HotSpringsClaims
```
Ils affirment que ces sources chaudes\ncalment la nervosité, soulagent les\ldouleurs musculaires, peuvent résoudre\lles soucis de cœur et attirent l'argent.$
```
### LavaridgeTown_Text_TownSign
```
VERMILAVA\p“CENTRE POKéMON des SOURCES\nCHAUDES. Détente garantie!”$
```
### LavaridgeTown_Text_GymSign
```
ARENE POKéMON de VERMILAVA\nCHAMPION: ADRIANE\l“Ici brûle le feu de la passion!”$
```
### LavaridgeTown_Text_HerbShopSign
```
HERBORISTERIE POKéMON\n“Plus c'est amer et plus c'est sain!”$
```
