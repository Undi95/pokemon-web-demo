...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_CAMPER` | 6,4 | `MOVEMENT_TYPE_FACE_LEFT` | `RustboroCity_House1_EventScript_Trader` | `0` |
| `` | `OBJ_EVENT_GFX_HIKER` | 9,2 | `MOVEMENT_TYPE_FACE_DOWN` | `RustboroCity_House1_EventScript_Hiker` | `0` |

## Warps (2)
- #0 (5,7) → `MAP_RUSTBORO_CITY` warp #7
- #1 (6,7) → `MAP_RUSTBORO_CITY` warp #7
...
```
Hein? Mon POKéMON est mignon?\nBien sûr que je le sais.\pMais si tu veux, je suis d'accord pour\nte l'échanger.\pJe t'échange mon {STR_VAR_2} contre\nun {STR_VAR_1} si tu veux.$
```
### RustboroCity_House1_Text_PleaseBeGoodToMyPokemon
```
Hé hé…\nSois sympa avec mon POKéMON.$
```
### RustboroCity_House1_Text_DoesntLookLikeMonToMe
```
Hein? Il ressemble pas vraiment à\nun {STR_VAR_1}!$
```
### RustboroCity_House1_Text_YouDontWantToThatsOkay
```
Oh, si tu ne veux pas, c'est d'accord.\nMais il est vraiment mignon, tu sais?$
```
### RustboroCity_House1_Text_AnyPokemonCanBeCute
```
Tous les POKéMON peuvent être mignons\ns'ils sont élevés avec gentillesse.$
```
### RustboroCity_House1_Text_AllSortsOfPlaces
```
Chaque endroit a ses types précis de\nPOKéMON et de personnes.\pJe trouve ça si fascinant que je me\nbalade un peu partout.$
```
