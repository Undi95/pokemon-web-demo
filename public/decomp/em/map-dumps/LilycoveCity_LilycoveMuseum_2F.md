...
|---|---|---|---|---|---|
| `LOCALID_MUSEUM_2F_CURATOR` | `OBJ_EVENT_GFX_GENTLEMAN` | 10,8 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_LilycoveMuseum_2F_EventScript_Curator` | `0` |
| `` | `OBJ_EVENT_GFX_GIRL_2` | 19,10 | `MOVEMENT_TYPE_WANDER_AROUND` | `LilycoveCity_LilycoveMuseum_2F_EventScript_Girl` | `FLAG_HIDE_LILYCOVE_MUSEUM_PATRON_2` |
| `` | `OBJ_EVENT_GFX_EXPERT_M` | 7,3 | `MOVEMENT_TYPE_WANDER_AROUND` | `LilycoveCity_LilycoveMuseum_2F_EventScript_ExpertM` | `FLAG_HIDE_LILYCOVE_MUSEUM_TOURISTS` |
| `` | `OBJ_EVENT_GFX_RICH_BOY` | 14,6 | `MOVEMENT_TYPE_WANDER_AROUND` | `LilycoveCity_LilycoveMuseum_2F_EventScript_RichBoy` | `FLAG_HIDE_LILYCOVE_MUSEUM_TOURISTS` |

## Warps (1)
- #0 (13,1) → `MAP_LILYCOVE_CITY_LILYCOVE_MUSEUM_1F` warp #2

## BG events / signs (12)
- (14,10) [sign] → `LilycoveCity_LilycoveMuseum_2F_EventScript_CutePainting`
- (15,10) [sign] → `LilycoveCity_LilycoveMuseum_2F_EventScript_CutePainting`
- (3,6) [sign] → `LilycoveCity_LilycoveMuseum_2F_EventScript_ToughPainting`
- (2,6) [sign] → `LilycoveCity_LilycoveMuseum_2F_EventScript_ToughPainting`
- (10,6) [sign] → `LilycoveCity_LilycoveMuseum_2F_EventScript_CoolPainting`
- (11,6) [sign] → `LilycoveCity_LilycoveMuseum_2F_EventScript_CoolPainting`
- (19,6) [sign] → `LilycoveCity_LilycoveMuseum_2F_EventScript_BeautyPainting`
- (18,6) [sign] → `LilycoveCity_LilycoveMuseum_2F_EventScript_BeautyPainting`
- (6,10) [sign] → `LilycoveCity_LilycoveMuseum_2F_EventScript_SmartPainting`
- (7,10) [sign] → `LilycoveCity_LilycoveMuseum_2F_EventScript_SmartPainting`
- (11,2) [sign] → `LilycoveCity_LilycoveMuseum_EventScript_BirdSculpture`
- (15,2) [sign] → `LilycoveCity_LilycoveMuseum_EventScript_BirdSculpture`
...
```
Notre salle d'œuvres contemporaines!$
```
### LilycoveCity_LilycoveMuseum_2F_Text_ExplainExhibitHall
```
Comme il est facile de le constater, il\nn'y a pas un seul tableau exposé.\pJe ne souhaite pas exposer d'œuvres\ndites “classiques” ici.\pLes œuvres de ce type ont déjà leur\nplace au rez-de-chaussée.\pMon désir est d'exposer des œuvres\nqui se distinguent de l'art classique.\pL'art, après tout, ne se limite pas\naux anciennes œuvres célèbres!\pCette salle, je désire l'emplir…\pd'œuvres modernes et pleines de vie\nreprésentant des POKéMON prêts à\ls'élancer dans une vie heureuse!$
```
### LilycoveCity_LilycoveMuseum_2F_Text_PleaseObtainPaintingsForExhibit
```
Je m'excuse. Je n'avais pas l'intention\nde monopoliser la conversation.\pTu es jeune et as déjà beaucoup\nvoyagé…\pTu as dû voir des POKéMON pleins\nde vie et des œuvres les dépeignant.\pSi tu découvrais une telle œuvre, nous\nte serions reconnaissants de bien\lvouloir obtenir la permission de\ll'artiste pour l'exposer ici.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_WishToFillExhibit
```
J'aimerais emplir cette salle de\ntableaux modernes et pleins de\lvie représentant des POKéMON.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_ThanksAddedNewPainting
```
Merci. Grâce à toi, nous avons ajouté\nun nouveau tableau à notre collection!\pIl paraît qu'il représente ton POKéMON.\nVraiment magnifique!\pMerci beaucoup!$
```
...
```
N'est-ce pas fabuleux?\nCette collection de tableaux!\pTous parfaits! Il n'y en a pas un\nmeilleur que les autres!\pCes tableaux ont fait augmenter le\nnombre de visiteurs dans notre MUSEE.\pMerci vraiment, {PLAYER}.\nMon rêve est devenu réalité!$
```
...
```
Je vais le garder pour toi jusqu'à notre\nprochaine rencontre.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_HonorToHaveYouVisit
```
Ah, ça fait plaisir de te voir, {PLAYER}!\pQuel honneur que tu trouves le temps de\nnous rendre visite entre tes voyages!\pDétends-toi et apprécie ta venue\nà nos côtés.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_ItsPinkPictureFrame
```
C'est un cadre à ornements roses.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_ItsYellowPictureFrame
```
C'est un cadre à ornements jaunes.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_ItsBluePictureFrame
```
C'est un cadre à ornements bleus.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_ItsRedPictureFrame
```
C'est un cadre à ornements rouges.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_ItsGreenPictureFrame
```
C'est un cadre à ornements verts.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_ItsPaintingOfPokemon
```
C'est un tableau de POKéMON.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_NewPaintingsSurprisedMe
```
J'ai été absente longtemps. Alors les\ntableaux à l'étage me surprennent.\pJ'aimerais bien que quelqu'un peigne\nmes POKéMON comme ça.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_NewPaintingsRatherAmusing
```
Alors voilà où sont exposés les\ntableaux à thèmes de POKéMON.\pJe vois, ces nouveaux tableaux sont\neffectivement très amusants.$
```
### LilycoveCity_LilycoveMuseum_2F_Text_ThesePaintingsOfYourPokemon
```
Waouh, c'est vrai? Ces tableaux\nreprésentent tes POKéMON.\pN'est-ce pas la joie ultime pour un\nDRESSEUR?$
```
