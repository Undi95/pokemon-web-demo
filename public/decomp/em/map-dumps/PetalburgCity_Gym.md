...
|---|---|---|---|---|---|
| `LOCALID_PETALBURG_GYM_NORMAN` | `OBJ_EVENT_GFX_NORMAN` | 4,2 | `MOVEMENT_TYPE_FACE_DOWN` | `PetalburgCity_Gym_EventScript_Norman` | `FLAG_HIDE_PETALBURG_GYM_NORMAN` |
| `` | `OBJ_EVENT_GFX_WOMAN_5` | 4,94 | `MOVEMENT_TYPE_FACE_DOWN` | `PetalburgCity_Gym_EventScript_Mary` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_3` | 4,81 | `MOVEMENT_TYPE_FACE_DOWN` | `PetalburgCity_Gym_EventScript_Randall` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_3` | 4,42 | `MOVEMENT_TYPE_FACE_DOWN` | `PetalburgCity_Gym_EventScript_Parker` | `0` |
| `` | `OBJ_EVENT_GFX_WOMAN_5` | 4,55 | `MOVEMENT_TYPE_FACE_DOWN` | `PetalburgCity_Gym_EventScript_Alexia` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_3` | 4,68 | `MOVEMENT_TYPE_FACE_DOWN` | `PetalburgCity_Gym_EventScript_George` | `0` |
| `` | `OBJ_EVENT_GFX_WOMAN_5` | 4,16 | `MOVEMENT_TYPE_FACE_DOWN` | `PetalburgCity_Gym_EventScript_Jody` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_3` | 4,29 | `MOVEMENT_TYPE_FACE_DOWN` | `PetalburgCity_Gym_EventScript_Berke` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_2` | 3,109 | `MOVEMENT_TYPE_FACE_DOWN` | `PetalburgCity_Gym_EventScript_GymGuide` | `FLAG_HIDE_PETALBURG_GYM_GREETER` |
| `LOCALID_PETALBURG_GYM_WALLY` | `OBJ_EVENT_GFX_WALLY` | 4,111 | `MOVEMENT_TYPE_FACE_UP` | `0x0` | `FLAG_HIDE_PETALBURG_GYM_WALLY` |
| `LOCALID_PETALBURG_GYM_WALLYS_DAD` | `OBJ_EVENT_GFX_POKEFAN_M` | 1,7 | `MOVEMENT_TYPE_FACE_UP` | `0x0` | `FLAG_HIDE_PETALBURG_GYM_WALLYS_DAD` |

## Warps (38)
- #0 (4,111) → `MAP_PETALBURG_CITY` warp #2
- #1 (5,111) → `MAP_PETALBURG_CITY` warp #2
- #2 (1,105) → `MAP_PETALBURG_CITY_GYM` warp #3
- #3 (7,85) → `MAP_PETALBURG_CITY_GYM` warp #2
- #4 (6,85) → `MAP_PETALBURG_CITY_GYM` warp #2
- #5 (7,105) → `MAP_PETALBURG_CITY_GYM` warp #6
- #6 (1,98) → `MAP_PETALBURG_CITY_GYM` warp #5
- #7 (2,98) → `MAP_PETALBURG_CITY_GYM` warp #5
- #8 (1,79) → `MAP_PETALBURG_CITY_GYM` warp #10
- #9 (7,79) → `MAP_PETALBURG_CITY_GYM` warp #12
- #10 (7,46) → `MAP_PETALBURG_CITY_GYM` warp #8
- #11 (6,46) → `MAP_PETALBURG_CITY_GYM` warp #8
- #12 (1,59) → `MAP_PETALBURG_CITY_GYM` warp #9
- #13 (2,59) → `MAP_PETALBURG_CITY_GYM` warp #9
- #14 (1,92) → `MAP_PETALBURG_CITY_GYM` warp #16
- #15 (7,92) → `MAP_PETALBURG_CITY_GYM` warp #18
- #16 (7,59) → `MAP_PETALBURG_CITY_GYM` warp #14
- #17 (6,59) → `MAP_PETALBURG_CITY_GYM` warp #14
- #18 (1,72) → `MAP_PETALBURG_CITY_GYM` warp #15
- #19 (2,72) → `MAP_PETALBURG_CITY_GYM` warp #15
- #20 (7,40) → `MAP_PETALBURG_CITY_GYM` warp #24
- #21 (1,53) → `MAP_PETALBURG_CITY_GYM` warp #26
- #22 (7,53) → `MAP_PETALBURG_CITY_GYM` warp #28
- #23 (1,66) → `MAP_PETALBURG_CITY_GYM` warp #30
- #24 (1,20) → `MAP_PETALBURG_CITY_GYM` warp #20
- #25 (2,20) → `MAP_PETALBURG_CITY_GYM` warp #20
- #26 (7,20) → `MAP_PETALBURG_CITY_GYM` warp #21
- #27 (6,20) → `MAP_PETALBURG_CITY_GYM` warp #21
- #28 (1,33) → `MAP_PETALBURG_CITY_GYM` warp #22
- #29 (2,33) → `MAP_PETALBURG_CITY_GYM` warp #22
- #30 (7,33) → `MAP_PETALBURG_CITY_GYM` warp #23
- #31 (6,33) → `MAP_PETALBURG_CITY_GYM` warp #23
- #32 (7,14) → `MAP_PETALBURG_CITY_GYM` warp #34
- #33 (1,27) → `MAP_PETALBURG_CITY_GYM` warp #36
- #34 (1,7) → `MAP_PETALBURG_CITY_GYM` warp #32
- #35 (2,7) → `MAP_PETALBURG_CITY_GYM` warp #32
- #36 (7,7) → `MAP_PETALBURG_CITY_GYM` warp #33
- #37 (6,7) → `MAP_PETALBURG_CITY_GYM` warp #33

## BG events / signs (14)
- (1,105) [sign] → `PetalburgCity_Gym_EventScript_SpeedRoomDoor`
- (7,105) [sign] → `PetalburgCity_Gym_EventScript_AccuracyRoomDoor`
- (1,79) [sign] → `PetalburgCity_Gym_EventScript_ConfusionRoomDoor`
- (7,79) [sign] → `PetalburgCity_Gym_EventScript_LeftDefenseRoomDoor`
- (1,92) [sign] → `PetalburgCity_Gym_EventScript_RightDefenseRoomDoor`
- (7,92) [sign] → `PetalburgCity_Gym_EventScript_RecoveryRoomDoor`
- (7,40) [sign] → `PetalburgCity_Gym_EventScript_LeftStrengthRoomDoor`
- (1,53) [sign] → `PetalburgCity_Gym_EventScript_RightStrengthRoomDoor`
- (7,53) [sign] → `PetalburgCity_Gym_EventScript_LeftOHKORoomDoor`
- (1,66) [sign] → `PetalburgCity_Gym_EventScript_RightOHKORoomDoor`
- (7,14) [sign] → `PetalburgCity_Gym_EventScript_LeftGymLeadersRoomDoor`
- (1,27) [sign] → `PetalburgCity_Gym_EventScript_RightGymLeadersRoomDoor`
- (1,110) [sign] → `PetalburgCity_Gym_EventScript_LeftGymStatue`
- (7,110) [sign] → `PetalburgCity_Gym_EventScript_RightGymStatue`
...
```
PAPA: Hum?\pTiens, mais c'est toi {PLAYER}!\nAlors, le déménagement est fini?\pJe suis surpris de voir que tu as réussi\nà venir jusqu'ici par toi-même.\pOh, je vois.\nTu es avec tes POKéMON.\pHum… Je suppose que tu vas aussi\ndevenir DRESSEUR, {PLAYER}.\pC'est une très bonne nouvelle!\nJ'attendais ça avec impatience!$
```
### PetalburgCity_Gym_Text_WallyIdLikeAPokemon
```
Oh… Je… j'aimerais avoir\nun POKéMON, s'il vous plaît…$
```
### PetalburgCity_Gym_Text_DadOhYoureWallyRight
```
PAPA: Hein? Tu es… Heu… Oh, mais oui,\ntu es TIMMY, n'est-ce pas?$
```
### PetalburgCity_Gym_Text_WallyIveNeverCaughtAPokemon
```
TIMMY: Je vais dans ma famille\nà VERGAZON.\pComme je savais que j'allais être seul,\nj'voulais attraper un POKéMON en route.\pMais je n'ai pas réussi.\nJe ne sais pas comment faire…$
```
...
```
PAPA: {PLAYER}, tu as entendu?\pVa avec TIMMY et assure-toi\nqu'il attrape un POKéMON sans risque.$
```
### PetalburgCity_Gym_Text_IllLoanYouMyZigzagoon
```
Tiens, TIMMY! Je te prête mon POKéMON.\pTIMMY reçoit un ZIGZATON!$
```
### PetalburgCity_Gym_Text_WallyThankYouAndDadGivesPokeBall
```
TIMMY: Oh, waouh!\nUn POKéMON!\pPAPA: Hum. Je vais aussi te donner\nune POKé BALL. Allez, file!\pTIMMY reçoit une POKé BALL!$
```
...
```
TIMMY: Oui, en effet. Merci.\nJe vous rends votre POKéMON.\p{PLAYER}, merci de m'avoir accompagné.\pC'est grâce à vous deux que j'ai \npu attraper mon POKéMON.\pJe promets de bien m'en occuper.\pOh! Maman m'attend. J'y vais!\nA plus, {PLAYER}!$
```
### PetalburgCity_Gym_Text_DadGoCollectBadges
```
PAPA: Bon, maintenant…\p{PLAYER}, si tu veux devenir un\nbon DRESSEUR, voici mon conseil.\pVa à MEROUVILLE, c'est la ville\nqui se trouve un peu plus loin.\pLà, il te faudra battre le CHAMPION\nD'ARENE, ROXANNE.\pEnsuite, rends-toi dans les autres\nARENES et bats les CHAMPIONS.\pTu dois obtenir leurs BADGES,\ncompris?\pBien sûr, je suis aussi un CHAMPION.\nUn jour, on s'affrontera, {PLAYER}.\pMais avant, il va falloir que tu\naméliores ton niveau, {PLAYER}.$
```
### PetalburgCity_Gym_Text_NormanGoToRustboro
```
PAPA: Hum? Tu ne devais pas aller\nà l'ARENE de MEROUVILLE?\pJe n'ai aucun intérêt à affronter \nun DRESSEUR novice.\p{PLAYER}, je t'affronterai quand\ntu auras un meilleur niveau.\pNous combattrons, {PLAYER}, quand tu\nauras quatre BADGES à me montrer, OK?$
```
### PetalburgCity_Gym_Text_NormanGoToDewford
```
PAPA: Je vois… Alors, tu as battu\nle CHAMPION de MEROUVILLE.\pMais il y a bien d'autres DRESSEURS\nplus forts que toi, {PLAYER}.\pTraverse l'océan et va au\nVILLAGE MYOKARA.\pTu devras y affronter un CHAMPION\nnommé BASTIEN.$
```
### PetalburgCity_Gym_Text_YouHaveGottenStronger
```
PAPA: Hum… {PLAYER}, tu as\nbien progressé.\pJe le vois au nombre de BADGES\nque tu as obtenus.$
```
### PetalburgCity_Gym_Text_NormanIntro
```
PAPA: Hum…\nAlors tu as obtenu quatre BADGES.\pTrès bien. Comme promis, nous allons\nfaire un combat de POKéMON.\p{PLAYER}…\pJe suis si heureux de pouvoir\naffronter mon propre enfant.\pMais un combat reste un combat!\pJe ferai tout ce qui est en mon pouvoir\nde CHAMPION D'ARENE pour gagner.\p{PLAYER}, il va aussi falloir donner\nle meilleur de toi-même!$
```
### PetalburgCity_Gym_Text_NormanDefeat
```
…\pJe… Je ne peux…\pJe ne peux pas le croire. {PLAYER}…\pJ'ai perdu contre toi?\pMais le jeu, c'est le jeu!\nVoilà pour toi!$
```
### PetalburgCity_Gym_Text_ReceivedBalanceBadge
```
{PLAYER} reçoit le BADGE BALANCIER\nde la part de PAPA!$
```
### PetalburgCity_Gym_Text_ExplainBalanceBadgeTakeThis
```
PAPA: Avec ce BADGE, la DEFENSE\nde tous tes POKéMON va augmenter.\pTout POKéMON connaissant la CS SURF\npourra se déplacer sur l'eau.\pJe te l'offre, {PLAYER}. Je suis sûr\nque tu en feras bon usage.$
```
### PetalburgCity_Gym_Text_ExplainFacade
```
PAPA: La CT42 contient FACADE.\pElle double la puissance des capacités\nsi le POKéMON est empoisonné,\lparalysé ou brûlé.\pElle peut permettre de retourner une\nsituation difficile à son avantage.$
```
### PetalburgCity_Gym_Text_DadHappyAndSad
```
PAPA: En tant que CHAMPION,\nje ne peux exprimer ma déception…\pEt en tant que père, ça me rend à la fois\nheureux et un peu triste. Etrange…$
```
### PetalburgCity_Gym_Text_PleaseComeWithMe
```
Ah, te voilà {PLAYER}{KUN}!\pViens avec moi, s'il te plaît.\nJe voudrais te donner quelque chose.$
```
### PetalburgCity_Gym_Text_LetMeBorrowPlayer
```
NORMAN, ça ne t'embête pas,\nn'est-ce pas?\pTu veux bien me laisser parler avec\n{PLAYER}{KUN} pendant deux minutes?$
```
### PetalburgCity_Gym_Text_DadGoingToKeepTraining
```
PAPA: {PLAYER}, tu devrais rendre visite\nà ta mère le plus souvent possible.\pJe vais rester ici et je redoublerai\nd'efforts pour mon entraînement.\pEn tant que DRESSEUR, je dois \nprendre ma revanche, {PLAYER}!$
```
### PetalburgCity_Gym_Text_DadNoAmountOfTrainingIsEnough
```
PAPA: {PLAYER}, comment ça se passe?\pLe monde des POKéMON est vaste.\pJe pense que tout l'entraînement du\nmonde ne suffirait jamais.$
```
### PetalburgCity_Gym_Text_GymGuideAdvice
```
Hé, comment ça va, futur MAITRE\n{PLAYER}?\pLes portes de cette ARENE s'ouvrent\ndès que tu bats un DRESSEUR.\pOups! Ici, les portes sont déjà\nouvertes. Ne m'attaque pas!\pLes DRESSEURS de l'ARENE de CLEMENTI\nutilisent toutes sortes d'objets.\pLa porte de gauche mène à la\nSALLE VITESSE.\pLa porte de droite mène à la\nSALLE PRECISION.\pLe nom de la salle sera inscrit\nsur la porte. Fais le bon choix!\pAprès avoir choisi la porte…\nOh, et après tout, vas-y!$
```
### PetalburgCity_Gym_Text_GymGuidePostVictory
```
{PLAYER}! Waouh! Tu as même battu\nton propre père!\pOh, waouh! Quel retournement\nde situation exceptionnel!$
```
### PetalburgCity_Gym_Text_RandallIntro
```
La capacité d'attaquer avant l'ennemi…\pRien que ça suffit à donner un avantage\ncertain, pas vrai?$
```
...
```
Va dans la salle suivante!\nUn autre combat t'attend.\pA gauche, se trouve la SALLE\nCONFUSION.\pA droite, se trouve la SALLE DEFENSE.$
```
### PetalburgCity_Gym_Text_RandallPostBadge
```
Peu importe qui tu bats ou contre\nqui tu perds, tu ne dois jamais négliger\ll'entraînement ni tes POKéMON.\pD'après moi, c'est avant tout\nle devoir d'un CHAMPION D'ARENE.$
```
### PetalburgCity_Gym_Text_ParkerIntro
```
C'est la SALLE CONFUSION.\pLaisse-moi voir si tu es vraiment\nproche de tes POKéMON!$
```
### PetalburgCity_Gym_Text_ParkerDefeat
```
Je n'ai pas réussi à vous rendre\nsuffisamment confus…\pLe lien qui t'unit à tes POKéMON\nest vraiment très fort.$
```
### PetalburgCity_Gym_Text_ParkerPostBattle
```
La salle suivante est la SALLE FORCE.\nPeux-tu résister à la force pure?$
```
### PetalburgCity_Gym_Text_ParkerPostBadge
```
Après ta victoire sur notre CHAMPION,\nl'entraînement s'est intensifié.$
```
### PetalburgCity_Gym_Text_GeorgeIntro
```
C'est au moment où tu crois gagner\nque ton adversaire récupère ses PV…\pTu peux imaginer comme c'est\nfrustrant?\pJe vais justement te montrer\nà quel point ça l'est!$
```
### PetalburgCity_Gym_Text_GeorgeDefeat
```
Je n'ai pas pu récupérer assez de PV…\nQuelle puissance d'ATTAQUE…$
```
### PetalburgCity_Gym_Text_GeorgePostBattle
```
Je n'en attendais pas moins d'un\nenfant de CHAMPION.\pNon, attends! Il faut se donner du mal\npour avoir les capacités d'un DRESSEUR.\pJ'ai perdu parce que je n'ai pas fait\nsuffisamment d'efforts.\pEn avant vers la SALLE COUP CRITIQUE!$
```
### PetalburgCity_Gym_Text_GeorgePostBadge
```
Je vais continuer à m'entraîner dans\nles ARENES et je deviendrai CHAMPION.$
```
### PetalburgCity_Gym_Text_BerkeIntro
```
Je ne vais pas te faciliter les choses\nparce que tu es l'enfant du CHAMPION.\pTu vas voir comme un coup critique\npeut être terrible pour un POKéMON!$
```
### PetalburgCity_Gym_Text_BerkeDefeat
```
Quelle puissance!\nJe ne regrette pas ce combat.$
```
### PetalburgCity_Gym_Text_BerkePostBattle
```
Ton père est très fort. C'est un\nCHAMPION que je respecte.\pEt je discerne en toi la même ardeur\nque celle de ton père.\pJ'espère que tu affronteras ton\npère dans un remarquable combat.$
```
### PetalburgCity_Gym_Text_BerkePostBadge
```
Depuis que ton père est le CHAMPION,\nles DRESSEURS de CLEMENTI-VILLE\lsont plus résistants.$
```
### PetalburgCity_Gym_Text_MaryIntro
```
Hi hi hi…\nC'est la SALLE PRECISION.\pC'est pénible de voir que certains\nne ratent jamais aucune attaque.$
```
### PetalburgCity_Gym_Text_MaryDefeat
```
A côté de moi, tu joues dans la\ncour des grands…$
```
### PetalburgCity_Gym_Text_MaryPostBattle
```
D'autres DRESSEURS, encore\nplus forts, t'attendent.\pA gauche, c'est la SALLE DEFENSE\net à droite, la SALLE RECUPERATION.\pDe toute façon, la puissance d'ATTAQUE\nde tes POKéMON sera mise à l'épreuve.$
```
### PetalburgCity_Gym_Text_MaryPostBadge
```
Sais-tu ce à quoi nous aspirons\nen tant que DRESSEURS?\pNous nous efforçons d'être en\ntotale osmose avec nos POKéMON.$
```
### PetalburgCity_Gym_Text_AlexiaIntro
```
Plus ma DEFENSE est élevée, plus\naudacieuse je peux être en attaquant.$
```
### PetalburgCity_Gym_Text_AlexiaDefeat
```
Nos défenses n'ont pas suffi…$
```
### PetalburgCity_Gym_Text_AlexiaPostBattle
```
Je dois dire que tu m'as donné\nune bonne leçon.\pAllez, continue! La porte de gauche\nmène à la SALLE FORCE.\pLa porte de droite donne sur\nla SALLE COUP CRITIQUE.\pToutes deux mènent à des DRESSEURS\nspécialisés dans l'offensive.$
```
### PetalburgCity_Gym_Text_AlexiaPostBadge
```
Salut! Tu as déjà utilisé SURF? Si ce\nn'est pas le cas, tu devrais essayer.$
```
### PetalburgCity_Gym_Text_JodyIntro
```
Notre CHAMPION nous a dit de\ntout donner pour te battre.\pMême si tu es son enfant!$
```
### PetalburgCity_Gym_Text_JodyDefeat
```
Mais… J'ai pourtant tout donné!$
```
### PetalburgCity_Gym_Text_JodyPostBattle
```
La façon dont tu utilises tes POKéMON…\nC'est tout ton père!\pContinue! Le CHAMPION, ton père,\nt'attend.$
```
### PetalburgCity_Gym_Text_JodyPostBadge
```
En effet, c'est très bien de rendre\ntes POKéMON plus forts.\pMais qu'est-ce qui détermine\nla victoire?\pCe sont les sentiments des DRESSEURS\npour leurs POKéMON.$
```
### PetalburgCity_Gym_Text_DoorAppearsLocked
```
Cette porte semble être fermée\nà clé pour le moment…$
```
### PetalburgCity_Gym_Text_EnterSpeedRoom
```
“SALLE VITESSE”\nest inscrit sur cette porte.\pVoulez-vous entrer?$
```
...
```
SALLE du CHAMPION D'ARENE\p“Va voir quels POKéMON t'attendent!”\nest inscrit sur cette porte.\pVoulez-vous entrer?$
```
...
```
ARENE POKéMON de CLEMENTI-VILLE\pNOUVEAU DRESSEUR RECONNU PAR NORMAN:\n{PLAYER}$
```
### PetalburgCity_Gym_Text_NormanPreRematch
```
PAPA: {PLAYER}…\nJe savais que tu viendrais.\pJe ne peux pas refuser un\ncombat contre toi.\pTu comprends, n'est-ce pas?\pDans une ARENE, les liens du sang\nne comptent pas.\p… Nous sommes de la même\nfamille, mais nous ne nous ferons pas\lde cadeau!\pN'est-ce pas, {PLAYER}?$
```
### PetalburgCity_Gym_Text_NormanRematchDefeat
```
Hein… Haha…\nTu n'as vraiment pas retenu tes coups…$
```
### PetalburgCity_Gym_Text_NormanPostRematch
```
PAPA: {PLAYER}…\nEtait-ce un rêve?\pMon rêve…\nHahaha…\lEn fait, il s'est déjà réalisé.$
```
### PetalburgCity_Gym_Text_NormanRematchNeedTwoMons
```
PAPA: {PLAYER}…\nJe savais que tu viendrais.\pJe ne peux pas refuser un\ncombat contre toi.\pTu comprends, n'est-ce pas?\pDans une ARENE, les liens du sang\nne comptent pas.\p… Une seconde, {PLAYER}.\nN'as-tu vraiment qu'un POKéMON\lcapable de se battre?\pDésolé, mais tu devras revenir avec\nau moins deux POKéMON.$
```
