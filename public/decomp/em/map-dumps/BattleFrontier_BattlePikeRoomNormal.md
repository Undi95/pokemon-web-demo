...
|---|---|---|---|---|---|
| `LOCALID_PIKE_ROOM_NPC_1` | `OBJ_EVENT_GFX_VAR_0` | 4,4 | `MOVEMENT_TYPE_FACE_DOWN` | `BattleFrontier_BattlePikeRoomNormal_EventScript_NPC` | `0` |
| `LOCALID_PIKE_ROOM_NPC_2` | `OBJ_EVENT_GFX_VAR_1` | 3,4 | `MOVEMENT_TYPE_FACE_DOWN` | `BattleFrontier_BattlePikeRoomNormal_EventScript_StatusMon` | `0` |

## Coord events / triggers (7)
- (4,3) → `BattleFrontier_BattlePikeRoomNormal_EventScript_Exit` (si `VAR_TEMP_1` == `0`)
- (3,6) → `BattleFrontier_BattlePikeRoomNormal_EventScript_SetEnteredRoom` (si `VAR_TEMP_2` == `0`)
- (3,7) → `BattleFrontier_BattlePikeRoomNormal_EventScript_NoTurningBack` (si `VAR_TEMP_3` == `1`)
- (4,6) → `BattleFrontier_BattlePikeRoomNormal_EventScript_SetEnteredRoom` (si `VAR_TEMP_2` == `0`)
- (5,6) → `BattleFrontier_BattlePikeRoomNormal_EventScript_SetEnteredRoom` (si `VAR_TEMP_2` == `0`)
- (4,7) → `BattleFrontier_BattlePikeRoomNormal_EventScript_NoTurningBack` (si `VAR_TEMP_3` == `1`)
- (5,7) → `BattleFrontier_BattlePikeRoomNormal_EventScript_NoTurningBack` (si `VAR_TEMP_3` == `1`)
...
```
Bienvenue…\nFélicitations pour votre chance…\pJe vais soigner vos POKéMON…$
```
### BattleFrontier_BattlePikeRoomNormal_Text_EnjoyRestOfChallenge
```
Pourvu que la chance ne vous abandonne\npas pour le reste du COMBAT HASARD…$
```
### BattleFrontier_BattlePikeRoomNormal_Text_WillRestoreOneMon
```
Ah, tu en as de la chance,\nje suis de bonne humeur.\pJe vais soigner un de tes POKéMON.$
```
...
```
Je suis vraiment désolé…\pMon KIRLIA est de nature TIMIDE…$
```
### BattleFrontier_BattlePikeRoomNormal_Text_ApologizeForDusclops
```
Je suis vraiment désolé…\pMon TERACLOPE est de nature TIMIDE…$
```
### BattleFrontier_BattlePikeRoomNormal_Text_AttacksWhenStartled
```
Il attaque sans prévenir quand il est\nsurpris…\pTes POKéMON et toi allez bien?$
```
### BattleFrontier_BattlePikeRoomNormal_Text_ApologizeHopeMonsAreFine
```
Ce qui est arrivé est navrant… J'espère\nque tes POKéMON et toi allez bien.$
```
...
```
Bienvenue…\pIci, vous allez affronter un DRESSEUR\nplutôt coriace…$
```
### BattleFrontier_BattlePikeRoomNormal_Text_RestoreToFullHealth
```
Magnifique…\pPour vous récompenser, je vais soigner\nvos POKéMON…$
```
...
```
Je suis CHARLINE…\nC'est moi qui commande ici…\lOn m'appelle la REINE VENIN…\pComme tu dois le savoir, tu vas devoir\nessayer de me battre…\pBon, ne reste pas là à bayer aux\ncorneilles. Dépêche-toi, voyons…$
```
### BattleFrontier_BattlePikeRoomNormal_Text_HopeYouDidntUseUpLuck
```
J'espère que tu n'as pas encore épuisé\ntoute ta chance…$
```
...
```
Le SYMBOLE CHANCE a été ajouté sur le\nPASSE ZONE!$
```
...
```
CHARLINE: Encore toi…\p… … … … … …\pJ'ai dû en écraser plus d'un pour arriver\nlà où je suis…\pJe ne me sens pas obligée de continuer\nà perdre contre toi…\p… … … … … …\nBien… Allons-y…$
```
...
```
Ah, c'est ton jour de chance,\nje suis de bonne humeur.\pJe vais soigner deux de tes POKéMON!$
```
