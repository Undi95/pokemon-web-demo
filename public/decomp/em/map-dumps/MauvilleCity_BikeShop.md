...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_MAN_2` | 2,5 | `MOVEMENT_TYPE_FACE_LEFT` | `MauvilleCity_BikeShop_EventScript_Rydel` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_5` | 7,6 | `MOVEMENT_TYPE_WANDER_AROUND` | `MauvilleCity_BikeShop_EventScript_Assistant` | `0` |

## Warps (2)
- #0 (3,8) → `MAP_MAUVILLE_CITY` warp #2
- #1 (4,8) → `MAP_MAUVILLE_CITY` warp #2

## BG events / signs (2)
- (8,1) [sign] → `MauvilleCity_BikeShop_EventScript_MachBikeHandbook`
- (11,1) [sign] → `MauvilleCity_BikeShop_EventScript_AcroBikeHandbook`
...
```
Bon, bon, bon… Qu'est-ce qu'on a là?\nUn client des plus énergiques!\pMoi? Tu peux m'appeler RODOLPHE.\nJe suis le propriétaire de ce magasin\lde vélos.$
```
### MauvilleCity_BikeShop_Text_DidYouComeFromFarAway
```
RODOLPHE: Tes CHAUSSURES DE SPORT…\nElles sont dégoûtantes!\pTu viens de loin?$
```
### MauvilleCity_BikeShop_Text_GuessYouDontNeedBike
```
RODOLPHE: C'est vrai?\pAlors je suppose que tu n'as pas\nbesoin de mes VELOS.$
```
### MauvilleCity_BikeShop_Text_ExplainBikesChooseWhichOne
```
RODOLPHE: Hum, hum… … … … …\n… … … … … … … …\pTu veux dire que tu as fait toute\nla route depuis BOURG-EN-VOL?\pMon dieu!\nC'est terriblement loin!\pSi tu avais un de mes VELOS, tu pourrais\naller n'importe où sans problème, en\lsentant le vent te caresser le visage!\pTu sais quoi?\nJe vais te donner un VELO!\pOh, attends une seconde!\pJ'ai oublié de te préciser qu'il y\na deux sortes de VELOS.\pIl y a le VELO COURSE et le\nVELO CROSS.\pLe VELO COURSE est pour ceux qui\nveulent sentir le souffle du vent.\pEt le VELO CROSS est pour ceux qui\npréfèrent les performances techniques.\pJe suis un vrai passionné. Tu peux\nprendre celui que tu veux.\pAlors, tu choisis lequel?$
```
### MauvilleCity_BikeShop_Text_ChoseMachBike
```
{PLAYER} choisit le VELO COURSE.$
```
### MauvilleCity_BikeShop_Text_ChoseAcroBike
```
{PLAYER} choisit le VELO CROSS.$
```
### MauvilleCity_BikeShop_Text_ComeBackToSwitchBikes
```
RODOLPHE: Si tu as besoin de changer de\nVELO, n'hésite pas à venir me voir!$
```
### MauvilleCity_BikeShop_Text_WantToSwitchBikes
```
RODOLPHE: Oh? Tu voudrais changer\nde VELO?$
```
### MauvilleCity_BikeShop_Text_IllSwitchBikes
```
RODOLPHE: D'accord, pas de problème!\nJe t'échange ton VELO contre l'autre.$
```
### MauvilleCity_BikeShop_Text_ExchangedMachForAcro
```
{PLAYER} reçoit le VELO CROSS en\néchange du VELO COURSE.$
```
### MauvilleCity_BikeShop_Text_ExchangedAcroForMach
```
{PLAYER} reçoit le VELO COURSE en\néchange du VELO CROSS.$
```
### MauvilleCity_BikeShop_Text_HappyYouLikeIt
```
RODOLPHE: Très bien!\nContent qu'il te convienne!$
```
### MauvilleCity_BikeShop_Text_OhYourBikeIsInPC
```
Oh? Qu'est-ce qui est arrivé au VELO\nque je t'avais donné?\pAh, je vois! Tu l'as stocké sur ton PC.\pBon, sors-le du PC et je te l'échangerai!\pQue les vents te soient favorables!$
```
### MauvilleCity_BikeShop_Text_HandbooksAreInBack
```
J'apprends beaucoup de choses sur les\nVELOS depuis que je travaille ici.\pSi tu as besoin de conseils pour\nconduire ton vélo, il y a deux ou\ltrois manuels derrière.$
```
### MauvilleCity_BikeShop_Text_MachHandbookWhichPage
```
C'est un guide sur le VELO COURSE.\pLire quelle page?$
```
### MauvilleCity_BikeShop_Text_HowToRideMachBike
```
Le VELO prend la direction indiquée\nsur la manette +.\pIl accélère dès qu'il commence à rouler.\pQuand la manette + n'est plus utilisée,\nle VELO ralentit et s'arrête.\pLire une autre page?$
```
### MauvilleCity_BikeShop_Text_HowToTurnMachBike
```
Le VELO COURSE est rapide, mais il ne\ns'arrête pas immédiatement.\pC'est aussi un peu difficile de prendre\nles virages.\pIl faut relâcher la manette +\net ralentir un peu avant le virage.\pLire une autre page?$
```
### MauvilleCity_BikeShop_Text_SandySlopes
```
De petites coulées de sable parsèment\ntoute la région de HOENN.\pCe sable fin empêche d'avancer\nnormalement.\pMais avec un VELO COURSE, il est\npossible de gravir un versant de sable.\pLire une autre page?$
```
### MauvilleCity_BikeShop_Text_AcroHandbookWhichPage
```
C'est un guide sur le VELO CROSS.\pLire quelle page?$
```
### MauvilleCity_BikeShop_Text_Wheelies
```
Si vous appuyez sur le bouton B en\nroulant, la roue avant se soulève.\pVous pouvez rouler sur une roue\nen utilisant la manette +.\pCette technique se nomme\nla roue arrière.\pLire une autre page?$
```
### MauvilleCity_BikeShop_Text_BunnyHops
```
En maintenant le bouton B enfoncé,\nvous pourrez sauter sur des rebords.\pCette technique est appelée bunny hop.\pVous pouvez aussi avancer en sautant.\pLire une autre page?$
```
### MauvilleCity_BikeShop_Text_Jumps
```
Appuyez simultanément sur le bouton B\net sur la manette + pour faire un saut.\pAppuyez sur gauche ou droite sur la\nmanette + pour faire un saut latéral.\pAppuyez sur haut sur la manette + pour\nchanger de direction en sautant.\pVoir une autre page?$
```
