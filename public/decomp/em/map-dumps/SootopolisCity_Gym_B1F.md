...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_LASS` | 7,17 | `MOVEMENT_TYPE_FACE_RIGHT` | `SootopolisCity_Gym_B1F_EventScript_Andrea` | `0` |
| `` | `OBJ_EVENT_GFX_BEAUTY` | 9,19 | `MOVEMENT_TYPE_FACE_LEFT` | `SootopolisCity_Gym_B1F_EventScript_Connie` | `0` |
| `` | `OBJ_EVENT_GFX_WOMAN_2` | 12,7 | `MOVEMENT_TYPE_FACE_LEFT` | `SootopolisCity_Gym_B1F_EventScript_Brianna` | `0` |
| `` | `OBJ_EVENT_GFX_BEAUTY` | 11,9 | `MOVEMENT_TYPE_FACE_UP` | `SootopolisCity_Gym_B1F_EventScript_Bridget` | `0` |
| `` | `OBJ_EVENT_GFX_BEAUTY` | 5,6 | `MOVEMENT_TYPE_FACE_DOWN` | `SootopolisCity_Gym_B1F_EventScript_Tiffany` | `0` |
| `` | `OBJ_EVENT_GFX_POKEFAN_F` | 7,8 | `MOVEMENT_TYPE_FACE_RIGHT` | `SootopolisCity_Gym_B1F_EventScript_Bethany` | `0` |
| `` | `OBJ_EVENT_GFX_LASS` | 4,8 | `MOVEMENT_TYPE_FACE_RIGHT` | `SootopolisCity_Gym_B1F_EventScript_Crissy` | `0` |
| `` | `OBJ_EVENT_GFX_BEAUTY` | 9,7 | `MOVEMENT_TYPE_FACE_DOWN` | `SootopolisCity_Gym_B1F_EventScript_Olivia` | `0` |
| `` | `OBJ_EVENT_GFX_WOMAN_2` | 6,13 | `MOVEMENT_TYPE_FACE_RIGHT` | `SootopolisCity_Gym_B1F_EventScript_Daphne` | `0` |
| `` | `OBJ_EVENT_GFX_POKEFAN_F` | 10,13 | `MOVEMENT_TYPE_FACE_LEFT` | `SootopolisCity_Gym_B1F_EventScript_Annika` | `0` |

## Warps (1)
- #0 (11,22) → `MAP_SOOTOPOLIS_CITY_GYM_1F` warp #2
...
```
Tu vas voir les super techniques que\nnotre CHAMPION JUAN m'a apprises!$
```
### SootopolisCity_Gym_B1F_Text_AndreaDefeat
```
De grâce, pardonne-moi, JUAN…$
```
### SootopolisCity_Gym_B1F_Text_AndreaPostBattle
```
Regarde ce qui arrive si tu casses tous\nles carreaux du sol.$
```
### SootopolisCity_Gym_B1F_Text_CrissyIntro
```
Tu as fait tout le chemin jusqu'ici, mais\ntu n'auras pas l'occasion de voir JUAN.\pPas si tu perds contre moi en tout cas!$
```
### SootopolisCity_Gym_B1F_Text_CrissyDefeat
```
Quelle force tu as!\nTon air tout gentil est trompeur!$
```
### SootopolisCity_Gym_B1F_Text_CrissyPostBattle
```
Tu réussiras peut-être à ne pas te\nfaire démolir par JUAN en un seul coup.$
```
### SootopolisCity_Gym_B1F_Text_DaphneIntro
```
Voir JUAN mener un combat…\pRien que la beauté de ce spectacle \nm'a incitée à devenir DRESSEUR.$
```
### SootopolisCity_Gym_B1F_Text_DaphneDefeat
```
Tu as combattu avec plus de brio\nque je n'ai su le faire…$
```
### SootopolisCity_Gym_B1F_Text_DaphnePostBattle
```
L'élégance que tu dégages au combat\nest fabuleuse.\pOh… J'ai vraiment de la chance d'avoir\ntrouvé des POKéMON.$
```
### SootopolisCity_Gym_B1F_Text_ConnieIntro
```
Je devrais t'apprendre combien les\ncombats peuvent être rudes.$
```
...
```
Je vais te donner un conseil.\pSi tu veux rejoindre JUAN, tu dois\nfouler chaque carreau une seule fois.$
```
### SootopolisCity_Gym_B1F_Text_BridgetIntro
```
La meilleure ARENE POKéMON de la\nrégion de HOENN…\pC'est l'ARENE d'ATALANOPOLIS.$
```
### SootopolisCity_Gym_B1F_Text_BridgetDefeat
```
Quel bon niveau tu as!$
```
### SootopolisCity_Gym_B1F_Text_BridgetPostBattle
```
Plutôt que de se contenter d'être dans\nune ARENE puissante, j'imagine que\ls'entraîner ailleurs rend plus fort. Et\lça a surtout l'air plus sympa.$
```
### SootopolisCity_Gym_B1F_Text_OliviaIntro
```
Je m'entraîne avec JUAN.\nJe ne suis pas une adversaire facile!$
```
...
```
Je pense que tu as du potentiel. Tu\ndevrais rester t'entraîner avec nous.$
```
### SootopolisCity_Gym_B1F_Text_TiffanyIntro
```
Une élégante glissade sur la glace\nsans dépasser aucune ligne…\pUn DRESSEUR capable de cette prouesse\nserait d'une élégance folle!$
```
### SootopolisCity_Gym_B1F_Text_TiffanyDefeat
```
Eh bien, tu m'excuses?!$
```
### SootopolisCity_Gym_B1F_Text_TiffanyPostBattle
```
Il est évident que l'âge n'a rien à voir\navec la puissance que tu peux avoir\len tant que DRESSEUR.$
```
### SootopolisCity_Gym_B1F_Text_BethanyIntro
```
Quand je suis avec mes POKéMON,\nle temps défile à la vitesse de l'éclair.$
```
...
```
J'aimerais aussi oublier les défaites\nà la vitesse de l'éclair.$
```
### SootopolisCity_Gym_B1F_Text_AnnikaIntro
```
Si tu veux, je peux me battre avec des\nPOKéMON très rares.$
```
### SootopolisCity_Gym_B1F_Text_AnnikaDefeat
```
Oh! Tu as eu le temps de bien voir\nmes POKéMON?$
```
### SootopolisCity_Gym_B1F_Text_AnnikaPostBattle
```
Je suis venue dans cette ARENE\nparce que JUAN m'avait fait les\llouanges de mes POKéMON chéris.\pOh, si seulement je l'avais rencontré\nquand j'étais plus jeune…$
```
### SootopolisCity_Gym_B1F_Text_BriannaIntro
```
Hi hi…\nTon regard est vraiment charmant.$
```
### SootopolisCity_Gym_B1F_Text_BriannaDefeat
```
Eh bien. Je devais être trop sûre de\nma victoire.$
```
### SootopolisCity_Gym_B1F_Text_BriannaPostBattle
```
Tu ne pourras pas poser un doigt\nsur JUAN, j'en suis sûre!$
```
