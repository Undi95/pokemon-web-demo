...
|---|---|---|---|---|---|
| `LOCALID_FRONTIER_RECEPTION_GREETER` | `OBJ_EVENT_GFX_TEALA` | 0,11 | `MOVEMENT_TYPE_FACE_RIGHT` | `BattleFrontier_ReceptionGate_EventScript_Greeter` | `0` |
| `LOCALID_FRONTIER_RECEPTION_GUIDE` | `OBJ_EVENT_GFX_TEALA` | 8,11 | `MOVEMENT_TYPE_FACE_LEFT` | `BattleFrontier_ReceptionGate_EventScript_FacilityGuide` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_2` | 8,4 | `MOVEMENT_TYPE_FACE_LEFT` | `BattleFrontier_ReceptionGate_EventScript_RulesGuide` | `0` |
| `LOCALID_FRONTIER_RECEPTION_SCOTT` | `OBJ_EVENT_GFX_SCOTT` | 4,5 | `MOVEMENT_TYPE_FACE_DOWN` | `0x0` | `FLAG_HIDE_BATTLE_FRONTIER_RECEPTION_GATE_SCOTT` |
| `` | `OBJ_EVENT_GFX_MAN_2` | 0,4 | `MOVEMENT_TYPE_FACE_RIGHT` | `BattleFrontier_ReceptionGate_EventScript_FrontierPassGuide` | `0` |

## Warps (2)
- #0 (4,13) → `MAP_BATTLE_FRONTIER_OUTSIDE_WEST` warp #8
- #1 (4,1) → `MAP_BATTLE_FRONTIER_OUTSIDE_WEST` warp #9
...
```
Bienvenue à la ZONE DE COMBAT!\nLe summum des combats de POKéMON!$
```
### BattleFrontier_ReceptionGate_Text_IssueFrontierPass
```
Pour votre première visite, nous devons\nvous délivrer le PASSE ZONE.\pVous pourrez l'utiliser dans toute la\nZONE DE COMBAT.\pTenez!$
```
...
```
SCOTT: Je suis très heureux de te\nvoir ici!\pJ'espère que tu prendras le temps\nde tout visiter.\pNaturellement, j'espère aussi que tu\nprofiteras de l'essence même du combat.\pJ'ai également une maison ici,\nn'hésite pas à me rendre visite.$
```
### BattleFrontier_ReceptionGate_Text_YourGuideToFacilities
```
Je suis là pour vous guider parmi\nles installations de la ZONE DE COMBAT.$
```
### BattleFrontier_ReceptionGate_Text_LearnAboutWhich2
```
Sur quel bâtiment souhaitez-vous\navoir des informations?$
```
### BattleFrontier_ReceptionGate_Text_BattleTowerInfo
```
C'est la tour gigantesque considérée\ncomme le symbole de la ZONE DE COMBAT.\pIl y a quatre types de SALLES DE\nCOMBAT, pour les COMBATS SOLO, DUO,\lMULTI et MULTI LINK.$
```
### BattleFrontier_ReceptionGate_Text_BattleDomeInfo
```
Le DOME DE COMBAT est un grand\nbâtiment en forme d'œuf.\pDes TOURNOIS DE COMBAT y ont lieu.\pLes TOURNOIS DE COMBAT se jouent en\nCOMBATS SOLO ou DUO.$
```
### BattleFrontier_ReceptionGate_Text_BattlePalaceInfo
```
Le PALACE DE COMBAT est le bâtiment\nrouge à droite dans la ZONE DE COMBAT.\pIl contient deux types de HALLS DE\nCOMBAT, pour les COMBATS SOLO et DUO.$
```
### BattleFrontier_ReceptionGate_Text_BattleArenaInfo
```
Le DOJO DE COMBAT se trouve à la\ndroite du centre de la ZONE DE COMBAT.\pDes TOURNOIS K.O. y ont lieu.$
```
### BattleFrontier_ReceptionGate_Text_BattleFactoryInfo
```
L'USINE DE COMBAT est le grand bâtiment\ntout près d'ici.\pOn y propose le COMBAT ECHANGE.\pCe défi peut prendre la forme de\nCOMBATS SOLO ou de COMBATS DUO.$
```
### BattleFrontier_ReceptionGate_Text_BattlePikeInfo
```
Le REPTILE DE COMBAT est le bâtiment\nqui a la forme d'un POKéMON, à la\lgauche du centre de la ZONE DE COMBAT.\pL'évènement appelé COMBAT HASARD\ny prend place.$
```
### BattleFrontier_ReceptionGate_Text_BattlePyramidInfo
```
La PYRAMIDE DE COMBAT est l'énorme\npyramide.\pL'évènement appelé COMBAT QUETE\ny a lieu.$
```
### BattleFrontier_ReceptionGate_Text_RankingHallInfo
```
Le HALL DE CLASSEMENT se trouve près\nde la TOUR DE COMBAT.\pOn peut y consulter les records des\nmeilleurs DRESSEURS s'étant battus\ldans la ZONE DE COMBAT.$
```
### BattleFrontier_ReceptionGate_Text_ExchangeCornerInfo
```
Le SERVICE D'ECHANGE se trouve près\nde la TOUR DE COMBAT.\pVous pouvez y échanger les POINTS DE\nCOMBAT obtenus dans la ZONE DE\lCOMBAT contre des prix fabuleux.$
```
### BattleFrontier_ReceptionGate_Text_YourGuideToRules
```
Je peux vous expliquer les règles de\nbase communes aux défis des différents\lbâtiments de la ZONE DE COMBAT.$
```
### BattleFrontier_ReceptionGate_Text_LearnAboutWhat
```
Sur quel sujet désirez-vous obtenir\ndes informations?$
```
### BattleFrontier_ReceptionGate_Text_LevelModeInfo
```
Pour tous les défis de la ZONE DE\nCOMBAT, vous pouvez choisir entre\lniveau 50 et niveau libre.$
```
### BattleFrontier_ReceptionGate_Text_Level50Info
```
Niveau 50 est ouvert aux POKéMON\ndont le niveau est de 50 ou moins.\pCependant, aucun des DRESSEURS que\nvous affronterez n'aura de POKéMON\ldont le niveau est inférieur à 50.\pC'est le niveau le plus bas pour les\ncombats de la ZONE DE COMBAT.\pPour commencer, nous vous conseillons\ndonc de choisir niveau 50.$
```
### BattleFrontier_ReceptionGate_Text_OpenLevelInfo
```
Niveau libre ne fixe pas de limite au\nniveau des POKéMON participant à\lun défi.\pLe niveau de vos adversaires sera\najusté en fonction du niveau de vos\lPOKéMON.\pCependant, aucun des DRESSEURS que\nvous affronterez n'aura de POKéMON\ldont le niveau est inférieur à 60.$
```
### BattleFrontier_ReceptionGate_Text_MonEntryInfo
```
Presque tous les POKéMON peuvent\nrelever les défis de tous les bâtiments.\pLes OEUFS et certains POKéMON ne\nsont pas autorisés.\pLe nombre de POKéMON nécessaires pour\nparticiper varie selon l'endroit.\pCependant, il est impossible d'inscrire\ndeux POKéMON identiques.$
```
### BattleFrontier_ReceptionGate_Text_HoldItemsInfo
```
Lorsque vous relevez un défi de la ZONE\nDE COMBAT, il ne faut pas que deux de\lvos POKéMON portent le même objet.\pAssurez-vous que les POKéMON que vous\nvoulez inscrire à un défi portent tous\ldes objets différents.$
```
### BattleFrontier_ReceptionGate_Text_YourGuideToFrontierPass
```
Je peux tout vous expliquer sur le\nPASSE ZONE.$
```
...
```
Il y a sept bâtiments dans la ZONE DE\nCOMBAT où gagner des symboles.\pLes DRESSEURS qui ont du succès\npeuvent se voir récompensés par des\lsymboles.\pPour obtenir un symbole, un DRESSEUR\ndevra peut-être battre plusieurs\lDRESSEURS à la suite et remporter un\lévènement plusieurs fois.\pObtenir des symboles n'est pas facile.\nJe vous souhaite bonne chance!$
```
### BattleFrontier_ReceptionGate_Text_RecordedBattleInfo
```
Il est possible d'enregistrer un combat\nsur votre PASSE ZONE.\pVous pouvez par exemple enregistrer\nun combat contre un ou plusieurs amis.\pVous pouvez également enregistrer un\ncombat ayant eu lieu dans la ZONE DE\lCOMBAT, sauf au REPTILE DE\lCOMBAT et dans la PYRAMIDE DE COMBAT.\pVous pouvez décider d'enregistrer ou\nnon à la fin du combat.$
```
### BattleFrontier_ReceptionGate_Text_BattlePointsInfo
```
Les POINTS DE COMBAT récompensent\nles DRESSEURS qui font de beaux\lcombats dans la ZONE DE COMBAT.\pVous pouvez échanger vos POINTS DE\nCOMBAT contre des prix au SERVICE\lD'ECHANGE.$
```
