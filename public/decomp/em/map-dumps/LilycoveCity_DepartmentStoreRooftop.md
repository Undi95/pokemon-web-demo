...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_MAN_2` | 4,4 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_DepartmentStoreRooftop_EventScript_Man` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_3` | 7,5 | `MOVEMENT_TYPE_WANDER_LEFT_AND_RIGHT` | `LilycoveCity_DepartmentStoreRooftop_EventScript_ThirstyMan` | `0` |
| `` | `OBJ_EVENT_GFX_ROOFTOP_SALE_WOMAN` | 6,1 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_DepartmentStoreRooftop_EventScript_SaleWoman` | `FLAG_HIDE_LILYCOVE_DEPARTMENT_STORE_ROOFTOP_SALE_WOMAN` |
| `` | `OBJ_EVENT_GFX_WOMAN_3` | 15,5 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_DepartmentStoreRooftop_EventScript_SubstituteTutor` | `0` |

## Warps (1)
- #0 (13,3) → `MAP_LILYCOVE_CITY_DEPARTMENT_STORE_5F` warp #2

## BG events / signs (2)
- (9,1) [sign] → `LilycoveCity_DepartmentStoreRooftop_EventScript_VendingMachine`
- (10,1) [sign] → `LilycoveCity_DepartmentStoreRooftop_EventScript_VendingMachine`
...
```
Ils n'ont pas fixé de date pour le\ndéstockage massif?\pJe regarde la télé, mais je n'ai\npas vu une seule pub.$
```
### LilycoveCity_DepartmentStoreRooftop_Text_BeenWaitingForClearOutSale
```
Oui! Ça faisait longtemps que\nj'attendais ces soldes.\pIl y a des objets qu'on ne trouve qu'ici.\nMoi, je vais faire le plein!$
```
