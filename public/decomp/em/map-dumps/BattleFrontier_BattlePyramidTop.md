...
|---|---|---|---|---|---|
| `LOCALID_PYRAMID_TOP_ATTENDANT` | `OBJ_EVENT_GFX_HIKER` | 17,11 | `MOVEMENT_TYPE_FACE_DOWN` | `BattleFrontier_BattlePyramidTop_EventScript_Attendant` | `0` |
| `LOCALID_BRANDON` | `OBJ_EVENT_GFX_BRANDON` | 17,7 | `MOVEMENT_TYPE_FACE_UP` | `0x0` | `0` |

## Coord events / triggers (1)
- (17,9) → `BattleFrontier_BattlePyramidTop_EventScript_BattleBrandon` (si `VAR_TEMP_2` == `0`)
...
```
Ça me fait plaisir de vous revoir!\nVous avez atteint le sommet de la\lPYRAMIDE DE COMBAT!\pDe là-haut, la vue est formidable.\pC'est un endroit réservé aux DRESSEURS\nqui ont conquis la PYRAMIDE.\pJe vous en prie!\nAllez-y!$
```
### BattleFrontier_BattlePyramidTop_Text_PlayerConqueredPyramid
```
Le nouveau conquérant de la PYRAMIDE!\nVous entendrez parler de {PLAYER}!$
```
### BattleFrontier_BattlePyramidTop_Text_ChiefBeatYouHere
```
Ça me fait plaisir de vous revoir!\pMalheureusement, vous n'êtes que\ndeuxième!\pNotre chef a conquis la PYRAMIDE\navant vous.\pCependant…\nVous êtes un vrai DRESSEUR!\pVous pouvez vous imposer et\ndéposséder notre chef de sa victoire!\pJe vous en prie! Faites preuve d'encore\nun peu de courage!$
```
### BattleFrontier_BattlePyramidTop_Text_ExplorationsAreGrandestAdventure
```
Jeune nomade…\pPartir à la conquête du monde, c'est la\nplus belle des aventures, non?\pAvec pour seul bagage, son esprit…\pSa force!\pSes POKéMON!\pEt surtout, une bonne dose de bravoure\npour pouvoir affronter l'inconnu…$
```
### BattleFrontier_BattlePyramidTop_Text_ImPyramidKingBrandon
```
Oui, c'est une vie grandiose!\nOui, grandiose!\pJe suis BAYAR!\pJe suis le ROI PYRAMIDE! C'est moi qui\nm'occupe de cet endroit.\pTout le monde m'appelle le chef!\pSi tu es là, c'est que tu ne manques pas\nde confiance en toi. Je me trompe?\pAh, ah, ah!\nOn va bien s'amuser!$
```
### BattleFrontier_BattlePyramidTop_Text_BringCourageToOurBattle
```
C'est parti!\nTu vas devoir faire preuve de bravoure!$
```
### BattleFrontier_BattlePyramidTop_Text_BrandonFrontierPassPlease
```
BAYAR: Ah, ah, ah! Grandiose!\nAbsolument grandiose!\pBravo! On va entendre parler de toi!\nTon PASSE ZONE, s'il te plaît!$
```
### BattleFrontier_BattlePyramidTop_Text_ReceivedBraveSymbol
```
Le SYMBOLE BRAVOURE a été ajouté\nsur le PASSE ZONE!$
```
### BattleFrontier_BattlePyramidTop_Text_LookForwardToNextMeeting
```
J'ai hâte de te revoir, jeune\nnomade!$
```
### BattleFrontier_BattlePyramidTop_Text_BrandonYouveReturned
```
BAYAR: Tu es enfin de retour…\pTa soif d'aventure émane du plus\nprofond de ton cœur…\pOui… J'étais comme toi quand\nj'avais ton âge…\p… … … Ah…\pNous allons nous battre…\npassionnément!\pDangereusement!\pDésespérément!\pEn route vers l'aventure, au péril\nde nos vies…$
```
### BattleFrontier_BattlePyramidTop_Text_MyCourageIsOffMeter
```
Maintenant!\pJe déborde de bravoure!$
```
...
```
BAYAR: Ah, ah, ah!\nRemarquable!\lTout simplement grandiose!\pJeune nomade…\pTa bravoure surpasse la mienne!\nTiens, prends ça!$
```
### BattleFrontier_BattlePyramidTop_Text_BraveSymbolTookGoldenShine
```
Le SYMBOLE BRAVOURE a pris une couleur\ndorée!$
```
### BattleFrontier_BattlePyramidTop_Text_FarewellForNow
```
Je suppose que j'ai encore beaucoup\nà apprendre, finalement!\pJ'espère que nos routes se croiseront\nde nouveau, jeune nomade!$
```
### BattleFrontier_BattlePyramidTop_Text_StepForwardWhenReady
```
Dès que vous le souhaitez, avancez.\nFaites preuve de bravoure.$
```
