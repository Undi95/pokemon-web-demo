...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_GENTLEMAN` | 17,5 | `MOVEMENT_TYPE_FACE_LEFT` | `RustboroCity_DevonCorp_3F_EventScript_MrStone` | `0` |
| `LOCALID_DEVON_CORP_3F_EMPLOYEE` | `OBJ_EVENT_GFX_MAN_2` | 3,5 | `MOVEMENT_TYPE_FACE_RIGHT` | `RustboroCity_DevonCorp_3F_EventScript_Employee` | `FLAG_HIDE_RUSTBORO_CITY_DEVON_CORP_3F_EMPLOYEE` |
| `` | `OBJ_EVENT_GFX_GENTLEMAN` | 15,5 | `MOVEMENT_TYPE_INVISIBLE` | `RustboroCity_DevonCorp_3F_EventScript_MrStone` | `0` |

## Warps (1)
- #0 (2,1) → `MAP_RUSTBORO_CITY_DEVON_CORP_2F` warp #1

## BG events / signs (2)
- (1,5) [sign] → `RustboroCity_DevonCorp_3F_EventScript_RareRocksDisplay`
- (1,7) [sign] → `RustboroCity_DevonCorp_3F_EventScript_RareRocksDisplay`
...
```
Je suis M. ROCHARD, le DIRECTEUR\nde DEVON.\pOn vient de me parler de toi.\pTu as sauvé notre équipe pas une,\nmais deux fois!\pJ'ai un service à demander à\nquelqu'un d'aussi remarquable que toi.\pJe sais que tu dois livrer un paquet\nau CHANTIER NAVAL de POIVRESSEL.\pSur la route, pourrais-tu faire une\npetite halte au VILLAGE MYOKARA?\pJ'aimerais que tu remettes une LETTRE\nà PIERRE.$
```
### RustboroCity_DevonCorp_3F_Text_MrStoneWantYouToHaveThis
```
M. ROCHARD: Tu dois savoir que je suis\nun grand DIRECTEUR.\pJe ne suis donc pas avare au point\nde demander un service sans rien\len retour.\pAlors j'aimerais que tu prennes ça!$
```
### RustboroCity_DevonCorp_3F_Text_ReceivedPokenav
```
{PLAYER} reçoit un POKéNAV.$
```
### RustboroCity_DevonCorp_3F_Text_MrStoneExplainPokenavRestUp
```
M. ROCHARD: Cet appareil…\pC'est un NAVIGATEUR POKéMON ou\nPOKéNAV pour faire plus court.\pC'est un outil indispensable dans la\nquête de tout DRESSEUR.\pIl y a une carte de la région de HOENN.\pTu pourras facilement voir où se\nsituent MYOKARA et POIVRESSEL.\pA propos, j'ai entendu dire que des\ncriminels, MAGMA ou AQUA je crois,\pcauseraient des problèmes un\npeu partout.\pJe crois que tu ferais mieux de te\nreposer avant de reprendre la route.$
```
### RustboroCity_DevonCorp_3F_Text_MrStoneGoWithCautionAndCare
```
M. ROCHARD: Bien, {PLAYER}{KUN},\nvoyage prudemment!$
```
### RustboroCity_DevonCorp_3F_Text_CountingOnYou
```
M. ROCHARD: Je compte sur toi!$
```
### RustboroCity_DevonCorp_3F_Text_ThankYouForDeliveringLetter
```
M. ROCHARD: Tu as livré ma LETTRE?\nMerci beaucoup!\pC'est ma façon de te remercier. Ça\ndevrait t'aider, en tant que DRESSEUR.$
```
### RustboroCity_DevonCorp_3F_Text_ExplainExpShare
```
M. ROCHARD: Un POKéMON qui porte\nMULTI EXP se verra attribuer certains\ldes points EXP obtenus au combat.\pIl obtiendra des points EXP, même s'il\nn'a pas vraiment combattu.\pJe trouve le MULTI EXP très utile pour\nfaire progresser les POKéMON faibles.$
```
### RustboroCity_DevonCorp_3F_Text_NotFamiliarWithTrends
```
M. ROCHARD: Toute ma jeunesse, je me\nsuis plongé dans le travail.\pAlors tu sais, je ne connais pas trop\nles modes et ce genre de choses.\pMais, les jeunes désirent-ils tous être\nDRESSEURS à la LIGUE POKéMON?$
```
### RustboroCity_DevonCorp_3F_Text_ThisIs3rdFloorWaitHere
```
C'est le deuxième étage du siège\nde DEVON.\pLe BUREAU du DIRECTEUR s'y trouve.\pJe ne saurais dire combien je suis\nreconnaissant de ce que tu as fait.\pHum… Au fait, le paquet que tu nous \nas ramené…\pJe peux te demander de le livrer au\nCHANTIER NAVAL de POIVRESSEL?\pCe serait désastreux si ces voleurs\ntentaient de le reprendre.\pHum… Alors je peux compter sur toi\npour me rendre ce service?\pOh, c'est vrai!\nTu pourrais attendre ici un instant?$
```
### RustboroCity_DevonCorp_3F_Text_WordWithPresidentComeWithMe
```
Notre DIRECTEUR aimerait te dire\nun mot.\pSi tu veux bien me suivre…$
```
...
```
Si tu vas au CHANTIER NAVAL de\nPOIVRESSEL, tu devrais aller voir le\lCAPT. POUPE.$
```
### RustboroCity_DevonCorp_3F_Text_RepeatAndTimerHugelyPopular
```
Les nouvelles créations de DEVON,\nla BIS BALL et la CHRONO BALL, sont\ltrès appréciées des DRESSEURS.$
```
### RustboroCity_DevonCorp_3F_Text_RareRocksDisplay
```
C'est une collection de roches et de\npierres rares réunies par le DIRECTEUR.$
```
