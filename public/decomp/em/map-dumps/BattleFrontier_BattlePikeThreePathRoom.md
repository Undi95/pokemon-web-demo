...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_LINK_RECEPTIONIST` | 4,10 | `MOVEMENT_TYPE_FACE_RIGHT` | `BattleFrontier_BattlePikeThreePathRoom_EventScript_Attendant` | `0` |
| `LOCALID_PIKE_HINT_GIVER` | `OBJ_EVENT_GFX_LINK_RECEPTIONIST` | 8,5 | `MOVEMENT_TYPE_FACE_DOWN` | `BattleFrontier_BattlePikeThreePathRoom_EventScript_HintGiver` | `0` |

## Coord events / triggers (9)
- (2,3) → `BattleFrontier_BattlePikeThreePathRoom_EventScript_LeftRoomWarp` (si `VAR_TEMP_1` == `0`)
- (6,3) → `BattleFrontier_BattlePikeThreePathRoom_EventScript_CenterRoomWarp` (si `VAR_TEMP_1` == `0`)
- (10,3) → `BattleFrontier_BattlePikeThreePathRoom_EventScript_RightRoomWarp` (si `VAR_TEMP_1` == `0`)
- (5,10) → `BattleFrontier_BattlePikeThreePathRoom_EventScript_NoTurningBack` (si `VAR_TEMP_3` == `1`)
- (6,10) → `BattleFrontier_BattlePikeThreePathRoom_EventScript_NoTurningBack` (si `VAR_TEMP_3` == `1`)
- (6,9) → `BattleFrontier_BattlePikeThreePathRoom_EventScript_SetEnteredRoom` (si `VAR_TEMP_2` == `0`)
- (5,9) → `BattleFrontier_BattlePikeThreePathRoom_EventScript_SetEnteredRoom` (si `VAR_TEMP_2` == `0`)
- (7,9) → `BattleFrontier_BattlePikeThreePathRoom_EventScript_SetEnteredRoom` (si `VAR_TEMP_2` == `0`)
- (7,10) → `BattleFrontier_BattlePikeThreePathRoom_EventScript_NoTurningBack` (si `VAR_TEMP_3` == `1`)
...
```
Voulez-vous abandonner ce\ndéfi du COMBAT HASARD?$
```
### BattleFrontier_BattlePikeThreePathRoom_Text_AwaitingReturnSaveBeforeResume
```
Nous attendions votre retour…\pAvant de reprendre le COMBAT HASARD,\nje dois sauvegarder la partie…$
```
...
```
Pour tout vous dire, il y a comme une\nvague de nostalgie qui s'en dégage…$
```
### BattleFrontier_BattlePikeThreePathRoom_Text_TerrifyingEvent
```
Malheureusement…\pUne chose absolument horrible\nva s'abattre sur vous…\pJe vous conseille vivement de rester\nsur vos gardes et de faire attention…$
```
### BattleFrontier_BattlePikeThreePathRoom_Text_DreadfulPresence
```
Dans tous les chemins, je ressens une\nprésence angoissante…$
```
