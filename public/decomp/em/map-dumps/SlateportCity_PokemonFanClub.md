...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_MAN_1` | 11,4 | `MOVEMENT_TYPE_FACE_DOWN` | `SlateportCity_PokemonFanClub_EventScript_Man` | `0` |
| `` | `OBJ_EVENT_GFX_TWIN` | 1,5 | `MOVEMENT_TYPE_WANDER_UP_AND_DOWN` | `SlateportCity_PokemonFanClub_EventScript_Twin` | `0` |
| `` | `OBJ_EVENT_GFX_REPORTER_F` | 11,5 | `MOVEMENT_TYPE_FACE_UP` | `SlateportCity_PokemonFanClub_EventScript_Reporter` | `0` |
| `` | `OBJ_EVENT_GFX_WOMAN_4` | 6,2 | `MOVEMENT_TYPE_LOOK_AROUND` | `SlateportCity_PokemonFanClub_EventScript_SootheBellWoman` | `0` |
| `` | `OBJ_EVENT_GFX_GENTLEMAN` | 6,5 | `MOVEMENT_TYPE_FACE_DOWN` | `SlateportCity_PokemonFanClub_EventScript_Chairman` | `0` |
| `` | `OBJ_EVENT_GFX_ZIGZAGOON_2` | 3,9 | `MOVEMENT_TYPE_LOOK_AROUND` | `SlateportCity_PokemonFanClub_EventScript_Zigzagoon` | `0` |
| `` | `OBJ_EVENT_GFX_SKITTY` | 8,3 | `MOVEMENT_TYPE_LOOK_AROUND` | `SlateportCity_PokemonFanClub_EventScript_Skitty` | `0` |
| `` | `OBJ_EVENT_GFX_AZUMARILL` | 10,8 | `MOVEMENT_TYPE_LOOK_AROUND` | `SlateportCity_PokemonFanClub_EventScript_Azumarill` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_4` | 11,8 | `MOVEMENT_TYPE_FACE_LEFT` | `SlateportCity_PokemonFanClub_EventScript_SwaggerTutor` | `0` |

## Warps (2)
- #0 (6,10) → `MAP_SLATEPORT_CITY` warp #4
- #1 (7,10) → `MAP_SLATEPORT_CITY` warp #4
...
```
Heu! Je suis le PRESIDENT du\nFAN CLUB POKéMON!\pEn tant que PRESIDENT, je suis bien sûr\nla personne la plus importante!\pPersonne ne sait mieux que moi comment\ndresser un POKéMON. Personne!\pMais laisse-moi te parler des\nCONCOURS POKéMON.\pIl y a des évènements où l'on peut\nmontrer ses POKéMON à tout le monde.\pMais ils ont lieu loin d'ici, alors je ne\npeux pas y participer souvent.\pC'est pour cela que nous nous\nréunissons ici, pour montrer nos\lPOKéMON aux autres.$
```
### SlateportCity_PokemonFanClub_Text_LikeToSeeEnteredContestPokemon
```
Le POKéMON d'un DRESSEUR ayant\nparticipé à un CONCOURS POKéMON,\lvoilà ce que j'aimerais voir.$
```
### SlateportCity_PokemonFanClub_Text_AllowMeToExamineYourPokemon
```
Hum, je vois que tu as participé à\nun CONCOURS POKéMON!\pS'il te plaît, laisse-moi voir comme\ntu as dressé tes POKéMON.\pJ'adore voir comment les autres\ndressent leurs POKéMON.\pL'enthousiasme des POKéMON\nbien dressés…\pLes regards attentionnés et\naffectueux des DRESSEURS…\pMon cœur déborde. C'est un bonheur\nintense!\pOh, pardon! Ça fait déjà trop\nlongtemps que je parle!\pPourrais-je examiner tes POKéMON\npour voir s'ils progressent bien?$
```
### SlateportCity_PokemonFanClub_Text_HowIsYourPokemonGrowing
```
