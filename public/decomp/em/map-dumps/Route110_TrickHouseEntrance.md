...
|---|---|---|---|---|---|
| `LOCALID_TRICK_MASTER` | `OBJ_EVENT_GFX_MAN_1` | 6,2 | `MOVEMENT_TYPE_FACE_LEFT` | `Route110_TrickHouseEntrance_EventScript_TrickMaster` | `FLAG_HIDE_TRICK_HOUSE_ENTRANCE_MAN` |

## Warps (3)
- #0 (5,7) → `MAP_ROUTE110` warp #1
- #1 (6,7) → `MAP_ROUTE110` warp #1
- #2 (5,2) → `MAP_ROUTE110_TRICK_HOUSE_PUZZLE1` warp #0

## Coord events / triggers (4)
- (4,7) → `Route110_TrickHouseEntrance_EventScript_TrickMasterHiding` (si `VAR_TRICK_HOUSE_BEING_WATCHED_STATE` == `0`)
- (5,6) → `Route110_TrickHouseEntrance_EventScript_TrickMasterHiding` (si `VAR_TRICK_HOUSE_BEING_WATCHED_STATE` == `0`)
- (6,6) → `Route110_TrickHouseEntrance_EventScript_TrickMasterHiding` (si `VAR_TRICK_HOUSE_BEING_WATCHED_STATE` == `0`)
- (7,7) → `Route110_TrickHouseEntrance_EventScript_TrickMasterHiding` (si `VAR_TRICK_HOUSE_BEING_WATCHED_STATE` == `0`)

## BG events / signs (1)
- (5,1) [sign] → `Route110_TrickHouseEntrance_EventScript_Door`
...
```
Hein? Grrr…\pComment as-tu su que je me cachais\nsous ce coussin? Tu as l'œil!$
```
### Route110_TrickHouseEntrance_Text_TheyCallMeTrickMaster
```
Prends garde!\pCar je suis l'homme le plus mystérieux\nde toute la région de HOENN!\lOn m'appelle…\pLe MAITRE DES PIEGES!\nOuahahaha! Enchanté de te rencontrer!$
```
### Route110_TrickHouseEntrance_Text_ComeToChallengeTrickHouse
```
Toi, tu viens relever les défis de ma\nMAISON DES PIEGES, n'est-ce pas?\pC'est pour ça que tu es ici, hein?\nJ'en suis sûr!\pTrès bien! Tu peux participer!\pEntre par le parchemin et prépare-toi\nau plus grand des mystères!\pJe t'attendrai derrière!$
```
...
```
Il y a un trou derrière le parchemin!\pVoulez-vous entrer?$
```
### Route110_TrickHouseEntrance_Text_LeavingOnJourneyNote
```
Un mot est accroché au parchemin…\p“Je suis parti en voyage. Inutile de\nme chercher. LE MAITRE DES PIEGES”$
```
### Route110_TrickHouseEntrance_Text_NextTimeUseThisTrick
```
La prochaine fois, j'utiliserai ce truc,\ncette ruse aussi, et cette feinte…\pHéhéhéhé… Si je peux me permettre,\nça va être super dur, même pour moi!$
```
### Route110_TrickHouseEntrance_Text_InMidstOfDevisingNewChallenges
```
Hein? Quoi?!\nOh, c'est toi!\pJe suis en train de réfléchir à de\nnouveaux défis astucieux!\pTu pourrais quand même me laisser\nréfléchir encore un peu, tu ne\lcrois pas? C'est la moindre des choses!\lReviens me voir plus tard!$
```
### Route110_TrickHouseEntrance_Text_YoureHereToAcceptReward
```
Ah, c'est toi! Tu viens chercher\nla récompense de la dernière fois,\ln'est-ce pas? Je suis perspicace!\pTiens!\nJe vais te la donner!$
```
### Route110_TrickHouseEntrance_Text_DidYouNotComeToClaimReward
```
Hein? Tu ne viens pas chercher\nta récompense?$
```
### Route110_TrickHouseEntrance_Text_MechadollWhichTent
```
MECAPOUPEE 5 JE SUIS!\nSI RECOMPENSE PAS PRISE PAR TOI, ALORS\lMAITRE DES PIEGES PAS POUVOIR SUIVRE.\pTENTE ROUGE OU TENTE BLEUE,\nLAQUELLE TOI PREFERER?$
```
...
```
ETAT DU PC: ENCORE PLEIN.\nC'EST PAS SYMPA.$
```
### Route110_TrickHousePuzzle_Text_FoundAScroll
```
{PLAYER} a trouvé un parchemin.$
```
### Route110_TrickHousePuzzle_Text_MemorizedSecretCode
```
{PLAYER} a mémorisé le code secret écrit\nsur le parchemin.$
```
### Route110_TrickHousePuzzle_Text_SecretCodeWrittenOnIt
```
Un code secret est écrit dessus.$
```
### Route110_TrickHouseEntrance_Text_DoorLockedWriteSecretCodeHere
```
La porte est fermée.\p… En regardant de plus près, on\npeut lire:\l“Ecrire le code secret ici.”$
```
