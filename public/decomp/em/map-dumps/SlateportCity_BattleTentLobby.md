...
|---|---|---|---|---|---|
| `LOCALID_SLATEPORT_TENT_ATTENDANT` | `OBJ_EVENT_GFX_SCIENTIST_1` | 6,5 | `MOVEMENT_TYPE_FACE_DOWN` | `SlateportCity_BattleTentLobby_EventScript_Attendant` | `0` |
| `` | `OBJ_EVENT_GFX_SAILOR` | 1,5 | `MOVEMENT_TYPE_WANDER_AROUND` | `SlateportCity_BattleTentLobby_EventScript_TormentGiver` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_3` | 3,7 | `MOVEMENT_TYPE_FACE_UP` | `SlateportCity_BattleTentLobby_EventScript_Man` | `0` |
| `` | `OBJ_EVENT_GFX_GIRL_1` | 1,8 | `MOVEMENT_TYPE_LOOK_AROUND` | `SlateportCity_BattleTentLobby_EventScript_Girl` | `0` |
| `` | `OBJ_EVENT_GFX_WOMAN_2` | 11,8 | `MOVEMENT_TYPE_FACE_LEFT` | `SlateportCity_BattleTentLobby_EventScript_Woman` | `0` |

## Warps (2)
- #0 (6,9) → `MAP_SLATEPORT_CITY` warp #3
- #1 (7,9) → `MAP_SLATEPORT_CITY` warp #3

## BG events / signs (1)
- (4,5) [sign] → `SlateportCity_BattleTentLobby_EventScript_RulesBoard`
...
```
Tu vois, comme j'pouvais pas trouver\nde POKéMON pour moi, tu vois…\pAlors j'suis allé me plaindre à ce\ntype là, tu vois.\pEt il voulait même pas m'écouter, quoi!\nUn vrai pourri, quoi!\pJ'pouvais pas en placer une!\nAttends, prends ça, quoi!$
```
### SlateportCity_BattleTentLobby_Text_ExplainTorment
```
Ben quoi, c'est la CT41, tu vois?\nHé, c'est TOURMENTE, tu piges?\pAvec ça, l'autre pourra pas utiliser\ndeux fois de suite la même capacité, OK?\pHé, maintenant, tu m'écoutes, alors!\nC'est quand même pas la mort!$
```
### SlateportCity_BattleTentLobby_Text_IllTryUsingBugMons
```
J'aime pas vraiment les POKéMON\nINSECTE, mais je devrais peut-être\len essayer, juste pour voir.\pQui sait, j'pourrais peut-être même\naimer ça!$
```
### SlateportCity_BattleTentLobby_Text_BattleEvenWithoutToughMons
```
Tu peux te battre quand tu veux ici,\nmême si tu ne possèdes pas de\lPOKéMON.$
```
### SlateportCity_BattleTentLobby_Text_NiceIfMoreSelection
```
Ne serait-ce pas super s'ils avaient\nplus de choix?$
```
