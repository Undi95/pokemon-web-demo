...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_NINJA_BOY` | 1,3 | `MOVEMENT_TYPE_FACE_RIGHT` | `FortreeCity_House1_EventScript_Trader` | `0` |
| `` | `OBJ_EVENT_GFX_ZIGZAGOON_2` | 2,3 | `MOVEMENT_TYPE_FACE_LEFT` | `FortreeCity_House1_EventScript_Zigzagoon` | `0` |
| `` | `OBJ_EVENT_GFX_EXPERT_F` | 7,4 | `MOVEMENT_TYPE_LOOK_AROUND` | `FortreeCity_House1_EventScript_ExpertF` | `0` |

## Warps (2)
- #0 (3,5) → `MAP_FORTREE_CITY` warp #1
- #1 (4,5) → `MAP_FORTREE_CITY` warp #1
...
```
Aaaah, j'en veux un!\nIl me faut un {STR_VAR_1}!\pJe ferais n'importe quoi pour en\navoir un! C'est un vrai cri du cœur!\pMaintenant que tu es au courant,\ntu vas échanger ton {STR_VAR_1} \lcontre un {STR_VAR_2}, n'est-ce pas?$
```
### FortreeCity_House1_Text_MonYouTakeCare
```
Ouais, super!\p{STR_VAR_1}, bienvenue!\n{STR_VAR_2}, fais attention à toi!$
```
### FortreeCity_House1_Text_ThisIsntAMon
```
Ah non, je ne pense pas!\nCe n'est pas un {STR_VAR_1}.$
```
### FortreeCity_House1_Text_YouWontTradeMe
```
Non? Tu ne veux pas faire d'échange?\nMême après que je t'ai ouvert mon cœur?$
```
### FortreeCity_House1_Text_GoingToMakeVolbeatStrong
```
A partir de maintenant, je vais rendre\nce MUCIOLE très fort!\pJ'espère que tu feras la même chose\navec POSIPI!$
```
### FortreeCity_House1_Text_TradingMemoriesWithOthers
```
Echanger des POKéMON avec d'autres…\pC'est comme échanger ses propres\nsouvenirs avec quelqu'un d'autre.$
```
