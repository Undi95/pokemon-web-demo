...
|---|---|---|---|---|---|
| `LOCALID_TOWER_ATTENDANT_SINGLES` | `OBJ_EVENT_GFX_TEALA` | 6,5 | `MOVEMENT_TYPE_FACE_DOWN` | `BattleFrontier_BattleTowerLobby_EventScript_SinglesAttendant` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_4` | 23,5 | `MOVEMENT_TYPE_FACE_DOWN` | `BattleFrontier_BattleTowerLobby_EventScript_FeelingsMan` | `0` |
| `` | `OBJ_EVENT_GFX_WOMAN_5` | 17,8 | `MOVEMENT_TYPE_WANDER_AROUND` | `BattleFrontier_BattleTowerLobby_EventScript_Woman` | `0` |
| `` | `OBJ_EVENT_GFX_BOY_1` | 15,9 | `MOVEMENT_TYPE_WANDER_AROUND` | `BattleFrontier_BattleTowerLobby_EventScript_Boy` | `0` |
| `LOCALID_TOWER_LOBBY_REPORTER` | `OBJ_EVENT_GFX_REPORTER_F` | 11,9 | `MOVEMENT_TYPE_FACE_RIGHT` | `BattleFrontier_BattleTowerLobby_EventScript_Reporter` | `FLAG_HIDE_BATTLE_TOWER_REPORTER` |
| `LOCALID_APPRENTICE` | `OBJ_EVENT_GFX_VAR_0` | 1,5 | `MOVEMENT_TYPE_FACE_DOWN` | `BattleFrontier_BattleTowerLobby_EventScript_Apprentice` | `FLAG_HIDE_APPRENTICE` |
| `LOCALID_TOWER_ATTENDANT_DOUBLES` | `OBJ_EVENT_GFX_TEALA` | 10,5 | `MOVEMENT_TYPE_FACE_DOWN` | `BattleFrontier_BattleTowerLobby_EventScript_DoublesAttendant` | `0` |
| `LOCALID_TOWER_ATTENDANT_MULTIS` | `OBJ_EVENT_GFX_TEALA` | 14,5 | `MOVEMENT_TYPE_FACE_DOWN` | `BattleFrontier_BattleTowerLobby_EventScript_MultisAttendant` | `0` |
| `LOCALID_TOWER_ATTENDANT_LINK_MULTIS` | `OBJ_EVENT_GFX_TEALA` | 18,5 | `MOVEMENT_TYPE_FACE_DOWN` | `BattleFrontier_BattleTowerLobby_EventScript_LinkMultisAttendant` | `0` |

## Warps (3)
- #0 (12,9) → `MAP_BATTLE_FRONTIER_OUTSIDE_EAST` warp #0
- #1 (13,9) → `MAP_BATTLE_FRONTIER_OUTSIDE_EAST` warp #0
- #2 (6,1) → `MAP_BATTLE_FRONTIER_BATTLE_TOWER_BATTLE_ROOM` warp #0

## BG events / signs (5)
- (4,5) [sign] → `BattleFrontier_BattleTowerLobby_EventScript_ShowSinglesResults`
- (8,5) [sign] → `BattleFrontier_BattleTowerLobby_EventScript_ShowDoublesResults`
- (12,5) [sign] → `BattleFrontier_BattleTowerLobby_EventScript_ShowMultisResults`
- (16,5) [sign] → `BattleFrontier_BattleTowerLobby_EventScript_ShowLinkMultisResults`
- (2,1) [sign] → `BattleFrontier_BattleTowerLobby_EventScript_RulesBoard`
...
```
Félicitations!\nVous avez battu les sept DRESSEURS!\p$
```
...
```
Nous vous attendions!\p$
```
### BattleFrontier_BattleTowerLobby_Text_ProgressWillBeSaved
```
Vous devez sauvegarder avant d'entrer\ndans une SALLE DE COMBAT. Patientez.$
```
### BattleFrontier_BattleTowerLobby_Text_AboutToFace50thTrainer
```
Vous êtes sur le point d'affronter le\n50{SUPER_E} DRESSEUR.\pA partir de maintenant, quand vous\nbattrez sept DRESSEURS à la suite\pavec un POKéMON différent, il recevra\nun RUBAN commémoratif.\pBonne chance!$
```
### BattleFrontier_BattleTowerLobby_Text_HereAreSomeRibbons
```
Voici des RUBANS pour avoir battu\nsept DRESSEURS costauds à la suite.\p{PLAYER} reçoit des RUBANS!$
```
### BattleFrontier_BattleTowerLobby_Text_PutRibbonOnMons
```
{PLAYER} met les RUBANS sur les\nPOKéMON ayant relevé le défi.$
```
### BattleFrontier_BattleTowerLobby_Text_DescribeFeelingsAboutBattleTower
```
Bonjour, bonjour!\nVous avez un instant?\pPouvez-vous me dire ce que vous\nressentez quand vous vous préparez?\lOu ce que vous dites quand vous avez\lgagné ou perdu un combat?$
```
### BattleFrontier_BattleTowerLobby_Text_FeelWhatWhenYouBegin
```
Quel est votre sentiment quand vous\nvous préparez à un combat?$
```
### BattleFrontier_BattleTowerLobby_Text_FeelWhatWhenYouveWon
```
Que dites-vous quand vous avez\ngagné un combat?$
```
...
```
Vous ne voulez rien dire?\nVous n'êtes pas facile, vous.$
```
### BattleFrontier_BattleTowerLobby_Text_ChangedYourMind
```
Hein? Vous avez changé d'avis?\nVous m'avez l'air bien lunatique.$
```
### BattleFrontier_BattleTowerLobby_Text_ThatsHowYouFeel
```
Tiens, je ne m'attendais pas à ça.\nVoilà qui n'est pas ordinaire.\pMerci!$
```
### BattleFrontier_BattleTowerLobby_Text_WinsInRowRecorded
```
Le nombre de matchs gagnés\nsuccessivement est enregistré.\pSi je me faisais battre, ce serait\nembarrassant!$
```
### BattleFrontier_BattleTowerLobby_Text_CanLeaveUntilLossOrSevenWins
```
Une fois dans la TOUR DE COMBAT,\ntu ne peux pas sortir avant de perdre\lou de battre sept DRESSEURS à la suite.\pT'as intérêt à te motiver sérieusement\navant de décider de relever le défi.$
```
...
```
Je regrette!\pVous n'avez pas quatre POKéMON\naptes à participer.\pIl vous faut quatre POKéMON différents\nde niveau 50 ou moins pour participer.\pIls doivent aussi tenir des objets\ndifférents.\pLes OEUFS{STR_VAR_1} inaptes au combat.\pRevenez me voir quand vous aurez\nce qu'il faut.$
```
...
```
Je regrette!\pVous n'avez pas deux POKéMON\naptes à participer.\pIl vous faut deux POKéMON différents\nde niveau 50 ou moins pour participer.\pIls doivent aussi tenir des objets\ndifférents.\pLes OEUFS{STR_VAR_1} inaptes au combat.\pRevenez me voir quand vous aurez\nce qu'il faut.$
```
...
```
Bienvenue dans la TOUR DE COMBAT!\pIci, nous mettons à l'épreuve\nle talent des DRESSEURS!\pJe serai votre guide dans les\nSALLES DE COMBAT SOLO!$
```
### BattleFrontier_BattleTowerLobby_Text_TakeSinglesChallenge
```
Voulez-vous relever le défi d'une\nSALLE DE COMBAT SOLO?$
```
### BattleFrontier_BattleTowerLobby_Text_ExplainSinglesChallenge
```
Les SALLES DE COMBAT SOLO de la TOUR\nDE COMBAT sont consacrées aux COMBATS\lSOLO avec trois POKéMON.\pIl y a de nombreuses SALLES DE COMBAT\nSOLO dans la TOUR DE COMBAT.\pDans chaque SALLE DE COMBAT SOLO, sept\nDRESSEURS sont prêts à vous défier.\pBattez-les tous et vous recevrez des\nPOINTS DE COMBAT.\pVous pouvez aussi faire une pause\net sauvegarder pendant un défi.\pAssurez-vous de bien sauvegarder, ou\nvous ne pourrez pas reprendre le défi.$
```
...
```
Les SALLES DE COMBAT DUO de la TOUR\nDE COMBAT sont consacrées aux COMBATS\lDUO avec quatre POKéMON.\pIl y a de nombreuses SALLES DE COMBAT\nDUO dans la TOUR DE COMBAT.\pDans chaque SALLE DE COMBAT DUO, sept\nDRESSEURS sont prêts à vous défier.\pBattez-les tous et vous recevrez des\nPOINTS DE COMBAT.\pVous pouvez aussi faire une pause\net sauvegarder pendant un défi.\pAssurez-vous de bien sauvegarder, ou\nvous ne pourrez pas reprendre le défi.$
```
### BattleFrontier_BattleTowerLobby_Text_PleaseSelectFourMons
```
Veuillez choisir les quatre POKéMON que\nvous souhaitez inscrire.$
```
### BattleFrontier_BattleTowerLobby_Text_WelcomeMultiBattle
```
Bienvenue dans la TOUR DE COMBAT!\pIci, nous mettons à l'épreuve\nle talent des DRESSEURS!\pJe serai votre guide dans les\nSALLES DE COMBAT MULTI!$
```
...
```
Les SALLES DE COMBAT MULTI de la TOUR\nDE COMBAT sont consacrées aux COMBATS\lMULTI.\pPour les COMBATS MULTI, vous devez\nfaire équipe avec un DRESSEUR de la\lTOUR et avoir deux POKéMON chacun.\pDans le SALON DE COMBAT de la TOUR,\nvous pouvez rencontrer d'autres\lDRESSEURS.\pVous devez y trouver le DRESSEUR qui\nsera votre partenaire de COMBAT MULTI.\pVous devez ensuite vous rendre dans\nune SALLE DE COMBAT MULTI.\pDans chaque SALLE DE COMBAT MULTI,\nsept équipes sont prêtes à vous défier.\pBattez-les toutes et vous recevrez\ndes POINTS DE COMBAT.\pVous pouvez aussi faire une pause\net sauvegarder pendant un défi.\pAssurez-vous de bien sauvegarder, ou\nvous ne pourrez pas reprendre le défi.$
```
### BattleFrontier_BattleTowerLobby_Text_PleaseSelectTwoMons
```
Veuillez choisir les deux POKéMON que\nvous souhaitez inscrire.$
```
### BattleFrontier_BattleTowerLobby_Text_WelcomeLinkMultiBattle
```
Bienvenue dans la TOUR DE COMBAT!\pIci, nous mettons à l'épreuve\nle talent des DRESSEURS!\pJe serai votre guide dans les\nSALLES DE COMBAT MULTI LINK.$
```
### BattleFrontier_BattleTowerLobby_Text_TakeLinkMultisChallenge
```
Voulez-vous relever le défi d'une\nSALLE DE COMBAT MULTI LINK?$
```
### BattleFrontier_BattleTowerLobby_Text_ExplainLinkMultisChallenge
```
Les SALLES DE COMBAT MULTI LINK de la\nTOUR DE COMBAT sont consacrées aux\lCOMBATS MULTI LINK avec un ami.\pVous devez vous connecter avec un ami\nà l'aide d'adaptateurs sans fil ou d'un\lcâble Game Boy Advance Game Link.\pVous devez faire équipe avec votre ami\net inscrire deux POKéMON différents.\pIl y a de nombreuses SALLES DE COMBAT\nMULTI LINK dans la TOUR DE COMBAT.\pDans chaque SALLE DE COMBAT MULTI\nLINK, sept équipes sont prêtes à vous\ldéfier.\pBattez-les toutes et vous recevrez\ndes POINTS DE COMBAT.\pContrairement aux autres SALLES, vous\nne pouvez pas faire de pause dans une\lSALLE DE COMBAT MULTI LINK.\pUne fois le défi commencé, vous devez\nfaire sept COMBATS MULTI LINK à la\lsuite sans vous arrêter.$
```
...
```
Les SALLES DE COMBAT MULTI LINK sont\nprévues pour deux joueurs connectés.$
```
### BattleFrontier_BattleTowerLobby_Text_FriendAlsoSelectedMon
```
Votre partenaire a également choisi le\nPOKéMON {STR_VAR_1}.$
```
### BattleFrontier_BattleTowerLobby_Text_FriendAlsoSelectedMons
```
Votre partenaire a également choisi les\nPOKéMON {STR_VAR_1} et {STR_VAR_2}.$
```
### BattleFrontier_BattleTowerLobby_Text_FriendChoseDifferentLvlMode
```
Votre partenaire a choisi un niveau\ndifférent.$
```
...
```
Choisissez deux POKéMON autres que\nceux de votre partenaire et le même\lniveau puis inscrivez-vous de nouveau.$
```
### BattleFrontier_BattleTowerLobby_Text_SaveGameBeforeShowingIn
```
Vous devez sauvegarder avant d'aller\nplus loin. Veuillez patienter.$
```
### BattleFrontier_BattleTowerLobby_Text_CongratsDefeatedMaiden
```
Félicitations!\nVous avez battu l'AS DU SALON et\lbalayé sept DRESSEURS!$
```
### BattleFrontier_BattleTowerLobby_Text_AwardYouTheseBattlePoints
```
Votre talent incommensurable va être\nrécompensé par des POINTS DE COMBATS.$
```
### BattleFrontier_Text_ObtainedXBattlePoints
```
{PLAYER} reçoit {STR_VAR_1} POINT(S) DE COMBAT.$
```
### BattleFrontier_BattleTowerLobby_Text_RulesAreListed
```
Règles de la TOUR DE COMBAT.$
```
...
```
La TOUR DE COMBAT accueille quatre\ntypes de combats: COMBAT SOLO, COMBAT\lDUO, COMBAT MULTI et COMBAT MULTI\lLINK.\pIl y a des SALLES DE COMBAT différentes\npour chaque type de combat.\pPrésentez-vous au guichet offrant le\ntype de combat que vous voulez faire.$
```
### BattleFrontier_BattleTowerLobby_Text_ExplainMonRules
```
Selon la SALLE DE COMBAT que vous\nchoisissez, vous devez avoir un nombre\ldifférent de POKéMON.\pPour un COMBAT SOLO, il faut trois\nPOKéMON.\pIl en faut quatre pour les COMBATS DUO\net deux pour les modes MULTI.$
```
### BattleFrontier_BattleTowerLobby_Text_ExplainSalonRules
```
Pour les COMBATS MULTI, vous devez\nfaire équipe avec un partenaire à\lchoisir dans le SALON DE COMBAT.\pPour choisir le partenaire adéquat,\nexaminez ses POKéMON et leurs\lcapacités.\pAprès sept victoires à la suite, vous\npouvez changer de partenaire.$
```
### BattleFrontier_BattleTowerLobby_Text_ExplainMultiLinkRules
```
En COMBAT MULTI LINK, deux amis\npeuvent faire équipe.\pVous devez vous connecter avec des\nadaptateurs sans fil ou un câble\lGame Boy Advance Game Link.\pVous devez choisir deux POKéMON au\nguichet d'inscription.\pCes POKéMON doivent être différents de\nceux de votre partenaire.\pContrairement aux autres SALLES, vous\nne pouvez pas faire de pause dans une\lSALLE DE COMBAT MULTI LINK.$
```
