...
|---|---|---|---|---|---|
| `LOCALID_CAVE_OF_ORIGIN_WALLACE` | `OBJ_EVENT_GFX_WALLACE` | 9,13 | `MOVEMENT_TYPE_FACE_UP` | `CaveOfOrigin_B1F_EventScript_Wallace` | `FLAG_HIDE_CAVE_OF_ORIGIN_B1F_WALLACE` |

## Warps (1)
- #0 (9,3) → `MAP_CAVE_OF_ORIGIN_1F` warp #1
...
```
Ah, tu dois être {PLAYER}{KUN}.\nJ'ai entendu parler de tes exploits.\pJe m'appelle MARC.\pJ'étais autrefois le CHAMPION D'ARENE\nd'ATALANOPOLIS.\pMais j'ai dû confier la garde de l'ARENE\nà JUAN, mon mentor.\p… … … … … …\n… … … … … …\pGROUDON et KYOGRE, les deux POKéMON\nqui s'affrontent, sont considérés\lcomme des POKéMON ancestraux.\pMais il n'existe pas que deux\nPOKéMON ancestraux.\pIl y en a un autre ailleurs.\pQuelque part, se trouve un POKéMON\nancestral appelé RAYQUAZA.\pOn raconte que dans le passé, c'est\nRAYQUAZA qui avait calmé les deux\lcombattants.\pMalheureusement, je n'ai aucune idée\nsur l'endroit où peut bien se cacher\lRAYQUAZA…$
```
### CaveOfOrigin_B1F_Text_WhereIsRayquaza
```
MARC: {PLAYER}{KUN}, mais peut-être\nsais-tu, toi, où se trouve RAYQUAZA?\pSi tu le sais, je t'en prie, dis-le moi.$
```
### CaveOfOrigin_B1F_Text_ButWereInCaveOfOrigin
```
MARC: La GROTTE ORIGINE?\pMais c'est ici!\nCe n'est pas possible!\pS'il te plaît, essaie de réfléchir à\nl'endroit où RAYQUAZA pourrait\lbien se trouver.$
```
### CaveOfOrigin_B1F_Text_OldLadyDidntMentionThat
```
MARC: Le MONT MEMORIA?\pMais la vieille femme que j'ai rencontrée\nlà-bas n'y a fait aucune allusion…\pEt je doute vraiment qu'elle ait essayé\nde me cacher quoi que ce soit.\p{PLAYER}{KUN}, est-ce que tu pourrais y\nréfléchir plus sérieusement?$
```
### CaveOfOrigin_B1F_Text_CantYouRememberSomehow
```
MARC: Hein? Tu ne te souviens plus?\nHmm… C'est un problème…\pTu ne peux vraiment pas t'en souvenir?$
```
### CaveOfOrigin_B1F_Text_WellHeadToSkyPillar
```
MARC: Le PILIER CELESTE?\pC'est ça!\nIl doit se trouver au PILIER CELESTE!\p{PLAYER}{KUN}, il n'y a pas un instant à\nperdre! Rendons-nous de suite au\lPILIER CELESTE!$
```
