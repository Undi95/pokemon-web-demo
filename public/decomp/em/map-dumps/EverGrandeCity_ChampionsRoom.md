...
|---|---|---|---|---|---|
| `LOCALID_CHAMPIONS_ROOM_WALLACE` | `OBJ_EVENT_GFX_WALLACE` | 6,5 | `MOVEMENT_TYPE_FACE_DOWN` | `0x0` | `0` |
| `LOCALID_CHAMPIONS_ROOM_RIVAL` | `OBJ_EVENT_GFX_VAR_0` | 6,12 | `MOVEMENT_TYPE_FACE_UP` | `0x0` | `FLAG_HIDE_CHAMPIONS_ROOM_RIVAL` |
| `LOCALID_CHAMPIONS_ROOM_BIRCH` | `OBJ_EVENT_GFX_PROF_BIRCH` | 6,12 | `MOVEMENT_TYPE_FACE_UP` | `0x0` | `FLAG_HIDE_CHAMPIONS_ROOM_BIRCH` |

## Warps (2)
- #0 (6,12) → `MAP_EVER_GRANDE_CITY_HALL4` warp #1
- #1 (6,2) → `MAP_EVER_GRANDE_CITY_HALL_OF_FAME` warp #0
...
```
MARC: Bienvenue, {PLAYER}{KUN}.\pA propos de l'incident d'ATALANOPOLIS…\nRéussir à mettre un terme à cette\lcrise par tes propres moyens…\pOups!\pÇa ne serait pas juste de dire que\ntout le mérite te revient.\pTu as surmonté la situation grâce aux\nliens que tu as avec tes POKéMON.\pNous, DRESSEURS, prenons soin de nos\nPOKéMON en leur donnant des objets\let en leur apprenant des capacités.\pMais nous apprenons également\nénormément de nos POKéMON.\pEt c'est ici que tu peux tester le\nrésultat de tous tes efforts, face à\ld'autres DRESSEURS et leurs POKéMON.\pMaintenant!\pQui de nous deux va pouvoir se vanter\nd'avoir les meilleurs POKéMON d'HOENN?\pNous allons le savoir tout de suite!$
```
### EverGrandeCity_ChampionsRoom_Text_Defeat
```
Tu as réussi à me battre, moi, le\nMAITRE POKéMON…\pBeau travail.\nTu as combattu de façon exaspérante,\lmais cependant glorieuse et élégante!\pGloire à toi, {PLAYER}{KUN}!\pTu es un DRESSEUR POKéMON\nde grande classe!$
```
### EverGrandeCity_ChampionsRoom_Text_PostBattleSpeech
```
MARC: Les POKéMON que tu as\nenvoyés au combat…\pPar moments, ils dansaient comme\nune brise de printemps, à d'autres,\lils frappaient comme l'éclair.\pBien que de manière assurée, tu\nas su combattre élégamment!\pMême moi, je fus ébloui par tant\nde maestria!\pTu te trouves désormais au\nsommet de la LIGUE POKéMON.\pJe te proclame maintenant le\nnouveau…$
```
### EverGrandeCity_ChampionsRoom_Text_MayAdvice
```
FLORA: {PLAYER}{KUN}!\pVoici un petit conseil avant que tu\nn'affrontes le MAITRE…$
```
### EverGrandeCity_ChampionsRoom_Text_MayItsAlreadyOver
```
FLORA: Hum? Quoi, quoi, quoi?\p{PLAYER}{KUN}…\nIl se pourrait que…\pC'est déjà fait?$
```
### EverGrandeCity_ChampionsRoom_Text_BrendanAdvice
```
BRICE: {PLAYER}!\pTu veux un conseil avant d'affronter\nle MAITRE…?$
```
### EverGrandeCity_ChampionsRoom_Text_BrendanYouveWon
```
BRICE: {PLAYER}…\pSerais-tu en train de me dire…\nque tu as battu le MAITRE?$
```
### EverGrandeCity_ChampionsRoom_Text_BirchArriveRatePokedex
```
PROF. SEKO: Tu as vu?\nQu'est-ce que je t'avais dit, {RIVAL}?\pNe t'avais-je pas dit de ne pas te faire\nde soucis pour {PLAYER}{KUN}?\p… … … … … … … …\n{PLAYER}{KUN}, tu as enfin réussi!\pQuand j'ai su que tu avais battu ton\npropre père à l'ARENE de CLEMENTI,\lje me suis dit que tu avais une chance…\pMais réaliser que tu es maintenant\nle MAITRE!\pAh, oui!\pQu'est-il advenu de ton POKéDEX?\nLaisse-moi voir.$
```
### EverGrandeCity_ChampionsRoom_Text_BirchCongratulations
```
PROF. SEKO: Quoi qu'il en soit,\nfélicitations!\pEntre fièrement dans l'ultime pièce!$
```
### EverGrandeCity_ChampionsRoom_Text_WallaceComeWithMe
```
MARC: {PLAYER}{KUN}…\pJe vais donc terminer ma phrase.\pLe nouveau MAITRE!\pSuis-moi.$
```
### EverGrandeCity_ChampionsRoom_Text_WallaceWaitOutside
```
MARC: Je suis désolé, mais…\pA partir d'ici, seuls les DRESSEURS\ndevenus MAITRES peuvent entrer.\pTu vas devoir attendre à l'extérieur\navec le PROFESSEUR.$
```
### EverGrandeCity_ChampionsRoom_Text_MayCongratulations
```
FLORA: Mais… Mais…\p… … … … … … … …\n… … … … … … … …\pJe plaisante! Pas de problème!\nC'est le règlement!\p{PLAYER}{KUN}!\nSincères félicitations!$
```
### EverGrandeCity_ChampionsRoom_Text_BrendanCongratulations
```
BRICE: … … … … … …\n… … … … … … … …\pSi c'est le règlement…\p{PLAYER}, c'est bien!\nFélicitations!$
```
