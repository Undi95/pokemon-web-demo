...
|---|---|---|---|---|---|
| `LOCALID_PALACE_BATTLE_PLAYER` | `OBJ_EVENT_GFX_VAR_1` | 1,9 | `MOVEMENT_TYPE_FACE_UP` | `0x0` | `0` |
| `LOCALID_PALACE_BATTLE_OPPONENT` | `OBJ_EVENT_GFX_VAR_0` | 13,1 | `MOVEMENT_TYPE_FACE_DOWN` | `0x0` | `0` |
| `LOCALID_PALACE_BATTLE_ATTENDANT` | `OBJ_EVENT_GFX_EXPERT_M` | 1,4 | `MOVEMENT_TYPE_FACE_RIGHT` | `0x0` | `0` |
| `LOCALID_PALACE_DUSCLOPS` | `OBJ_EVENT_GFX_DUSCLOPS` | 13,1 | `MOVEMENT_TYPE_FACE_DOWN` | `0x0` | `0` |
| `LOCALID_PALACE_AZURILL` | `OBJ_EVENT_GFX_AZURILL` | 13,1 | `MOVEMENT_TYPE_FACE_DOWN` | `0x0` | `0` |

## Warps (2)
- #0 (0,9) → `MAP_BATTLE_FRONTIER_BATTLE_PALACE_CORRIDOR` warp #2
- #1 (1,9) → `MAP_BATTLE_FRONTIER_BATTLE_PALACE_CORRIDOR` warp #2
...
```
Voulez-vous abandonner votre défi?$
```
...
```
Et maintenant…\pVos victoires vous donnent le droit\nd'affronter le CAPT. PALACE…$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_ReadyForPalaceMaven
```
Le CAPT. PALACE est prêt.\nPouvons-nous commencer?$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_AnnounceArrivalOfSpenser
```
Veuillez accueillir comme il se doit\nle CAPT. PALACE, ESTEBAN!$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_SpenserFirstIntro
```
ESTEBAN: Je ne fais qu'un avec mes\nPOKéMON!\pMon cœur et celui de mes POKéMON\nbattent à l'unisson!\pDis-moi, DRESSEUR en herbe!\nCrois-tu en tes POKéMON?\pEst-ce que tu crois en eux, quoi\nqu'il arrive?\pLa confiance qui te lie à tes POKéMON\ndevra être à toute épreuve!$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_ProveYourBondWithMons
```
Montre-moi les liens qui t'unissent à\ntes POKéMON!$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_SpenserPostSilverBattle
```
ESTEBAN: Ahhhhh!\pJe me suis un peu trop vanté!\nDésolé pour ce petit numéro!\pAllez…\nDonne-moi ce que tu sais!$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_LetsSeeFrontierPass
```
Un bon DRESSEUR ne doit jamais perdre\nla face…\pAllez, ton PASSE ZONE!\nEt que ça saute!$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_ReceivedSpiritsSymbol
```
Le SYMBOLE ESPRIT a été ajouté sur le\nPASSE ZONE!$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_SpenserAwaitNextTime
```
ESTEBAN: Tu lis dans tes POKéMON comme\ndans un livre ouvert.\pJ'attendrai avec impatience l'instant\noù nous nous reverrons.$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_SpenserThisTimeWontHoldBack
```
ESTEBAN: Ahhhhh!!!\nTe voilà?\pTu as dû développer un lien\nunique avec tes POKéMON.\pBattons-nous!\pCette fois, je ne vais pas faire le\nmalin! Cette fois, je vais gagner!\pOn peut commencer?\nPrépare-toi à perdre!$
```
...
```
ESTEBAN: Ce fut une belle\ndémonstration! Mes POKéMON, même\ldéchaînés, n'ont pas battu les tiens.\pVotre esprit d'équipe est remarquable!\pAllez…\nTu me donnes ce que tu sais?$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_HurryWithFrontierPass
```
Un bon DRESSEUR se doit d'être\nrespectueux…\pAllez, grouille-toi!\nSors-moi ton PASSE ZONE!$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_SpiritsSymbolTookGoldenShine
```
Le SYMBOLE ESPRIT a pris une couleur\ndorée!$
```
### BattleFrontier_BattlePalaceBattleRoom_Text_SpenserComeSeeMeAgain
```
ESTEBAN: Ah, ah!\pReviens me voir quand tu veux!\nNous t'attendrons de pied ferme!$
```
