...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_WOMAN_2` | 12,7 | `MOVEMENT_TYPE_FACE_DOWN` | `MauvilleCity_GameCorner_EventScript_Woman2` | `0` |
| `` | `OBJ_EVENT_GFX_GENTLEMAN` | 16,5 | `MOVEMENT_TYPE_FACE_DOWN` | `MauvilleCity_GameCorner_EventScript_Gentleman` | `0` |
| `` | `OBJ_EVENT_GFX_BEAUTY` | 11,2 | `MOVEMENT_TYPE_FACE_DOWN` | `MauvilleCity_GameCorner_EventScript_CoinsClerk` | `0` |
| `` | `OBJ_EVENT_GFX_BEAUTY` | 14,2 | `MOVEMENT_TYPE_FACE_DOWN` | `MauvilleCity_GameCorner_EventScript_PrizeCornerDolls` | `0` |
| `` | `OBJ_EVENT_GFX_GIRL_2` | 7,3 | `MOVEMENT_TYPE_FACE_DOWN` | `MauvilleCity_GameCorner_EventScript_Girl` | `0` |
| `` | `OBJ_EVENT_GFX_POKEFAN_M` | 19,4 | `MOVEMENT_TYPE_FACE_DOWN` | `MauvilleCity_GameCorner_EventScript_PokefanM` | `0` |
| `` | `OBJ_EVENT_GFX_OLD_MAN` | 20,8 | `MOVEMENT_TYPE_FACE_LEFT` | `MauvilleCity_GameCorner_EventScript_OldMan` | `0` |
| `` | `OBJ_EVENT_GFX_COOK` | 6,8 | `MOVEMENT_TYPE_FACE_RIGHT` | `MauvilleCity_GameCorner_EventScript_Cook` | `0` |
| `` | `OBJ_EVENT_GFX_BEAUTY` | 13,2 | `MOVEMENT_TYPE_FACE_DOWN` | `MauvilleCity_GameCorner_EventScript_PrizeCornerTMs` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_3` | 1,7 | `MOVEMENT_TYPE_FACE_RIGHT` | `MauvilleCity_GameCorner_EventScript_Man` | `0` |
| `` | `OBJ_EVENT_GFX_MANIAC` | 9,8 | `MOVEMENT_TYPE_FACE_LEFT` | `MauvilleCity_GameCorner_EventScript_Maniac` | `0` |
| `` | `OBJ_EVENT_GFX_WOMAN_5` | 4,6 | `MOVEMENT_TYPE_FACE_LEFT` | `MauvilleCity_GameCorner_EventScript_Woman` | `0` |

## Warps (2)
- #0 (11,10) → `MAP_MAUVILLE_CITY` warp #5
- #1 (12,10) → `MAP_MAUVILLE_CITY` warp #5

## BG events / signs (24)
- (2,6) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine0`
- (2,8) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine1`
- (2,9) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine2`
- (3,7) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine3`
- (3,8) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine4`
- (3,9) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine5`
- (7,6) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine6`
- (7,7) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine7`
- (7,9) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine8`
- (8,6) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine9`
- (8,7) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine10`
- (8,9) [sign] → `MauvilleCity_GameCorner_EventScript_SlotMachine11`
- (14,6) [sign] → `Roulette_EventScript_Table1`
- (15,6) [sign] → `Roulette_EventScript_Table1`
- (14,7) [sign] → `Roulette_EventScript_Table1`
- (15,7) [sign] → `Roulette_EventScript_Table1`
- (14,8) [sign] → `Roulette_EventScript_Table1`
- (15,8) [sign] → `Roulette_EventScript_Table1`
- (18,6) [sign] → `Roulette_EventScript_Table2`
- (19,6) [sign] → `Roulette_EventScript_Table2`
- (18,7) [sign] → `Roulette_EventScript_Table2`
- (19,7) [sign] → `Roulette_EventScript_Table2`
- (18,8) [sign] → `Roulette_EventScript_Table2`
- (19,8) [sign] → `Roulette_EventScript_Table2`
...
```
C'est le CASINO DE LAVANDIA.$
```
### MauvilleCity_GameCorner_Text_NeedCoinCaseForCoins
```
Oui, vous vouliez quelques JETONS\npour jouer?\pMais vous n'avez pas de BOITE JETONS\npour les ranger.$
```
### MauvilleCity_GameCorner_Text_WereYouLookingForCoins
```
Seriez-vous à la recherche de JETONS?\pC'est 1000¥ les 50 JETONS.\nCombien en voulez-vous?$
```
...
```
Hum… Vous n'avez pas l'air d'avoir\nassez d'argent…$
```
### MauvilleCity_GameCorner_Text_CoinCaseIsFull
```
Oh!\nVotre BOITE JETONS est pleine.$
```
### MauvilleCity_GameCorner_Text_DontNeedCoinsThen
```
Alors vous n'avez pas besoin de JETONS?\nBonne chance pour la suite!$
```
### MauvilleCity_GameCorner_Text_ExchangeCoinsForPrizes
```
Bienvenue.\pVous pouvez échanger vos JETONS\ncontre les prix à gagner ici.$
```
### MauvilleCity_GameCorner_Text_WhichPrize
```
Quel prix souhaiteriez-vous recevoir?$
```
...
```
Je me suis trompée. J'ai deux fois\nla même POUPEE.\pTu en veux une?$
```
...
```
Hein?\nTu la veux, mais pas tout de suite?$
```
### MauvilleCity_GameCorner_Text_DontBeNegative
```
Oh, ne t'en fais pas!\nTu sais, tu peux la prendre.$
```
### MauvilleCity_GameCorner_Text_CantWinJackpot
```
Il y a un prix que j'aimerais bien avoir,\nmais je ne peux pas gagner le jackpot.$
```
### MauvilleCity_GameCorner_Text_NeedCoinCaseGoNextDoor
```
Hé, mon p'tit, si tu veux jouer ici,\nil te faut une BOITE JETONS.\pJe pense que la jeune femme de la\nmaison voisine en a une. Va la voir!$
```
### MauvilleCity_GameCorner_Text_LuckOnlyLastSoLongTakeCoins
```
La chance continue!\nC'est trop pour moi.\pTiens, prends des JETONS!$
```
### MauvilleCity_GameCorner_Text_MauvilleSomethingForEveryone
```
On a tous une bonne raison\nd'aimer LAVANDIA.\pLa mienne, c'est le CASINO.$
```
### MauvilleCity_GameCorner_Text_RouletteTablesDifferentRates
```
La mise n'est pas la même pour\ntoutes les tables de ROULETTE.\pVérifie tes JETONS avant de choisir\nune table de jeu.$
```
### MauvilleCity_GameCorner_Text_EasyToLoseTrackOfTime
```
On perd vite la notion du temps ici!\nJe ferais bien de retourner travailler.$
```
### MauvilleCity_GameCorner_Text_CoinsAreNeededToPlay
```
Il faut des JETONS pour jouer\nau CASINO.$
```
### MauvilleCity_GameCorner_Text_RouletteOnlyLuck
```
Cette ROULETTE…\nC'est très difficile.\pGagner ou perdre, tout est une question\nde chance.$
```
### MauvilleCity_GameCorner_Text_UpTo3CoinsCanBeUsed
```
Il est possible de mettre jusqu'à trois\nJETONS dans une MACHINE A SOUS.\pPlus tu mets de JETONS, plus tu as\nde chances de gagner le jackpot.$
```
### MauvilleCity_GameCorner_Text_DifficultToStopOn7
```
C'est très dur de la faire s'arrêter en\nplein sur le “7”.\pSi ça s'arrête sur “7”\npendant le jeu bonus du REEL TIME, tu\lgagnes des JETONS en plus.$
```
### MauvilleCity_GameCorner_Text_HeresSomeSlotsInfo
```
Voilà quelques petits trucs à savoir\nà propos des MACHINES A SOUS.\pPlus tu as d'éclairs, plus tu as de\nchances de tomber sur le REEL TIME.\pDans une partie avec un maximum de\n5 chances pour le REEL TIME…\pIl est possible d'avoir quatre bonus\net après, un big bonus.\pÇa fait un total de 660 JETONS,\nmais c'est très difficile de l'avoir.$
```
### MauvilleCity_GameCorner_Text_CantPlayWithNoCoinCase
```
Tu ne peux pas jouer si tu n'as\npas de BOITE JETONS.$
```
