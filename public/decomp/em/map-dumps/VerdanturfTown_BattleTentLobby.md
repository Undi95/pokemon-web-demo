...
|---|---|---|---|---|---|
| `LOCALID_VERDANTURF_TENT_ATTENDANT` | `OBJ_EVENT_GFX_EXPERT_M` | 6,5 | `MOVEMENT_TYPE_FACE_DOWN` | `VerdanturfTown_BattleTentLobby_EventScript_Attendant` | `0` |
| `` | `OBJ_EVENT_GFX_GIRL_1` | 0,5 | `MOVEMENT_TYPE_WANDER_AROUND` | `VerdanturfTown_BattleTentLobby_EventScript_AttractGiver` | `0` |
| `` | `OBJ_EVENT_GFX_BOY_3` | 9,7 | `MOVEMENT_TYPE_WANDER_AROUND` | `VerdanturfTown_BattleTentLobby_EventScript_Boy1` | `0` |
| `` | `OBJ_EVENT_GFX_BOY_2` | 1,8 | `MOVEMENT_TYPE_FACE_RIGHT` | `VerdanturfTown_BattleTentLobby_EventScript_Boy2` | `0` |
| `` | `OBJ_EVENT_GFX_SCOTT` | 12,6 | `MOVEMENT_TYPE_FACE_LEFT` | `VerdanturfTown_BattleTentLobby_EventScript_Scott` | `FLAG_HIDE_VERDANTURF_TOWN_SCOTT` |
| `` | `OBJ_EVENT_GFX_LITTLE_BOY` | 2,8 | `MOVEMENT_TYPE_FACE_LEFT` | `VerdanturfTown_BattleTentLobby_EventScript_LittleBoy` | `0` |

## Warps (2)
- #0 (6,9) → `MAP_VERDANTURF_TOWN` warp #0
- #1 (7,9) → `MAP_VERDANTURF_TOWN` warp #0

## BG events / signs (1)
- (4,5) [sign] → `VerdanturfTown_BattleTentLobby_EventScript_RulesBoard`
...
```
Si un POKéMON n'aime pas une attaque,\nil n'aura pas envie de l'utiliser.\pEt sa force n'a absolument rien à voir\nlà-dedans.\pPar exemple, un POKéMON GENTIL de\nnature n'appréciera certainement pas\lde blesser ses adversaires.\pSi un POKéMON n'est pas à la hauteur\nde son potentiel, c'est sûrement qu'il\lutilise une capacité qu'il n'aime pas.$
```
### VerdanturfTown_BattleTentLobby_Text_GentleMonsScaryIfAngry
```
La plupart du temps, ma grande sœur\nest très gentille. Mais quand elle\ls'énerve, elle fait peur!\pLes POKéMON gentils font sûrement peur\naussi quand ils s'énervent!$
```
### VerdanturfTown_BattleTentLobby_Text_AttractionRunsDeep
```
Oh, mon bel amour pour les POKéMON…\nJe suis touchée par cette ATTRACTION…\pOups! Je ne savais pas que tu étais là!\nBref, peu importe.\pTu veux cette CT pour tes POKéMON?$
```
### VerdanturfTown_BattleTentLobby_Text_AttractionMutual
```
Mes sentiments pour les POKéMON…\nOh, ils me plaisent tellement…\pIls se battent exactement comme j'en\nai envie!$
```
### VerdanturfTown_BattleTentLobby_Text_TaughtWhatKindsOfMoves
```
Quel genre de capacités as-tu\nenseigné à tes POKéMON?\pCe serait déjà un avantage certain\ns'ils savaient se soigner et se\lprotéger.$
```
### VerdanturfTown_BattleTentLobby_Text_ScottCanMeetToughTrainers
```
SCOTT: Hé, {PLAYER}{KUN}!\nJe pensais bien te trouver ici.\pDans une TENTE DE COMBAT, on\nrencontre souvent de bons DRESSEURS.\pEt ce, quelles que soient les règles!\p{PLAYER}{KUN}, ne me déçois pas!\nDonne tout ce que tu as!$
```
### VerdanturfTown_BattleTentLobby_Text_ScottVisitRegularly
```
SCOTT: Je viens souvent ici.\nJ'espère toujours y rencontrer des\lDRESSEURS forts en pleine action.$
```
