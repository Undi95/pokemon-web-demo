# LilycoveCity_ContestHall

## Métadonnées
- **id** : `MAP_LILYCOVE_CITY_CONTEST_HALL`
- **layout** : `LAYOUT_LILYCOVE_CITY_CONTEST_HALL`
- **music** : `MUS_CONTEST_LOBBY`
- **region_map_section** : `MAPSEC_LILYCOVE_CITY`
- **weather** : `WEATHER_NONE`
- **map_type** : `MAP_TYPE_INDOOR`
- **battle_scene** : `MAP_BATTLE_SCENE_NORMAL`
- **show_map_name** : `False`
- **allow_cycling** : `False`
- **allow_running** : `False`

## Object events (32 NPCs)
| local_id | gfx | x,y | mvmt | script | flag |
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_BOY_1` | 30,30 | `MOVEMENT_TYPE_WANDER_AROUND` | `LilycoveCity_ContestHall_EventScript_Boy1` | `0` |
| `` | `OBJ_EVENT_GFX_BOY_3` | 11,7 | `MOVEMENT_TYPE_WANDER_AROUND` | `LilycoveCity_ContestHall_EventScript_Boy2` | `0` |
| `` | `OBJ_EVENT_GFX_GIRL_3` | 40,8 | `MOVEMENT_TYPE_WANDER_AROUND` | `LilycoveCity_ContestHall_EventScript_Girl` | `0` |
| `LOCALID_SMART_MC` | `OBJ_EVENT_GFX_WOMAN_3` | 12,22 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_ContestHall_EventScript_SmartContestMC` | `0` |
| `LOCALID_SMART_JUDGE` | `OBJ_EVENT_GFX_CONTEST_JUDGE` | 15,22 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_ContestHall_EventScript_SmartContestJudge` | `0` |
| `LOCALID_SMART_CONTESTANT_1` | `OBJ_EVENT_GFX_BLACK_BELT` | 10,25 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_SmartContestant1` | `0` |
| `LOCALID_SMART_CONTESTANT_2` | `OBJ_EVENT_GFX_EXPERT_M` | 12,25 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_SmartContestant2` | `0` |
| `LOCALID_SMART_CONTESTANT_3` | `OBJ_EVENT_GFX_MANIAC` | 14,25 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_SmartContestant3` | `0` |
| `LOCALID_SMART_CONTESTANT_4` | `OBJ_EVENT_GFX_LINK_RECEPTIONIST` | 16,25 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_SmartContestant4` | `0` |
| `` | `OBJ_EVENT_GFX_EXPERT_F` | 8,24 | `MOVEMENT_TYPE_FACE_RIGHT` | `LilycoveCity_ContestHall_EventScript_SmartContestAudience1` | `0` |
| `LOCALID_SMART_AUDIENCE_4` | `OBJ_EVENT_GFX_WOMAN_5` | 18,24 | `MOVEMENT_TYPE_FACE_LEFT` | `LilycoveCity_ContestHall_EventScript_SmartContestAudience4` | `0` |
| `LOCALID_SMART_AUDIENCE_2` | `OBJ_EVENT_GFX_BOY_1` | 8,22 | `MOVEMENT_TYPE_FACE_RIGHT` | `LilycoveCity_ContestHall_EventScript_SmartContestAudience2` | `0` |
| `LOCALID_BEAUTY_MC` | `OBJ_EVENT_GFX_WOMAN_3` | 24,10 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_ContestHall_EventScript_BeautyContestMC` | `0` |
| `LOCALID_BEAUTY_JUDGE` | `OBJ_EVENT_GFX_CONTEST_JUDGE` | 27,10 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_ContestHall_EventScript_BeautyContestJudge` | `0` |
| `LOCALID_BEAUTY_CONTESTANT_1` | `OBJ_EVENT_GFX_WOMAN_4` | 22,13 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_BeautyContestant1` | `0` |
| `LOCALID_BEAUTY_CONTESTANT_2` | `OBJ_EVENT_GFX_TWIN` | 24,13 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_BeautyContestant2` | `0` |
| `LOCALID_BEAUTY_CONTESTANT_3` | `OBJ_EVENT_GFX_PSYCHIC_M` | 26,13 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_BeautyContestant3` | `0` |
| `LOCALID_BEAUTY_CONTESTANT_4` | `OBJ_EVENT_GFX_POKEFAN_F` | 28,13 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_BeautyContestant4` | `0` |
| `LOCALID_BEAUTY_AUDIENCE_1` | `OBJ_EVENT_GFX_WOMAN_2` | 20,10 | `MOVEMENT_TYPE_FACE_RIGHT` | `LilycoveCity_ContestHall_EventScript_BeautyContestAudience1` | `0` |
| `LOCALID_BEAUTY_AUDIENCE_3` | `OBJ_EVENT_GFX_WOMAN_1` | 30,11 | `MOVEMENT_TYPE_FACE_LEFT` | `LilycoveCity_ContestHall_EventScript_BeautyContestAudience3` | `0` |
| `LOCALID_BEAUTY_AUDIENCE_2` | `OBJ_EVENT_GFX_MAN_1` | 26,8 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_ContestHall_EventScript_BeautyContestAudience2` | `0` |
| `LOCALID_SMART_AUDIENCE_3` | `OBJ_EVENT_GFX_CAMPER` | 13,20 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_ContestHall_EventScript_SmartContestAudience3` | `0` |
| `LOCALID_CUTE_MC` | `OBJ_EVENT_GFX_WOMAN_3` | 36,22 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_ContestHall_EventScript_CuteContestMC` | `0` |
| `LOCALID_CUTE_JUDGE` | `OBJ_EVENT_GFX_CONTEST_JUDGE` | 39,22 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_ContestHall_EventScript_CuteContestJudge` | `0` |
| `LOCALID_CUTE_CONTESTANT_1` | `OBJ_EVENT_GFX_BOY_2` | 34,25 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_CuteContestant1` | `0` |
| `LOCALID_CUTE_CONTESTANT_2` | `OBJ_EVENT_GFX_GIRL_2` | 36,25 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_CuteContestant2` | `0` |
| `LOCALID_CUTE_CONTESTANT_3` | `OBJ_EVENT_GFX_BOY_3` | 38,25 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_CuteContestant3` | `0` |
| `LOCALID_CUTE_CONTESTANT_4` | `OBJ_EVENT_GFX_EXPERT_F` | 40,25 | `MOVEMENT_TYPE_FACE_UP` | `LilycoveCity_ContestHall_EventScript_CuteContestant4` | `0` |
| `LOCALID_CUTE_AUDIENCE_1` | `OBJ_EVENT_GFX_WOMAN_2` | 32,23 | `MOVEMENT_TYPE_FACE_RIGHT` | `LilycoveCity_ContestHall_EventScript_CuteContestAudience1` | `0` |
| `LOCALID_CUTE_AUDIENCE_3` | `OBJ_EVENT_GFX_MAN_1` | 37,20 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_ContestHall_EventScript_CuteContestAudience3` | `0` |
| `LOCALID_CUTE_AUDIENCE_2` | `OBJ_EVENT_GFX_LITTLE_GIRL` | 42,24 | `MOVEMENT_TYPE_FACE_LEFT` | `LilycoveCity_ContestHall_EventScript_CuteContestAudience2` | `0` |
| `` | `OBJ_EVENT_GFX_SAILOR` | 23,26 | `MOVEMENT_TYPE_WANDER_AROUND` | `LilycoveCity_ContestHall_EventScript_Sailor` | `0` |

## Warps (4)
- #0 (19,32) → `MAP_LILYCOVE_CITY_CONTEST_LOBBY` warp #2
- #1 (30,32) → `MAP_LILYCOVE_CITY_CONTEST_LOBBY` warp #3
- #2 (20,32) → `MAP_LILYCOVE_CITY_CONTEST_LOBBY` warp #2
- #3 (31,32) → `MAP_LILYCOVE_CITY_CONTEST_LOBBY` warp #3

## BG events / signs (6)
- (27,16) [sign] → `LilycoveCity_ContestHall_EventScript_BeautyStageSign`
- (39,28) [sign] → `LilycoveCity_ContestHall_EventScript_CuteStageSign`
- (11,28) [sign] → `LilycoveCity_ContestHall_EventScript_SmartStageSign`
- (15,28) [sign] → `LilycoveCity_ContestHall_EventScript_SmartStageSign`
- (23,16) [sign] → `LilycoveCity_ContestHall_EventScript_BeautyStageSign`
- (35,28) [sign] → `LilycoveCity_ContestHall_EventScript_CuteStageSign`

## Scripts (35)
### LilycoveCity_ContestHall_EventScript_Boy1
```
msgbox LilycoveCity_ContestHall_Text_TodayWonSmartnessContest, MSGBOX_NPC
end
```
### LilycoveCity_ContestHall_EventScript_Boy2
```
msgbox LilycoveCity_ContestHall_Text_EnteredBunchOfContests, MSGBOX_NPC
end
```
### LilycoveCity_ContestHall_EventScript_Girl
```
msgbox LilycoveCity_ContestHall_Text_ManWhoWonEarlierHadPokeblocks, MSGBOX_NPC
end
```
### LilycoveCity_ContestHall_EventScript_Sailor
```
msgbox LilycoveCity_ContestHall_Text_IsntThisPlaceHumongous, MSGBOX_NPC
end
```
### LilycoveCity_ContestHall_EventScript_SmartContestMC
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_GiveItBestSmartAppeal, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_SMART_MC, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_SmartContestJudge
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_AreYouEnjoyingThisContest, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_SMART_JUDGE, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_SmartContestant1
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_EnteredWrongContest, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_SMART_CONTESTANT_1, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_SmartContestant2
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_RaisedMonToBeSmart, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_SMART_CONTESTANT_2, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_SmartContestant3
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_IfMonPullsSmartMoveNext, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_SMART_CONTESTANT_3, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_SmartContestant4
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_DontAppreciateCuteLeechLife, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_SMART_CONTESTANT_4, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_SmartContestAudience1
```
msgbox LilycoveCity_ContestHall_Text_YoureBeautifulGrandpa, MSGBOX_SIGN
end
```
### LilycoveCity_ContestHall_EventScript_SmartContestAudience2
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_AllSeemToUseDifferentMoves, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_SMART_AUDIENCE_2, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_SmartContestAudience3
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_PokemonSmarterThanTrainers, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_SMART_AUDIENCE_3, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_SmartContestAudience4
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_StillLoveSmartnessContests, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_SMART_AUDIENCE_4, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_BeautyContestMC
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_AreYouEnteringBeautyContest, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_BEAUTY_MC, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_BeautyContestJudge
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_EveryPokemonPristineBeauty, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_BEAUTY_JUDGE, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_BeautyContestant1
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_EyesWillBeGluedToMyBeauty, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_BEAUTY_CONTESTANT_1, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_BeautyContestant2
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_OverdidGrooming, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_BEAUTY_CONTESTANT_2, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_BeautyContestant3
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_JudgeWontSeeAuroraBeam, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_BEAUTY_CONTESTANT_3, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_BeautyContestant4
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_PokemonLooksLikeYoungerMe, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_BEAUTY_CONTESTANT_4, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_BeautyContestAudience1
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_WinBeautyContestMakesMeHappy, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_BEAUTY_AUDIENCE_1, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_BeautyContestAudience2
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_GanderAtAllThosePrettyPokemon, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_BEAUTY_AUDIENCE_2, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_BeautyContestAudience3
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_CantWinOnBeautyAlone, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_BEAUTY_AUDIENCE_3, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_CuteContestMC
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_InTheMiddleOfContest, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_CUTE_MC, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_CuteContestJudge
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_SuchCharmingCuteAppeals, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_CUTE_JUDGE, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_CuteContestant1
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_MyAzurillWasDistracted, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_CUTE_CONTESTANT_1, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_CuteContestant2
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_NeverWonBattleButContest, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_CUTE_CONTESTANT_2, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_CuteContestant3
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_PetalDanceIsMarvel, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_CUTE_CONTESTANT_3, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_CuteContestant4
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_MyMonAppealSoMuchCuter, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_CUTE_CONTESTANT_4, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_CuteContestAudience1
```
lockall
applymovement LOCALID_CUTE_AUDIENCE_1, Common_Movement_FacePlayer
waitmovement 0
msgbox LilycoveCity_ContestHall_Text_MyChildIsInContest, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_CUTE_AUDIENCE_1, Common_Movement_FaceOriginalDirection
waitmovement 0
delay 25
msgbox LilycoveCity_ContestHall_Text_ComeOnDear, MSGBOX_DEFAULT
releaseall
end
```
### LilycoveCity_ContestHall_EventScript_CuteContestAudience2
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_WantCuteMonOfMyOwn, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_CUTE_AUDIENCE_2, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_CuteContestAudience3
```
lock
faceplayer
msgbox LilycoveCity_ContestHall_Text_ThatGirlThereIsCutest, MSGBOX_DEFAULT
closemessage
applymovement LOCALID_CUTE_AUDIENCE_3, Common_Movement_FaceOriginalDirection
waitmovement 0
release
end
```
### LilycoveCity_ContestHall_EventScript_BeautyStageSign
```
msgbox LilycoveCity_ContestHall_Text_BeautyContestStage, MSGBOX_SIGN
end
```
### LilycoveCity_ContestHall_EventScript_CuteStageSign
```
msgbox LilycoveCity_ContestHall_Text_CuteContestStage, MSGBOX_SIGN
end
```
### LilycoveCity_ContestHall_EventScript_SmartStageSign
```
msgbox LilycoveCity_ContestHall_Text_SmartContestStage, MSGBOX_SIGN
end
```

## Textes (36)
### LilycoveCity_ContestHall_Text_TodayWonSmartnessContest
```
Ce POKéMON a déjà gagné le CONCOURS\nDE BEAUTE ici.\pEt tu sais quoi? Je lui ai fait gagner le\nCONCOURS D'INTELLIGENCE, aujourd'hui!\pJe m'effraie moi-même…$
```
### LilycoveCity_ContestHall_Text_EnteredBunchOfContests
```
J'ai fait pas mal de CONCOURS,\ndonc je sais comment ça se passe.\pDans un CONCOURS DE SANG-FROID, les\ncapacités SANG-FROID ont du succès.\pMais les capacités INTELLIGENCE et\nGRACE, par exemple, n'ont pas trop\ld'effet.$
```
### LilycoveCity_ContestHall_Text_ManWhoWonEarlierHadPokeblocks
```
Le jeune garçon qui vient de gagner\navait plein de {POKEBLOCK}S différents.\pTu gagnerais, toi, si t'en avais autant?$
```
### LilycoveCity_ContestHall_Text_IsntThisPlaceHumongous
```
Waouh, cet endroit est super!\pOn peut sentir la tension dans l'air…\nÇa, c'est un vrai lieu de CONCOURS.$
```
### LilycoveCity_ContestHall_Text_GiveItBestSmartAppeal
```
ANIMATRICE: POKéMON INTELLIGENTS\net leurs DRESSEURS, vous êtes prêts?!\pMontrez-vous sous votre meilleur jour!\nC'est parti!$
```
### LilycoveCity_ContestHall_Text_AreYouEnjoyingThisContest
```
JUGE: Vous aimez ce genre de CONCOURS?\pRevenez avec trois amis et vous\npourrez tous participer à un CONCOURS!$
```
### LilycoveCity_ContestHall_Text_EnteredWrongContest
```
Aïe, aïe, aïe…\nJe me suis trompé de CONCOURS.\pJ'ai inscrit ce POKéMON ROBUSTE à un\nCONCOURS D'INTELLIGENCE…\pAllez, montre-leur ton ECLATE-ROC.$
```
### LilycoveCity_ContestHall_Text_RaisedMonToBeSmart
```
Cela m'a pris des années et beaucoup\nde sagesse, pour que ce POKéMON\ldevienne aussi intelligent.\pIl ne se laisse pas battre par n'importe\nquel POKéMON inexpérimenté.$
```
### LilycoveCity_ContestHall_Text_IfMonPullsSmartMoveNext
```
Voilà!\pUne capacité INTELLIGENCE maintenant,\net l'enthousiasme du public augmentera!$
```
### LilycoveCity_ContestHall_Text_DontAppreciateCuteLeechLife
```
Oh, non!\pJ'utilise tout le temps le VAMPIRISME\nde mon NOSFERAPTI, il est si gracieux!\pMais ces gens sont méchants et\nn'apprécient pas du tout!$
```
### LilycoveCity_ContestHall_Text_YoureBeautifulGrandpa
```
Ouais! T'es super, pépé!$
```
### LilycoveCity_ContestHall_Text_AllSeemToUseDifferentMoves
```
Même quand les DRESSEURS inscrivent\nle même genre de POKéMON, ils semblent\lutiliser des capacités différentes.\pC'est comme pour les combats, chacun\na son propre style.$
```
### LilycoveCity_ContestHall_Text_PokemonSmarterThanTrainers
```
Je trouve que les POKéMON ont l'air bien\nplus intelligents que leurs DRESSEURS.$
```
### LilycoveCity_ContestHall_Text_StillLoveSmartnessContests
```
Par-dessus tout, j'adore les\nCONCOURS D'INTELLIGENCE.\pCette couleur verte si intellectuelle…\nC'est… super cool.$
```
### LilycoveCity_ContestHall_Text_AreYouEnteringBeautyContest
```
ANIMATRICE: Tu vas aussi participer au\nCONCOURS DE BEAUTE? Bonne chance!$
```
### LilycoveCity_ContestHall_Text_EveryPokemonPristineBeauty
```
JUGE: Ah, les POKéMON ici présents\nsont des spécimens d'une rare beauté!\pOh, j'en ai presque oublié de les noter!$
```
### LilycoveCity_ContestHall_Text_EyesWillBeGluedToMyBeauty
```
Un POKéMON de cette magnificence…\nIl n'y en a pas deux comme ça.\pTout le monde sera subjugué par tant\nde beauté.$
```
### LilycoveCity_ContestHall_Text_OverdidGrooming
```
Ahhhhh!\pJ'ai brossé et coiffé mon POKéMON\navec soin pour ce CONCOURS…\pMais j'ai exagéré…\nMaintenant, il a le poil tout ébouriffé…\pQu'est-ce que je peux faire?$
```
### LilycoveCity_ContestHall_Text_JudgeWontSeeAuroraBeam
```
Cette ONDE BOREALE est si éblouissante\nque le JUGE ne pourra pas la voir.\pEuh, attends…\nMais ça sert à rien, alors!$
```
### LilycoveCity_ContestHall_Text_PokemonLooksLikeYoungerMe
```
Ce joli POKéMON, c'est exactement moi\nquand j'étais plus jeune.\pJuste au moment où je l'ai dit, mon mari\ns'est étouffé avec son café.\pJ'ai dit quelque chose d'amusant?$
```
### LilycoveCity_ContestHall_Text_WinBeautyContestMakesMeHappy
```
Tu sais, en tant que femme, je serais\nplus qu'heureuse de gagner un CONCOURS\lDE BEAUTE.$
```
### LilycoveCity_ContestHall_Text_GanderAtAllThosePrettyPokemon
```
Waouhhhh!\pTu devrais jeter un coup d'œil\nà ces jolis POKéMON.\pQuelle splendeur! J'adore!$
```
### LilycoveCity_ContestHall_Text_CantWinOnBeautyAlone
```
Tu ne peux pas toujours compter\nsur la beauté uniquement.\pTu dois aussi coiffer ton POKéMON, pour\nqu'il soit aussi brillant que le mien.$
```
### LilycoveCity_ContestHall_Text_InTheMiddleOfContest
```
ANIMATRICE: Hum! Ohé!\nOn est en plein CONCOURS!\pVeuillez vous inscrire au guichet et\nrevenez ensuite, vous voulez bien?$
```
### LilycoveCity_ContestHall_Text_SuchCharmingCuteAppeals
```
JUGE: Oh, quelles démonstrations\nravissantes et gracieuses!\pJuste ciel! Quelle parfaite et adorable\ndémonstration de TOURNIQUET!$
```
### LilycoveCity_ContestHall_Text_MyAzurillWasDistracted
```
Oh, non… Mon doux AZURILL s'est\nlaissé distraire par un autre POKéMON.$
```
### LilycoveCity_ContestHall_Text_NeverWonBattleButContest
```
Mon POKéMON ne gagne jamais de combat,\nmais en CONCOURS, c'est le meilleur!$
```
### LilycoveCity_ContestHall_Text_PetalDanceIsMarvel
```
La DANSE-FLEUR de mon POKéMON est une\nmerveille d'élégance.\pJe ne laisserai personne perturber sa\nprestation.$
```
### LilycoveCity_ContestHall_Text_MyMonAppealSoMuchCuter
```
Tous les POKéMON sont gracieux.\pMais si mon POKéMON faisait une\ndémonstration de grâce…\pJe suis sûre qu'il serait beaucoup plus\ngracieux que les autres.$
```
### LilycoveCity_ContestHall_Text_MyChildIsInContest
```
Mon enfant participe au CONCOURS.$
```
### LilycoveCity_ContestHall_Text_ComeOnDear
```
Vas-y, courage!\nTon POKéMON est le meilleur!$
```
### LilycoveCity_ContestHall_Text_ThatGirlThereIsCutest
```
Je trouve que cette fille est vraiment\nla plus gracieuse.\pComment? Ils jugent les POKéMON\nsur leur apparence?$
```
### LilycoveCity_ContestHall_Text_WantCuteMonOfMyOwn
```
Oh, regarder tous ces gracieux POKéMON\nme donne envie d'en avoir un!\pJe vais aller en attraper quelques-uns!$
```
### LilycoveCity_ContestHall_Text_BeautyContestStage
```
EPREUVE DE BEAUTE\nPOKéMON D'UNE BEAUTE EPOUSTOUFLANTE!$
```
### LilycoveCity_ContestHall_Text_CuteContestStage
```
EPREUVE DE GRACE\nPOKéMON D'UNE GRACE EXTREME!$
```
### LilycoveCity_ContestHall_Text_SmartContestStage
```
EPREUVE D'INTELLIGENCE\nPOKéMON D'UNE INTELLIGENCE INOUIE!$
```
