...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_FISHERMAN` | 2,4 | `MOVEMENT_TYPE_FACE_RIGHT` | `SootopolisCity_LotadAndSeedotHouse_EventScript_LotadBrother` | `0` |
| `` | `OBJ_EVENT_GFX_HIKER` | 5,4 | `MOVEMENT_TYPE_FACE_LEFT` | `SootopolisCity_LotadAndSeedotHouse_EventScript_SeedotBrother` | `0` |

## Warps (2)
- #0 (3,6) → `MAP_SOOTOPOLIS_CITY` warp #11
- #1 (4,6) → `MAP_SOOTOPOLIS_CITY` warp #11

## BG events / signs (2)
- (5,1) [sign] → `SootopolisCity_LotadAndSeedotHouse_EventScript_SeedotSizeRecord`
- (2,1) [sign] → `SootopolisCity_LotadAndSeedotHouse_EventScript_LotadSizeRecord`
...
```
Tu connais le POKéMON GRAINIPIOT?\nIl se fait rare à ATALANOPOLIS.\pMoi, j'adore les grands GRAINIPIOT.\nPlus ils sont grands et mieux c'est.\pMais mon p'tit frère, il dit que les\nNENUPIOT sont plus grands.\pC'est pas vrai. Les GRAINIPIOT\nsont encore plus grands!\pHum? Tu as un GRAINIPIOT avec toi?\nOh, fais-moi voir, s'il te plaît!$
```
### SootopolisCity_LotadAndSeedotHouse_Text_GoshMightBeBiggerThanLotad
```
{STR_VAR_2} cm!\nWaouh, il est grand celui-là!\pIl pourrait même dépasser le grand\nNENUPIOT que mon p'tit frère a vu!\pMerci de me l'avoir montré.\nTiens, c'est ma façon de te remercier!$
```
...
```
Oh?\nTon SAC est carrément plein.$
```
### SootopolisCity_LotadAndSeedotHouse_Text_SeenBiggerSeedot
```
{STR_VAR_2} cm, c'est ça?\pHum… J'ai vu un plus grand GRAINIPIOT\nque celui-là.$
```
### SootopolisCity_LotadAndSeedotHouse_Text_ThatsNotSeedot
```
Ah oui, c'est quelque chose!\nMais c'est pas un GRAINIPIOT!$
```
### SootopolisCity_LotadAndSeedotHouse_Text_DontHaveBigSeedot
```
Tu n'as pas de grand GRAINIPIOT?\nQuel dommage!\pSi tu trouves un grand GRAINIPIOT,\nviens me le montrer.$
```
### SootopolisCity_LotadAndSeedotHouse_Text_BiggestSeedotInHistory
```
Le plus grand GRAINIPIOT jamais vu!\nUn géant de {STR_VAR_3} cm appartenant\là {STR_VAR_2}!\pUn GRAINIPIOT encore plus\ngrand qu'un NENUPIOT!$
```
### SootopolisCity_LotadAndSeedotHouse_Text_PleaseShowMeBigLotad
```
Tu connais le POKéMON NENUPIOT?\nOn en voit rarement à ATALANOPOLIS.\pJ'adore, j'adore les grands NENUPIOT!\pMon grand frère dit que le GRAINIPIOT\nest plus grand.\pMais c'est faux! Tout le monde sait\nbien que le NENUPIOT est plus grand.\pHum? Tu as un NENUPIOT?\nFais-moi voir, s'il te plaît!$
```
### SootopolisCity_LotadAndSeedotHouse_Text_WowMightBeBiggerThanSeedot
```
{STR_VAR_2} cm!\nWaouh, c'est beaucoup!\pC'est peut-être encore plus grand que\nl'énorme GRAINIPIOT que mon frère a vu.\pMerci de me l'avoir montré!\nTiens, c'est pour te remercier!$
```
...
```
{STR_VAR_2} cm?\pHum… J'ai vu un plus grand NENUPIOT\nque celui-là.$
```
### SootopolisCity_LotadAndSeedotHouse_Text_ThatsNotLotad
```
Eh bien, ça c'est quelque chose!\nMais c'est pas un NENUPIOT!$
```
### SootopolisCity_LotadAndSeedotHouse_Text_DontHaveBigLotad
```
Tu n'as pas de grand NENUPIOT?\nC'est dommage…\pSi tu trouves un grand NENUPIOT,\nviens me le montrer.$
```
### SootopolisCity_LotadAndSeedotHouse_Text_BiggestLotadInHistory
```
Le plus grand NENUPIOT jamais vu!\nUn colosse de {STR_VAR_3} cm appartenant\là {STR_VAR_2}!\pUn NENUPIOT encore plus\ngrand qu'un GRAINIPIOT!$
```
