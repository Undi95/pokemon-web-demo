...
|---|---|---|---|---|---|
| `LOCALID_LILYCOVE_HARBOR_ATTENDANT` | `OBJ_EVENT_GFX_BEAUTY` | 8,10 | `MOVEMENT_TYPE_FACE_DOWN` | `LilycoveCity_Harbor_EventScript_FerryAttendant` | `FLAG_HIDE_LILYCOVE_HARBOR_FERRY_ATTENDANT` |
| `LOCALID_LILYCOVE_HARBOR_SS_TIDAL` | `OBJ_EVENT_GFX_SS_TIDAL` | 8,9 | `MOVEMENT_TYPE_FACE_RIGHT` | `0x0` | `FLAG_HIDE_LILYCOVE_HARBOR_SSTIDAL` |
| `` | `OBJ_EVENT_GFX_SAILOR` | 3,13 | `MOVEMENT_TYPE_WANDER_AROUND` | `LilycoveCity_Harbor_EventScript_Sailor` | `0` |
| `LOCALID_LILYCOVE_HARBOR_FERRY_SAILOR` | `OBJ_EVENT_GFX_SAILOR` | 8,10 | `MOVEMENT_TYPE_FACE_DOWN` | `0x0` | `FLAG_HIDE_LILYCOVE_HARBOR_FERRY_SAILOR` |
| `LOCALID_LILYCOVE_HARBOR_BRINEY` | `OBJ_EVENT_GFX_EXPERT_M` | 8,10 | `MOVEMENT_TYPE_FACE_DOWN` | `0x0` | `FLAG_HIDE_LILYCOVE_HARBOR_EVENT_TICKET_TAKER` |

## Warps (2)
- #0 (11,14) → `MAP_LILYCOVE_CITY` warp #12
- #1 (12,14) → `MAP_LILYCOVE_CITY` warp #12
...
```
Alors, cap sur POIVRESSEL!$
```
### LilycoveCity_Harbor_Text_BattleFrontierItIs
```
Alors, cap sur la ZONE DE COMBAT!$
```
...
```
Nous, les marins, nous ne pouvons rien\nfaire tant que le ferry n'est pas fini…\pJ'espère qu'ils vont s'activer, les gars\ndu CHANTIER NAVAL de POIVRESSEL.$
```
### LilycoveCity_Harbor_Text_SailorFerryAvailable
```
Le ferry LE MARINA est enfin\nopérationnel.\pLes gens du CHANTIER NAVAL de\nPOIVRESSEL ont dû travailler dur.\pMaintenant, à mon tour de bosser!$
```
