...
|---|---|---|---|---|---|
| `LOCALID_TOWER_BATTLE_OPPONENT` | `OBJ_EVENT_GFX_VAR_0` | 5,1 | `MOVEMENT_TYPE_FACE_DOWN` | `0x0` | `FLAG_HIDE_BATTLE_TOWER_OPPONENT` |
| `LOCALID_TOWER_BATTLE_ATTENDANT_1` | `OBJ_EVENT_GFX_TEALA` | 1,7 | `MOVEMENT_TYPE_FACE_RIGHT` | `0x0` | `0` |
| `LOCALID_TOWER_BATTLE_ATTENDANT_2` | `OBJ_EVENT_GFX_TEALA` | 4,8 | `MOVEMENT_TYPE_FACE_UP` | `0x0` | `0` |

## Warps (2)
- #0 (5,8) → `MAP_BATTLE_FRONTIER_BATTLE_TOWER_LOBBY` warp #2
- #1 (6,8) → `MAP_BATTLE_FRONTIER_BATTLE_TOWER_LOBBY` warp #2
...
```
Voulez-vous annuler le défi de la\nSALLE DE COMBAT?$
```
...
```
Vous savez…\pNotre chef, l'AS DU SALON, arrive.\nElle espère pouvoir vous affronter.\pElle sera ici d'une minute à l'autre.$
```
### BattleFrontier_BattleTowerBattleRoom_Text_ReadyForSalonMaiden
```
Vous allez affronter l'AS DU SALON.\nPouvons-nous commencer?$
```
### BattleFrontier_BattleTowerBattleRoom_Text_GreetingsImAnabel
```
Bonjour…\nJe m'appelle CATHY.\pJe suis l'AS DU SALON et je suis la\nresponsable de la TOUR DE COMBAT…\pJe dois dire que j'ai beaucoup entendu\nparler de toi…\pCe que j'ai entendu n'était pas\ntoujours très flatteur, d'ailleurs…\pTu dois te demander ce que je fais\nici, alors… Eh bien, moi aussi!$
```
### BattleFrontier_BattleTowerBattleRoom_Text_LetMeSeeYourTalent
```
Allez, essaie de m'éblouir par ton\ntalent…$
```
### BattleFrontier_BattleTowerBattleRoom_Text_AnabelTalentShallBeRecognized
```
CATHY: Oh, oh…\pTon PASSE ZONE, s'il te plaît…\nTon talent doit être reconnu.$
```
### BattleFrontier_BattleTowerBattleRoom_Text_ReceivedAbilitySymbol
```
Le SYMBOLE CAPACITE a été ajouté\nsur le PASSE ZONE!$
```
### BattleFrontier_BattleTowerBattleRoom_Text_UntilNextTime
```
… … … … … …\pTu as une réelle confiance en tes\nPOKéMON, n'est-ce pas?\pJ'ai hâte de t'affronter à nouveau\net de gagner!\pJe t'attendrai le temps qu'il faudra.\nNous nous reverrons, j'en suis sûre…$
```
### BattleFrontier_BattleTowerBattleRoom_Text_AnabelYouCameBack
```
CATHY: Toi? Je n'étais pas sûre\nque tu reviennes me voir…\p… … … … … …\pTu as gagné beaucoup de combats…\nJe ne te faciliterai pas la tâche…\pCela fait si longtemps…\pCela fait si longtemps que j'attends ce\nmoment…$
```
...
```
CATHY: Eh bien, félicitations…\nTon PASSE ZONE, s'il te plaît…$
```
### BattleFrontier_BattleTowerBattleRoom_Text_AbilitySymbolTookGoldenShine
```
Le SYMBOLE CAPACITE a pris une couleur\ndorée!$
```
### BattleFrontier_BattleTowerBattleRoom_Text_WishICouldBattleYouAgain
```
Quel beau combat…\pJe crois que je n'avais jamais autant\napprécié un combat de POKéMON…\pJ'espère avoir l'occasion de me battre\nà nouveau contre toi…$
```
