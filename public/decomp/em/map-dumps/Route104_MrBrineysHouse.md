...
|---|---|---|---|---|---|
| `LOCALID_BRINEYS_HOUSE_BRINEY` | `OBJ_EVENT_GFX_EXPERT_M` | 5,3 | `MOVEMENT_TYPE_WALK_SEQUENCE_DOWN_LEFT_UP_RIGHT` | `Route104_MrBrineysHouse_EventScript_Briney` | `FLAG_HIDE_BRINEYS_HOUSE_MR_BRINEY` |
| `LOCALID_BRINEYS_HOUSE_PEEKO` | `OBJ_EVENT_GFX_WINGULL` | 6,3 | `MOVEMENT_TYPE_FACE_DOWN` | `Route104_MrBrineysHouse_EventScript_Peeko` | `FLAG_HIDE_BRINEYS_HOUSE_PEEKO` |

## Warps (2)
- #0 (5,8) → `MAP_ROUTE104` warp #0
- #1 (6,8) → `MAP_ROUTE104` warp #0
...
```
M. MARCO: Attends, mon p'tit!\nPIKO, viens ici!$
```
### Route104_MrBrineysHouse_Text_ItsYouLetsSailToDewford
```
Hein? Tu es {PLAYER}{KUN}!\nTu as sauvé mon PIKO adoré!\lNous te devons une fière chandelle!\pComment?\nTu veux embarquer avec moi?\pHumm…\pAlors comme ça, tu as une LETTRE à\ndestination de MYOKARA et un\lpaquet à déposer à POIVRESSEL?\pTu ne dois pas avoir le temps\nde t'ennuyer!\pQuoi qu'il en soit, pas de problème.\nJe peux t'aider.\pTu as frappé à la bonne porte!\nOn met le cap sur MYOKARA.$
```
### Route104_MrBrineysHouse_Text_SetSailForDewford
```
M. MARCO: VILLAGE MYOKARA,\nnous voilà!\pLevons l'ancre!\nPIKO, mon chouchou, on met les voiles!$
```
### Route104_MrBrineysHouse_Text_DeclineDeliverySail
```
M. MARCO: Ah bon?\nTa livraison peut attendre?\pDans ce cas, reviens me voir quand tu\nvoudras mettre les voiles!$
```
### Route104_MrBrineysHouse_Text_NeedToMakeDeliveriesSailToDewford
```
M. MARCO: Ohé!\nJe vois très bien ce que tu veux dire!\pTu dois livrer une LETTRE à MYOKARA\net un paquet à POIVRESSEL.\pÇa n'me pose pas d'problème. Je suis\nl'homme de la situation!\pOn mettra d'abord le cap sur MYOKARA.$
```
### Route104_MrBrineysHouse_Text_NeedToDeliverPackageSailToDewford
```
M. MARCO: Ohé!\nJe vois très bien ce que tu veux dire!\pTu dois livrer un paquet au CAPT. POUPE\nà POIVRESSEL.\pÇa n'me pose pas d'problème. Je suis\nl'homme de la situation!\pOn mettra d'abord le cap sur MYOKARA.$
```
### Route104_MrBrineysHouse_Text_WhereAreWeBound
```
M. MARCO: Ohé!\nJe peux appareiller quand tu veux!\pBien! Où va-t-on, mon pote?$
```
### Route104_MrBrineysHouse_Text_TellMeWheneverYouWantToSail
```
M. MARCO: Ah bon?\nBah, PIKO te doit la vie.\pTu peux revenir quand tu veux et\nje mettrai les voiles pour toi!$
```
