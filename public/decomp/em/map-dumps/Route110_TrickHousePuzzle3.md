...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_CAMPER` | 7,19 | `MOVEMENT_TYPE_FACE_DOWN` | `Route110_TrickHousePuzzle3_EventScript_Justin` | `0` |
| `` | `OBJ_EVENT_GFX_PICNICKER` | 4,4 | `MOVEMENT_TYPE_FACE_RIGHT` | `Route110_TrickHousePuzzle3_EventScript_Martha` | `0` |
| `` | `OBJ_EVENT_GFX_HIKER` | 10,10 | `MOVEMENT_TYPE_FACE_DOWN` | `Route110_TrickHousePuzzle3_EventScript_Alan` | `0` |
| `` | `OBJ_EVENT_GFX_ITEM_BALL` | 1,2 | `MOVEMENT_TYPE_LOOK_AROUND` | `Route110_TrickHousePuzzle3_EventScript_ItemWoodMail` | `FLAG_ITEM_TRICK_HOUSE_PUZZLE_3_WOOD_MAIL` |
| `` | `OBJ_EVENT_GFX_ITEM_BALL` | 4,2 | `MOVEMENT_TYPE_LOOK_AROUND` | `Route110_TrickHousePuzzle3_EventScript_ItemShadowMail` | `FLAG_ITEM_TRICK_HOUSE_PUZZLE_3_SHADOW_MAIL` |
| `` | `OBJ_EVENT_GFX_BREAKABLE_ROCK` | 1,20 | `MOVEMENT_TYPE_LOOK_AROUND` | `EventScript_RockSmash` | `FLAG_TEMP_15` |
| `` | `OBJ_EVENT_GFX_BREAKABLE_ROCK` | 2,21 | `MOVEMENT_TYPE_LOOK_AROUND` | `EventScript_RockSmash` | `FLAG_TEMP_16` |
...
- (4,14) → `Route110_TrickHousePuzzle3_EventScript_Button1` (si `VAR_TEMP_1` == `0`)
- (3,11) → `Route110_TrickHousePuzzle3_EventScript_Button2` (si `VAR_TEMP_2` == `0`)
- (12,5) → `Route110_TrickHousePuzzle3_EventScript_Button3` (si `VAR_TEMP_3` == `0`)
- (8,2) → `Route110_TrickHousePuzzle3_EventScript_Button4` (si `VAR_TEMP_4` == `0`)

## BG events / signs (1)
- (0,14) [sign] → `Route110_TrickHousePuzzle3_EventScript_Scroll`
...
```
{PLAYER} écrit le code secret\nsur la porte.\p“Le MAITRE DES PIEGES est envié.”\n… … … … … … … …\pLa porte s'ouvre!$
```
...
```
Je suis déjà perdu, et en plus, tu\nme bats? C'est vraiment trop injuste!$
```
### Route110_TrickHousePuzzle3_Text_JustinPostBattle
```
Il y a trop de portes, ici! C'est trop\npetit et on n'y voit rien! A l'aide!$
```
...
```
Je sais que je suis faible!\nEt je n'ai aucun sens de l'orientation!$
```
### Route110_TrickHousePuzzle3_Text_AlanIntro
```
Je ne comprends pas. Qui peut bien\ns'amuser dans une maison aussi bizarre?$
```
### Route110_TrickHousePuzzle3_Text_AlanDefeat
```
Je ne comprends pas.\nComment ai-je fait pour perdre?$
```
### Route110_TrickHousePuzzle3_Text_AlanPostBattle
```
Je ne comprends pas. Combien de pièges\ny a-t-il dans cette maison?\pTu me donnes une réponse?$
```
