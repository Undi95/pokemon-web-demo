...
|---|---|---|---|---|---|
| `` | `OBJ_EVENT_GFX_WINONA` | 15,2 | `MOVEMENT_TYPE_FACE_DOWN` | `FortreeCity_Gym_EventScript_Winona` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_5` | 4,14 | `MOVEMENT_TYPE_FACE_DOWN` | `FortreeCity_Gym_EventScript_Jared` | `0` |
| `` | `OBJ_EVENT_GFX_CAMPER` | 10,10 | `MOVEMENT_TYPE_FACE_LEFT` | `FortreeCity_Gym_EventScript_Flint` | `0` |
| `` | `OBJ_EVENT_GFX_PICNICKER` | 5,17 | `MOVEMENT_TYPE_FACE_LEFT` | `FortreeCity_Gym_EventScript_Ashley` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_5` | 9,8 | `MOVEMENT_TYPE_FACE_DOWN` | `FortreeCity_Gym_EventScript_Edwardo` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_2` | 15,21 | `MOVEMENT_TYPE_FACE_DOWN` | `FortreeCity_Gym_EventScript_GymGuide` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_5` | 4,23 | `MOVEMENT_TYPE_FACE_RIGHT` | `FortreeCity_Gym_EventScript_Humberto` | `0` |
| `` | `OBJ_EVENT_GFX_MAN_5` | 1,10 | `MOVEMENT_TYPE_FACE_UP` | `FortreeCity_Gym_EventScript_Darius` | `0` |

## Warps (2)
- #0 (15,24) → `MAP_FORTREE_CITY` warp #2
- #1 (16,24) → `MAP_FORTREE_CITY` warp #2

## BG events / signs (2)
- (14,21) [sign] → `FortreeCity_Gym_EventScript_LeftGymStatue`
- (17,21) [sign] → `FortreeCity_Gym_EventScript_RightGymStatue`
...
```
Yo, ça baigne pour toi, futur\nMAITRE {PLAYER}?\pALIZEE, CHAMPION D'ARENE de\nCIMETRONELLE est une spécialiste\ldes POKéMON du type VOL.\pElle attend au fond de cette ARENE,\nderrière les tourniquets.\pElle attend de nouveaux challengeurs\nqui essaient de prendre leur envol!\pOK, vas-y!$
```
...
```
Regarde l'élégance des POKéMON\nOISEAU au combat!$
```
### FortreeCity_Gym_Text_JaredDefeat
```
Toi…\nTu as une telle puissance…$
```
### FortreeCity_Gym_Text_JaredPostBattle
```
Un DRESSEUR doit être rusé pour faire\nface aux retournements inattendus.\pVoyons si tu as assez de jugeote pour\narriver jusqu'à notre CHAMPION!$
```
### FortreeCity_Gym_Text_EdwardoIntro
```
Tous les amoureux des POKéMON OISEAU\nveulent rejoindre cette ARENE.\pEn tant que membre de l'ARENE de\nCIMETRONELLE, je ne peux pas perdre!$
```
### FortreeCity_Gym_Text_EdwardoDefeat
```
C'était un poids trop lourd à\nporter pour moi…$
```
### FortreeCity_Gym_Text_EdwardoPostBattle
```
Le monde est grand et des tas de\nDRESSEURS très puissants le peuplent.\pJe vais continuer à m'entraîner pour\ndevenir encore plus fort.$
```
### FortreeCity_Gym_Text_FlintIntro
```
Pas la peine pour ALIZEE, notre\nCHAMPION, de s'attarder sur ton cas!\pJe suis bien assez fort pour toi!$
```
...
```
ALIZEE est maligne et très forte.\nElle représente le CHAMPION absolu!\pOh, je rougis…$
```
### FortreeCity_Gym_Text_AshleyIntro
```
ALIZEE m'a tout appris!\nTu vas avoir du mal à me battre!$
```
...
```
Grâce à ALIZEE, les habitants de\nCIMETRONELLE peuvent vivre sans peur.$
```
### FortreeCity_Gym_Text_HumbertoIntro
```
Quand ALIZEE se lance dans un combat,\nson visage s'illumine…\pMais je ne te laisserai pas voir ça\nde tes petits yeux!$
```
...
```
Tu ferais bien de te méfier!\nNe te laisse pas distraire par le visage\ld'ALIZEE ou tu risques de le regretter!$
```
### FortreeCity_Gym_Text_DariusIntro
```
Tu ferais bien de savoir qu'il y a\nplusieurs sortes de POKéMON de\ltype VOL.\pTu le sais, hein?$
```
...
```
OK, tu m'as battu.\nMais tu ferais bien de faire attention!\pLes POKéMON de notre CHAMPION\nvont te ramener sur terre!$
```
### FortreeCity_Gym_Text_WinonaIntro
```
Je suis ALIZEE, CHAMPION\nD'ARENE de CIMETRONELLE.\pJe me suis unie avec les POKéMON\nOISEAU et envolée dans les airs…\pMême si le combat a été exténuant,\non l'a emporté avec grâce…\pAdmire l'élégante chorégraphie que mes\nPOKéMON OISEAU et moi avons préparée!$
```
### FortreeCity_Gym_Text_WinonaDefeat
```
Je n'avais encore jamais vu de DRESSEUR\ncommander ses POKéMON avec autant de\lgrâce que moi. Tu es le premier…\pEn récompense de ta prouesse, je\nt'offre ce BADGE.$
```
### FortreeCity_Gym_Text_ReceivedFeatherBadge
```
{PLAYER} reçoit le BADGE PLUME\nde la part d'ALIZEE.$
```
### FortreeCity_Gym_Text_ExplainFeatherBadgeTakeThis
```
Avec le BADGE PLUME, tous les POKéMON\njusqu'au niveau 70, même ceux échangés,\lobéiront à tous tes ordres.\pTu pourras aussi utiliser l'attaque CS\nVOL en dehors des combats.\pEt ça…\nC'est un cadeau que je te fais.$
```
### FortreeCity_Gym_Text_ExplainAerialAce
```
La CT40 contient AEROPIQUE.\pSa vitesse…\nAucun POKéMON ne pourra l'éviter.\p… … … … … …$
```
### FortreeCity_Gym_Text_RegisteredWinona
```
Vous avez enregistré le CHAMPION\nD'ARENE ALIZEE dans le POKéNAV.$
```
### FortreeCity_Gym_Text_WinonaPostBattle
```
Même après ma défaite, je reste dévouée\naux POKéMON OISEAU.$
```
...
```
ARENE POKéMON de CIMETRONELLE\pNOUVEAU DRESSEUR RECONNU PAR ALIZEE:\n{PLAYER}$
```
### FortreeCity_Gym_Text_WinonaPreRematch
```
ALIZEE: Nous autres humains,\nnous ne pouvons pas échapper à la\lforce de la gravité.\pMais dans notre quête d'excellence,\nnous parvenons à nous élever!\pTu as atteint les sommets en\ndevenant le MAITRE.\pS'il te plaît, laisse-moi voir\nl'étendue de ta puissance!$
```
### FortreeCity_Gym_Text_WinonaRematchDefeat
```
J'ai encore échoué… Je n'ai pas\npu atteindre ton niveau…$
```
### FortreeCity_Gym_Text_WinonaPostRematch
```
ALIZEE: Même dans la défaite, les ailes\nde mon cœur restent déployées.\pJe pourrai m'élever encore et toujours.\pJ'en suis convaincue!$
```
### FortreeCity_Gym_Text_WinonaRematchNeedTwoMons
```
ALIZEE: Nous autres humains,\nnous ne pouvons pas échapper à la\lforce de la gravité.\pMais dans notre quête d'excellence,\nnous parvenons à nous élever!\pTu as atteint les sommets en\ndevenant le MAITRE.\pS'il te plaît, laisse-moi voir l'étendue\nde ta puissance!\pOh, non, tu n'as qu'un POKéMON\navec toi?\pS'il te plaît, reviens me voir avec au\nmoins deux POKéMON.$
```
